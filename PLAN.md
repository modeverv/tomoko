...
## Phase S18: two-stage gates and internal turn-opportunity WebSocket

発話判断計算モデルを `InferenceStartGate` と `SpeechEmissionGate` に分割し、
hot-path の MaAI/VAP 由来 signal を 200ms 程度の `TurnOpportunitySnapshot` として
tomoko-process 側へ internal WebSocket で渡す。

この snapshot は durable log ではなく latest-wins の realtime 制御材料である。
DB / NOTIFY には raw MaAI/VAP frame を流さない。

### 実装手順

- [x] `TurnOpportunitySnapshot` DTO を追加する。
- [x] `InferenceStartGateInput` / `InferenceStartGateOutput` を追加する。
- [x] `SpeechEmissionGateInput` / `SpeechEmissionGateOutput` を追加する。
- [x] `server.tomoko.gates` に二段 gate の純粋計算を実装する。
- [x] `TomokoConversationCore` で `InferenceStartGate` -> `SpeechScheduler` -> `SpeechEmissionGate` の順に判断する。
- [x] hot-path に `TurnSignalAggregator` を追加し、audio RMS と MaAI result を 200ms snapshot に集約する。
- [x] hot-path から Tomoko realtime app へ `turn_opportunity` JSON を送る internal WebSocket client を追加する。
- [x] `server.tomoko.realtime` に `/internal/hot-path` WebSocket endpoint を追加し、latest snapshot を保持する。
- [x] direct hot-path conversation では同じ snapshot を in-process `TomokoConversationCore` にも反映する。
- [x] unit test で gate 計算、snapshot 集約、internal WS 受信を固定する。

### 完了条件

- [x] `InferenceStartGate` が partial/final STT、semantic saturation、無音、外部調査、calendar、motivation を発話候補生成 fire 判断に分けて扱える。
- [x] `SpeechEmissionGate` が生成済み候補の送出を、motivation、semantic confidence、interruption risk、stop intent と競合させて判断できる。
- [x] hot-path から Tomoko 側へ 200ms `TurnOpportunitySnapshot` を JSON WebSocket で送れる。
- [x] `pytest -m unit` が通る。

## Phase S19: Material -> Pressure -> Gate vocabulary and scheduler removal

S18 の `TurnOpportunitySnapshot` / `TurnSignalAggregator` / `SpeechScheduler` 中心の整理を、
`Materials -> Pressures -> Gates` の設計語彙へ置き換える。

`input` は gate への入力なのか raw signal なのか曖昧なので使わない。
音声情報、無音時間、VAP/MaAI、STT、外部情報、人格傾向は `materials` と呼ぶ。
materials から `DialogueTurnPressure` / `NaturalSpeechPressure` /
`MotivationPressure` / `WorldPressure` を計算し、gate は pressure と candidate を読む。

### 実装手順

- [x] `TurnOpportunitySnapshot` を `TurnMaterials` に改名し、WS payload を `turn_materials` にする。
- [x] `WorldMaterials` / `PersonalityMaterials` を追加する。
- [x] `DialogueTurnPressure` / `NaturalSpeechPressure` / `MotivationPressure` / `WorldPressure` DTO を追加する。
- [x] `server.tomoko.pressures` に pressure model を追加する。
- [x] `InferenceStartGateInput` / `SpeechEmissionGateInput` を pressure/candidate ベースへ変更する。
- [x] `TomokoConversationCore` の通常発話裁定経路から `SpeechScheduler.decide()` を外す。
- [x] 通常経路を `materials -> pressures -> InferenceStartGate -> LLM -> PreparedSpeechCandidate -> SpeechEmissionGate -> SpeechOrder` にする。
- [x] unit test を新しい語彙へ更新する。

### 完了条件

- [x] `SpeechEmissionGate` が LLM 後の `PreparedSpeechCandidate` を受けて hot-path 送出可否を決める。
- [x] hot-path から Tomoko 側へ 200ms `TurnMaterials` を JSON WebSocket で送れる。
- [x] `pytest -m unit` が通る。

## Phase S20: pressure synthesis gate before LLM

S19 の `InferenceStartGate` は、名前と enum がまだ `main_reply` /
`initiative` / `external_summary` のような intent 的な分岐を含んでいた。
LLM 前の責務は intent 分岐ではなく、materials から作った pressures を合成して
LLM に行くかだけを決める gate に限定する。

### 実装手順

- [x] `InferenceStartDecision` を `LlmFireDecision` に置き換える。
- [x] decision は `do_not_fire` / `fire` / `cancel_or_replace_pending` だけにする。
- [x] `InferenceStartGateInput` / `Output` を `LlmFireGateInput` / `Output` に置き換える。
- [x] `InferenceStartGate` を `LlmFireGate` に置き換える。
- [x] `LlmFireGate` は pressure score を合成し、pressure source ごとの intent 分岐を持たない。
- [x] `TomokoConversationCore` は `materials -> pressures -> LlmFireGate -> LLM -> PreparedSpeechCandidate -> SpeechEmissionGate` の順にする。
- [x] unit test を `FIRE` / `DO_NOT_FIRE` ベースに更新する。
