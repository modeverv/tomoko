...
- [ ] `stop_intent_observations` テーブルを追加する
  - `id UUID PRIMARY KEY`
  ...
  - `error TEXT`
- [ ] `stop_intent_shadow_signals` テーブルを追加する
  - `id UUID PRIMARY KEY`
  ...
  - `created_at TIMESTAMPTZ NOT NULL DEFAULT now()`
- [ ] pending / processing / completed / error を PostgreSQL の row state として管理する
- [ ] `FOR UPDATE SKIP LOCKED` で最大1件ずつ処理できる store method を用意する
- [ ] retry は `attempts` と `locked_at` で管理し、古い lock は回収できるようにする
- [ ] SQL 分析しやすいよう、method / predicted_kind / confidence / latency の index を追加する

### Phase 10.9.1: hot path は observation insert だけにする

- [ ] `BargeInDetector` / `_withdraw_decision` / `initiative_feedback.classify_feedback()` の既存ルール判定は維持する
- [ ] transcript が stop / wait / withdraw / defer / accept の候補になりうる時、`TomoroSession` は observation を作る
- [ ] `/ws` hot path では embedding / LLM を直接 await しない
- [ ] `TomoroSession` は observation を PostgreSQL に保存する command だけを返す
- [ ] DB insert が失敗しても、stop / interrupt の本体制御を失敗させない
- [ ] observation には、あとで採否を分析できる state snapshot を残す
...
- [ ] `StopIntentClassifierWorker` を追加する
- [ ] worker は PostgreSQL から pending observation を古い順に取得する
- [ ] embedding classifier と LLM classifier を observation ごとに実行する
- [ ] LLM classifier は最大1同時に制限する
  - process 内では `asyncio.Semaphore(1)`
  - 複数 process 起動時は PostgreSQL lock により同じ observation を二重処理しない
- [ ] embedding classifier は将来 concurrency を上げられる設計にするが、初期値は1でよい
- [ ] classifier failure / timeout / malformed JSON は observation を壊さず signal または error として保存する
- [ ] worker は `/ws` event drain や `TomoroSession` lock の中で動かさない
- [ ] backlog が増えても会話本体を止めない

### Phase 10.9.3: advisory result を SessionEvent として戻す

- [ ] classifier result を `SessionEvent(type="stop_intent_classified")` として `TomoroSession` に戻せる経路を追加する
- [ ] event payload は stale check に必要な id を必ず持つ
  - `observation_id`
  - `turn_id`
  - `transcript_id`
  - `method`
  ...
  - `confidence`
  - `latency_ms`
- [ ] `TomoroSession` は event 到着時に現在 turn と照合する
- [ ] すでに別 turn に進んでいる場合は `stale_stop_intent` としてログだけ残す
- [ ] confidence が低い result は制御に使わず、observation として保存する
- [ ] `hard_stop` / 高信頼 `soft_stop` / `withdraw` は、現在 turn に間に合う場合だけ会話停止 command へ変換する
- [ ] LLM result が遅れても、過去 turn の音声や表示を変更しない

### Phase 10.9.4: 固定 WAV「はい、止めます」で会話停止を完了する

- [ ] `assets/audio/stop_ack.wav` を追加する
  - 発話文は「はい、止めます」
  - 16kHz / mono / PCM WAV を基本にする
  - 生成方法と採用 voice を `_docs/latency.md` または実装コメントに残す
- [ ] `StopAckAudioProvider` を追加し、固定 WAV を `AudioChunkOut` として返せるようにする
- [ ] stop-intent 採用時、`TomoroSession` は以下を順に実行する command を返す
  ...
  - fixed WAV `audio_end`
- [ ] 固定 WAV は通常の Tomoko 返答として `conversation_logs` に保存しない
- [ ] 保存する場合は control event / interrupted turn として扱う
- [ ] fixed WAV の再生 telemetry は既存 playback echo protection に乗せる
- [ ] fixed WAV 自体の回り込み transcript は hard interrupt 以外 `echo` として扱う

### Phase 10.9.5: 分析と安全性

- [ ] SQL で rule / embedding / LLM の一致率を見られる view または query example を追加する
- [ ] `classification_arrived_before_first_reply_text` を分析できるようにする
- [ ] `classification_arrived_before_first_audio` を分析できるようにする
- [ ] `would_have_changed_action` を後から計算できるよう、adopted action と classifier result を両方残す
- [ ] LLM classifier の prompt は JSON-only schema にし、自由文で session を操作しない
- [ ] LLM classifier は `InferenceRouter.select("candidate_gen", "privacy")` など privacy-safe local backend を使う
- [ ] LLM classifier の raw output は長文保存せず、schema 化した `raw_reason_json` と短い reason に限定する
- [ ] queue depth / avg latency / p95 latency / error count を log に出す

### Phase 10.9.6: regression tests

- [ ] rule hard stop は embedding / LLM を待たず即 `audio_control stop` になる
- [ ] observation insert failure でも hard stop 本体は止まる
- [ ] worker は pending observation を `FOR UPDATE SKIP LOCKED` で1件だけ処理する
- [ ] LLM classifier は最大1同時で実行される
- [ ] stale `stop_intent_classified` event は会話制御に影響しない
- [ ] current turn に間に合った high-confidence stop は fixed WAV「はい、止めます」を送る
- [ ] fixed WAV は通常 conversation reply として保存されない
- [ ] fixed WAV playback 中の回り込みは echo protection に入る
- [ ] `pytest -m unit tests/unit/test_stop_intent_queue.py tests/unit/test_stop_ack_audio.py tests/unit/test_phase105_session_runtime.py` が通る
- [ ] `pytest -m integration tests/integration/test_stop_intent_db.py` が通る
- [ ] `pytest -m unit` が通る
