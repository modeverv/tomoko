...
- [ ] `conversation_sessions` テーブルを作成する
  ...
  - `summary_error TEXT`
- [ ] `conversation_logs` に `conversation_session_id UUID NULL` を追加する
  - `conversation_sessions(id)` を参照する
  - 既存ログは NULL のまま維持し、移行で無理に過去セッションを推定しない
- [ ] `TomoroSession` に現在の会話セッション ID を持たせる
  - authoritative state は引き続き `TomoroSession` が所有する
  - クライアントにセッション判断ロジックを置かない
- [ ] `attention_mode` が `ambient -> engaged` へ遷移した瞬間、または最初の `should_participate=True` 発話で会話セッションを開始する
...
- [ ] `ThinkFastMode` / `ThinkDeepMode` の外部契約は変えない
...
今後 `tools/bench_tts_backends.py` の出力先は `logs/tts-bench/` とする。
