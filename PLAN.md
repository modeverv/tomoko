...
これまでの段階では、VAD や VAP などのシステムによる自動判定タイミングのログ出力のみに依存して、ターンテイキングの評価や分析を行っていた。この「自動判定のログのみに頼る」評価方法を否定する。システム判定 of ログだけでは、人間が実際に「ここで話し終わった（応答してほしい）」と感じた主観的なタイミング（Ground Truth）と、システムが「発話終了」と判定したタイミングのミリ秒単位のズレや早とちりを客観的に可視化・最適化できないからである。したがって、本フェーズでは UI 上のボタン押下および Space キー押下を契機に、WebSocket 経由で発話終了シグナルを送信し、サーバー側の意思決定ログ（`logs/turn-taking-main.jsonl`）にそのタイムスタンプを記録するデバッグ機構を実装する。

### 完了条件

- [x] ブラウザ UI 上に「[発話終わり]」デバッグボタンを配置し、さらに Space キー押下によっても同様のシグナルが送信できる仕組みを実装する。
- [x] WebSocket を経由して、クライアントから `debug_user_speech_end` イベント（または既存のプロトコルに適合する形式）を送信する。
- [x] サーバー側（`TomoroSession` 側）で当該イベントを受信し、現在の `session_id`, `turn_id` などの文脈情報と共に意思決定ログ（`logs/turn-taking-main.jsonl`）にタイムスタンプを正確に記録する。
- [x] クライアントからのデバッグイベントの処理とログ出力を保証するユニットテストを追加する。
...
- [x] lora ディレクトリを作成する
- [x] システムプロンプトから学習用データ（対話データセット）を合成・生成するスクリプトを作成する
- [x] MLX-LM（あるいはPyTorch/PEFT）を用いたLoRAファインチューニングの実行スクリプトを作成する
- [x] 学習済みLoRAモデルの評価・推論スクリプトを作成する
- [x] 動作確認用のREADMEおよび使用手順書を作成する

## 2026-06-11 compress dynamic prompt contexts

動的コンテキスト（タスク、長期メモリ、短期メモリ、検索結果）の記述を冗長な表現から圧縮表現へ切り替える。
これにより、プロンプトのトークン消費を抑え、LLM のプレフィル負荷を軽減する。

### 完了条件

- [x] タスクコンテキストを `## TASKS` の形式で圧縮する
- [x] 長期メモリコンテキストを `## MEMORIES` の形式で圧縮する
- [x] 短期メモリコンテキストを `## SHORT MEMORY` (説明文カット) の形式で圧縮する
- [x] 検索結果コンテキストを `## RESEARCH` の形式で圧縮する
- [x] すべての関連ユニットテストがパスする
...
## 2026-05-30 MaAI dialogue raw score smoke

MaAI の診断を threshold 後の `BackchannelSuggestion` だけで見る方針は否定する。
`say` で user / Tomoko の二者会話 timeline を合成し、MaAI `bc_2type` から返る
raw `p_bc_react` / `p_bc_emo` をすべて JSON に出す smoke を追加する。

### 完了条件

- [x] `make smoke-maai-dialogue` で合成二者会話を MaAI へ流せる
- [x] JSON summary に `raw_scores[].p_bc_react` / `raw_scores[].p_bc_emo` と raw payload を出す
- [x] user / Tomoko 音声を同一 16kHz timeline の ch1 / ch2 frame として投入できる
- [x] focused unit / ruff / 実 smoke が通る

## 2026-05-30 MaAI LLM-less backchannel release

MaAI 由来の `BackchannelSuggestion` を観測だけで止める方針は否定する。
ただし会話 log や本返答 LLM へ混ぜず、TomoroSession が gesture audio として短い固定相槌を出す。

### 完了条件

- [x] `p_bc_react >= 0.68` の MaAI suggestion だけを release 対象にする
- [x] Tomoko が喋っていない、user が話している、同一 user speech segment で未発話、global cooldown 2000ms の時だけ release する
- [x] release する文言は `["うん", "なるほど", "そっか"]` から選ぶ
- [x] backchannel audio は conversation log に保存せず、通常 reply / TTS 経路と区別する
- [x] focused unit / ruff / full unit が通る

## 2026-05-30 MaAI dialogue session release smoke

`make smoke-maai-dialogue` を MaAI raw score / suggestion だけで終える方針は否定する。
同じ合成二者会話 timeline 上で MaAI suggestion を TomoroSession に流し、
`backchannel_released` / `backchannel_skipped` まで JSON に出す。

### 完了条件

- [x] JSON summary に `session_releases[]` を追加する
- [x] 各 entry に suggestion、timeline 上の user speaking / Tomoko speaking、session emission、audio bytes、reply_done control を出す
- [x] 実 `make smoke-maai-dialogue` で `backchannel_released` が観測できる
- [x] focused unit / ruff / 実 smoke が通る

## 2026-05-30 MaAI material WAV smoke

MaAI の公開サンプル音声確認を、手動で waveform を眺めるだけで終える方針は否定する。
`_tools/materials/maai.wav` を 16kHz 2ch timeline として MaAI 本体へ流し、
raw score / suggestion / TomoroSession release 判定を JSON に出す。

### 完了条件

- [x] stereo WAV を ch1=user / ch2=tomoko の 16kHz timeline として投入できる
- [x] JSON summary に raw score / suggestion / session release 判定を出す
- [x] `make smoke-maai-material` で `_tools/materials/maai.wav` を確認できる
- [x] focused unit / ruff / 実 smoke が通る
...
`low_audio_short_text` が低音量の短い誤認識を落とすための filter であるにもかかわらず、
`大変良いと思いますよ私は` のような成立した一文まで drop する挙動を否定する。
日本語では 20 文字以内でも普通の短文が成立するため、低音量 filter の「短い」の境界を狭める。

### 完了条件

- [x] 低音量 `大変良いと思いますよ私は` が accept される test を先に追加して赤を確認する
- [x] 既存の短い低音量 fragment / known hallucination / clock query 例外は維持する
- [x] focused unit / ruff が通る
- [x] full unit / global ruff が通る

### 検証メモ

- `LOW_AUDIO_SHORT_MAX_CHARS` は 20 から 6 へ狭める
- `いいと思います` は短いが成立した文として accept する
- `たぶんね` のような 6 文字以下の低音量 fragment は `low_audio_short_text` として drop する

## 2026-05-31 Research MCP command boundary initial integration

Perplexity / Chrome UI automation を Tomoko 本体へ import する方針は否定する。
Tomoko は rule-based に search intent を検出し、別 repo の `tomoko-research-operator`
を MCP-style subprocess として呼ぶ。ただし呼び出しは会話 hot path で同期的に待たず、
`SessionCommand` と runner を経由して background result を `SessionEvent` として戻す。

この Phase では DB 永続化と「教えて」で本文を読む UI/会話体験までは実装しない。
まず TomoroSession / gateway 境界で、検索 request と result ready がどう流れるかを固定する。

### 変更対象

- `server/gateway/research.py`
  - `ResearchIntentDetector`
  - `ResearchRequest` / `ResearchResult` / `ResearchCitation`
  - `ResearchMcpClient`
  - `ResearchCommandRunner`
- `server/session.py`
  - `research_requested` event から `submit_research_request` command を出す
  - `research_result_ready` event を受け、通知 candidate 相当の emission を出す
- `tests/unit/test_research_gateway.py`
  - rule detector、MCP result parser、citation URL dedupe、failure mapping を固定する
- `tests/unit/test_research_session_contract.py`
  - TomoroSession が research command/result を final owner として扱うことを固定する

### 今回触らないもの

- Chrome / Perplexity DOM automation の実装
- Tomoko DB schema / migration
- `ContextSnapshotBuilder`
- `MemoryGate`
- LLM tool calling
- user の「教えて」follow-up から result 本文を読む処理
- conversation prompt への research result 直入れ

### 完了条件

- [x] rule-based detector が「調べて」「検索して」「最新」を search request として検出する
- [x] TomoroSession が `research_requested` を command 化する
- [x] `ResearchCommandRunner` が MCP result を `research_result_ready` event として戻す
- [x] result citations は URL dedupe される
- [x] failed / timeout / needs_human は success と混ざらない
- [x] focused unit / ruff が通る
- [x] full unit / global ruff が通る
...
### Phase 10.20.17: keep listening on unfinished engaged follow-up and increase MaAI backchannels

この Phase では、以前の「同一 user speech segment で MaAI 相槌は 1 回だけ」という方針を否定する。
実会話では長い user speech segment の途中で複数回の相槌が自然に必要になるため、
相槌は通常 reply ではなく gesture audio として、global cooldown と Tomoko idle gate で抑制する。

また、`...よくさぁ` / `...関係が` のような長い未完 follow-up が
`attention_engaged_followup` として通常 reply を開始する挙動を否定する。
これは VAD silence threshold だけで解決する問題ではなく、speech_end 後の transcript を
「返答開始してよい完成発話」と「聞き続ける未完発話」に分ける participation gate の問題として扱う。

#### 変更対象

- `server/edge/participation/wake_word.py`
  - 長い発話でも `さぁ` / 助詞終端などの未完 tail は `low_confidence_followup` にする
- `server/session.py`
  - MaAI backchannel の同一 speech segment 1 回制限を外す
  - global cooldown を短縮し、長い発話中に複数回の相槌を許す
- `tests/unit/test_participation.py`
  - 実ログ由来の `...よくさぁ` / `...関係が` が invited にならないことを固定する
- `tests/unit/test_maai_backchannel_tap.py`
  - 同一 speech segment でも cooldown 後なら複数回 release されることを固定する
...
- [x] 実ログ由来の未完 follow-up が invited になる赤テストを確認する
- [x] 同一 speech segment 2 回目の相槌が skip される赤テストを確認する
- [x] 未完 follow-up は `low_confidence_followup` として observer に落ちる
- [x] MaAI 相槌は cooldown 後なら同一 speech segment でも release される
- [x] focused unit / ruff が通る
- [x] full unit / global ruff / diff check が通る
- [x] git commit まで完了する

### Phase 10.20.18: MaAI gesture audio must not mark Tomoko speaking

この Phase では、MaAI 相槌の TTS 音声が `AudioTurnController` の通常 audio turn に入り、
`Tomoko is speaking` / `speaking` playback state を立てる挙動を否定する。

実ログでは `なるほど` の相槌直後に user transcript が finalized され、
`playback_state=speaking` として barge-in 判定に入った後、通常 `attention_engaged_followup` として
reply_start していた。
つまり `turn_id=None` の playback telemetry を無視するだけでは不十分で、
サーバー内部の speaking state からも gesture audio を分離する必要がある。

#### 変更対象

- `server/session.py`
  - MaAI backchannel release では通常 audio turn を begin しない
  - backchannel TTS chunk は `AudioTurnController.reserve_audio_chunk()` を通さず送る
  - backchannel audio send は reply output latency state を進めない
- `tests/unit/test_maai_backchannel_tap.py`
  - MaAI backchannel release 後に `audio_turns.is_tomoko_speaking()` が False のままであることを固定する
  - backchannel release では `audio_start` / `audio_end` を出さないことを固定する

#### 今回触らないもの

- 通常 reply / stop ack / pregenerated candidate の audio turn
- MaAI score / cooldown / threshold
- VAD silence threshold
- STT / ParticipationJudge

#### 完了条件

- [x] backchannel release 後に Tomoko speaking が立つ赤テストを確認する
- [x] MaAI gesture audio が通常 audio turn / reply output state を進めない
- [x] focused unit / ruff が通る
- [x] full unit / global ruff が通る
- [x] git commit まで完了する
...
### Phase 10.20.20: accept low-audio clock queries and cancel stale unstarted replies

この Phase では、短い時計質問 `今何時` が STT では認識されているのに
`TranscriptFilter` の `low_audio_short_text` で drop される挙動を否定する。
時刻質問は Tomoko が system prompt の current local time で即答できるため、低音量でも明示的に accept する。

また、VAD が speech_end を出した直後に user が話し続けて `listening` へ戻った場合でも、
すでに起動した未出力 reply が cancel されずに話し始める挙動を否定する。
新しい user speech が始まった時点で、まだ reply output が始まっていない reply は stale として cancel する。

#### 変更対象

- `server/edge/pipeline/stt_filter.py`
  - `今何時` / `いま何時` / `何時` / `時刻` などの短い clock query は low-audio short filter の例外にする
- `server/session.py`
  - `process_audio_chunk()` で VAD が `listening` へ遷移した時、未出力 reply を cancel する
- `tests/unit/test_stt_filter.py`
  - 低音量 `今何時` が drop されないことを固定する
- `tests/unit/test_streaming_tts_pipeline.py`
  - resumed user speech が unstarted reply を cancel することを固定する

#### 今回触らないもの

- VAD silence threshold
- STT backend
- MaAI gesture audio lane
- 通常 reply LLM / TTS の生成経路

#### 完了条件

- [x] 低音量 `今何時` が drop される赤テストを確認する
- [x] resumed listening でも未出力 reply が cancel されない赤テストを確認する
- [x] clock query は低音量短文でも accept される
- [x] resumed user speech で未出力 reply が stale cancel される
- [x] focused unit / ruff が通る
- [x] full unit / global ruff が通る

### Phase 10.20.21: explicit output lane and floor ownership characterization

この Phase では、TomoroSession の出力を「人間 transcript に対する通常 reply」と暗黙に同一視する方針を否定する。
MaAI 相槌や将来の Tomoko 割り込み発話では、音声を鳴らすことと会話 turn を開始することは同じではない。

まず `OutputLane = reply_turn | initiative_turn | gesture_audio | stop_ack | interrupting_turn` を明示し、
現状の reply task / AudioTurnController / candidate reply gate / conversation log 書き込みが
どの lane を前提にしているかを characterization test で固定する。
closed-loop の観点では、各 lane が「入力を受ける / 床を取る / 出力する / 出力結果が次の入力判定へ戻る」
のどこへ接続されるかを明示し、gesture audio が通常 turn の echo / barge-in / log 経路へ混ざらないことを固定する。
その上で、実装は既存挙動を大きく変えず、lane と floor ownership の意味がコード上で読める最小変更に留める。

#### 変更対象

- `server/shared/models.py`
  - `OutputLane` を追加する
- `server/gateway/audio_turn.py`
  - `AudioTurnController` が扱う lane を turn audio に限定する
  - `gesture_audio` は明示的に対象外にする
- `server/session.py`
  - `candidate_reply_gate` を `can_start_output(lane, floor_policy)` に近い意味の helper へ寄せる
  - reply / initiative / arrival / stop ack の lane を明示する
  - conversation log に保存する lane / 保存しない lane の境界を helper で明示する
- `server/gateway/gesture_audio.py`
  - MaAI 相槌は `gesture_audio` lane として、TomoroSession mutation / audio turn / conversation log の外に置く
- `tests/unit/`
  - 現状の各 lane の期待動作を characterization test として固定する

#### 今回触らないもの

- `server/session.py` の package split
- event sourcing / 外部 event bus / Redis queue
- MaAI score / threshold / cooldown
- VAD silence threshold
- ThinkingMode / TTS backend の生成品質
- conversation_logs schema migration
- Tomoko の実割り込み発話そのものの実装

#### 完了条件

- [x] `OutputLane` が追加され、全 lane 名が test で固定される
- [x] `AudioTurnController` は `reply_turn` / `initiative_turn` / `stop_ack` / `interrupting_turn` の turn audio だけを受ける
- [x] `gesture_audio` は `AudioTurnController` で拒否され、`GestureAudioEmitter` 側で lane として記録される
- [x] candidate / arrival gate が `initiative_turn` lane の floor policy として読める
- [x] conversation log 保存対象 lane と非保存 lane が helper / test で明示される
- [x] closed-loop 上で通常 turn に戻る lane と gesture として外に留まる lane が test で固定される
- [x] focused unit / ruff が通る
- [x] full unit / global ruff が通る
- [ ] git commit まで完了する
...
## 2026-05-31 Research result summary deep context

Research result の raw answer をそのまま deep context に混ぜる方針は否定する。
取り込み時に LLM summary を作り、その summary を embedding して保存する。
`ContextSnapshotBuilder(depth="deep")` は research result store を optional source として検索し、
prompt へは raw answer ではなく summary だけを `RESEARCH CONTEXT` として渡す。

### 完了条件

- [x] `ResearchCommandRunner` が MCP result 取り込み時に LLM summary を作る
- [x] summary text を embedding して research result store に保存する
- [x] `deep` / `reflective` policy だけが research result source を読む
- [x] prompt へ混ぜる文字列は research summary であり raw answer ではない
- [x] fake MCP / fake LLM summary / fake embedding store の smoke で取り込みから deep fetch まで確認できる
- [x] focused unit / integration smoke / real operator smoke / full unit / ruff が通る

## 2026-05-31 Research result PostgreSQL persistence

Research result store を InMemory 前提にする方針は否定する。
外部調査結果は Tomoko 側の責務として PostgreSQL `research_results` table に保存し、
`summary_text` と `summary_embedding` を deep / reflective context の検索索引として扱う。

raw answer は prompt へ直接混ぜず、保存済み summary だけを ContextSnapshotBuilder 経由で取得する。
DB は source of truth、process-local store は test / smoke 用の fake として残す。

### 完了条件

- [x] `research_results` migration が追加される
- [x] PostgreSQL backed store が insert / upsert / vector search を提供する
- [x] `ResearchCommandRunner` が embedding model 名も保存できる
...
## 2026-06-07 Time-last prompt ordering and topic-return check

`CURRENT LOCAL TIME` を turn context の先頭に置く方針は否定する。
現在時刻は相対表現を解釈するための補助情報であり、通常の会話意味論より先に毎 turn 読ませる必要は薄い。
live prompt の揺れを減らすため、current user utterance を先に置き、時刻は turn context の最後へ送る。

ただし、時刻質問そのものに答えられなくなる変更は受け入れない。
3 turn 目に時刻を聞き、4 turn 目に元の話題へ戻る scenario で、
時刻意味論と会話復帰の両方を見る。

### 完了条件

- [x] 現状 prompt order で4 turn time-return `/ws` smoke を測る
- [x] current user utterance を最後の user message の先頭に置く
- [x] `CURRENT LOCAL TIME` を turn context の最後へ送る
- [x] `さっきの...話に戻って` cue にだけ dynamic `TOPIC RETURN HINT` を入れる
- [x] 時刻質問に正しい現在時刻で答えられることを実測する
- [x] 4 turn 目で元の速度話題へ戻れることを実測する
...
## 2026-06-07 PostgreSQL connection pool for context hot path

same-session context read が遅い原因を SQL / index とみなす方針は否定する。
`conversation_logs_session_recorded_at_idx` は効いており、query 自体は sub-ms で返る。
毎 turn の PostgreSQL connection 作成をなくし、既存ライブラリ `psycopg_pool` の pool を使う。

### 完了条件

- [x] `psycopg-pool` を dependency に追加する
- [x] DSN ごとの `AsyncConnectionPool` helper を追加する
- [x] conversation / ambient / session stores の connection 作成を pool 経由へ置き換える
- [x] pool reuse / close の unit test を追加する
- [x] same-session writer read が warm 1ms 台へ落ちることを実DB microbench で確認する
- [x] fast context budget を 20ms に戻して perf test を通す
- [x] 実 config / 実 PostgreSQL / 実 dflash 26B runtime simulation で recent turns と prefix cache hit が維持されることを確認する
- [x] `_docs/latency.md` / `MEMORY.md` / `ARCHITECTURE.md` / `LOG.md` に実測と判断を追記する
- [x] focused unit / perf / ruff / full unit が通る

## 2026-06-07 Session scoped calendar/task context cache

`calendar_events` / `task_ledger` を通常 TTL だけで毎 turn 扱う方針は否定する。
active conversation session 中は同じ session_id の後続 turn で再利用し、
session_id が変わったら cache miss にする。

### 完了条件

- [x] `calendar_events` / `task_ledger` を active session 中だけ session scoped cache にする
- [x] session scoped cache key に `active_session_id` を含める
- [x] `active_session_id=None` では従来 TTL cache を使う
- [x] session_id が変わったら cache miss になる unit test を追加する
- [x] task ledger write 成功時に `task_ledger` session cache を invalidate する
- [x] `same_session_turns` は cache 無効のまま維持する
