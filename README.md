...
| STT | `local_apple_speech_ja` | Apple Speech。Mac 実機では有力な primary lane |
| STT fallback / 比較候補 | `local_whisper_mlx_large_turbo_q4` | 実会話品質が良かった MLX Whisper lane |
| STT fallback / 比較候補 | `local_whisperkit_serve_large_turbo_632m_cpu_ne` | WhisperKit turbo 632MB + CPU/ANE lane |
...
STT は現行 default で Apple Speech を使います。これは汎用 OSS 構成としては Whisper 系より
Mac 専用ですが、Apple Silicon Mac で動かす Tomoko では、OS 側の Speech framework に STT を逃がして
GPU / MLX 側を会話 LLM や TTS に残せる実用上の利点があります。
再現性や fallback のために `local_whisper_mlx_large_turbo_q4` と WhisperKit lane も残しています。
`make prepare` は Apple Speech sidecar の build と、VOICEVOX Engine の起動確認を現行 config に沿って行います。
...
- LM Studio
- VOICEVOX Engine
- macOS Speech Recognition permission（Apple Speech STT を使う場合）
