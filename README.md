...
の PR1823 既定ポート `http://127.0.0.1:50122` を使います。
...
make information-collect-world
...
Codex / Computer Use の手動操作に頼らず、隣 repo の `tomoko-research-operator` から
ログイン済み Chrome / Perplexity を CDP 経由で動かして収集できます。
...
`information-collect-world` は `tomoko-research-mcp` の `world.observe` tool を subprocess 起動し、
`informations/work/YYYY-MM-DD-world-observation.md` を作ります。
operator は provider text と raw artifact を返すだけで、Tomoko 側が frontmatter を付け直して
既存 validator / normalizer / DB ingest に渡します。
