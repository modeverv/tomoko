...
あなたは外部観測レポートを作る係です。
以下の条件で、日本語 Markdown の本文を 6000〜8000 字程度で作成してください。

成果物:
- title: `world_observation_2026-05-25`
- output format: Markdown
- code fence で囲まない
- 会話文の説明ではなく、保存可能な Markdown 本文だけをチャット回答欄に直接出力する
- Perplexity のページ / document / file / canvas / 共有成果物は作成しない
- 「作成しました」「共有しました」「概要は以下です」のような説明で終えず、本文を全文そのまま出力する
...
出力後の利用:
- Tomoko operator はチャット回答本文を CDP 経由で取得して保存する
- Perplexity の document download 前提にはしない
- frontmatter delimiter が画面表示で崩れても、Tomoko 側で validated frontmatter を付け直す
