...
TOMOKO_V2_DISTILLED_SATURATION_MODEL ?= make-model/artifacts/public-synthetic-gemma26b-200-plus-anchors-life-h8192-l001-saturation-model.json
DFLASH_HOST ?= 0.0.0.0
DFLASH_CONNECT_HOST ?= 127.0.0.1
TOMOKO_V2_LLM_READY_URLS ?= http://$(DFLASH_CONNECT_HOST):$(DFLASH_31B_PORT)/v1/models http://$(DFLASH_CONNECT_HOST):$(DFLASH_26B_PORT)/v1/models
TOMOKO_V2_LLM_URL ?= http://$(DFLASH_CONNECT_HOST):$(DFLASH_26B_PORT)
TOMOKO_V2_LLM_MODEL ?= gemma-4-26b-a4b-it-mlx
VOICEVOX_HOST ?= 0.0.0.0
VOICEVOX_CONNECT_HOST ?= 127.0.0.1
VOICEVOX_PORT ?= 50122
TOMOKO_V2_VOICEVOX_READY_URL ?= http://$(VOICEVOX_CONNECT_HOST):$(VOICEVOX_PORT)/version
TOMOKO_V2_VOICEVOX_URL ?= http://$(VOICEVOX_CONNECT_HOST):$(VOICEVOX_PORT)
TOMOKO_V2_VOICEVOX_SPEED ?= 1.5
TOMOKO_V2_MAAI_BACKCHANNEL ?= 1
...
TOMOKO_INTERNAL_WS_HOST ?= $(TOMOKO_INTERNAL_WS_CONNECT_HOST)
...
llm-run:
	DFLASH_HOST="$(DFLASH_HOST)" DFLASH_31B_MODEL="$(DFLASH_31B_MODEL)" DFLASH_31B_DRAFT="$(DFLASH_31B_DRAFT)" DFLASH_31B_PORT="$(DFLASH_31B_PORT)" DFLASH_26B_MODEL="$(DFLASH_26B_MODEL)" DFLASH_26B_DRAFT="$(DFLASH_26B_DRAFT)" DFLASH_26B_PORT="$(DFLASH_26B_PORT)" bash scripts/run_llm.sh

llm-stop:
	bash scripts/run_llm_stop.sh

voicevox-run:
	VOICEVOX_COMMAND="$(VOICEVOX_COMMAND)" HOST="$(VOICEVOX_HOST)" PORT="$(VOICEVOX_PORT)" bash scripts/run_voicevox.sh
...
v2-db-split-smoke:
	$(PYTHON) -m scripts.v2_db_split_smoke --fake-runtime --start-db --start-processes
