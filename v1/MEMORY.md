...
### 確定した判断: VAD idle pre-roll は wake word 冒頭欠落対策として保持する
2026-06-12 に、「智子聞こえる？」が「聞こえる」とだけ STT される原因を、
Silero VAD の発話検知遅延で発話冒頭が segment から欠落している問題として扱う判断を確定した。

`VADProcessor` が idle 中の音声を捨てる方針は否定する。
idle 中も直近 `vad_pre_roll_ms`（既定 500ms）の float32 chunk を pre-roll buffer に保持し、
`idle -> listening` 遷移時に segment 先頭へ連結する。

pre-roll は VAD hot path の内部 primitive buffer とし、SpeechSegment DTO は発話終了時だけ生成する。
上限を超えたサンプルは古い方から切り詰め、segment 確定後と `reset()` では pre-roll をクリアする。
設定値は `AudioSection.vad_pre_roll_ms` として `config/central_realtime.toml` / `config/edge_kitchen.toml` から渡す。

### 確定した判断: VAPハイブリッド制御（Phase TT-v2.5）パラメータ delta_silence_ms の外だし
2026-06-11 時点では、VAP の確度（ターン譲渡確率）に応じて VAD 無音判定時間を動的に伸縮させる制御において、ユーザーからの指示に基づきパラメータ「150, 650, 90%」を直接設定ファイルに外だしする設計へ移行した。
- `vap_hybrid_min_silence_ms` (150)、`vap_hybrid_delta_silence_ms` (650)、`vap_hybrid_threshold_probability` (0.90) を設定ファイルに定義。
- 従来の `vap_hybrid_max_silence_ms` (800) を指定している既存の動作環境もサポートするため、config パース時に相互変換しフォールバックする互換ロジックを実装。

### 確定した判断: LoRA（mlx-lm）によるシステムプロンプトの焼き込みプログラム一式の導入
2026-06-11 時点では、毎回の推論に巨大なシステムプロンプト（キャラクター、ルール等）を含めることによるプレフィルレイテンシーの増大を防ぎ、かつモデルの指示追従性・一貫性を強固にするために、システムプロンプトの振る舞いをベースモデルに焼き込む（Baking）ための LoRA ファインチューニングプログラム一式（`lora/`）を導入する。
- `generate_data.py` でシステムプロンプトを適用した対話データを自動生成（Ollama/MLX-LM）し、`train.jsonl` と `valid.jsonl` に分割保存する。**基本のシステムプロンプト（`prompts/base_persona.md`）の隣にある `prompts/persona_overlay.md`（人格オーバーレイ）を自動で結合してデータ生成に反映するロジック（および明示的指定）を追加する。**
- **`mlx_lm.generate` の最新API仕様に対応するため、`temp` / `temperature` を直接引数に渡すのではなく、`from mlx_lm.sample_utils import make_sampler` を用いてサンプラーオブジェクトを構築し `sampler` 引数として渡すように変更（古いバージョン向けには直接 `temperature` を渡すフォールバックも用意）。**
- **`mlx-lm` 新仕様への追従のため、LoRA学習呼び出しコマンドを `python3 -m mlx_lm lora` に変更し、学習レイヤー指定の引数を廃止された `--lora-layers` から新仕様の `--num-layers` に修正する。**
- `train.sh` で `mlx_lm.lora` を用いて、Mac環境（Apple Silicon）で効率的にLoRA学習を実行する。
- `evaluate.py` でバッチ評価およびインタラクティブチャットモードを提供し、焼き込み結果を直接評価可能にする。
- プログラム自体の整合性を維持するため `tests/unit/test_lora_programs.py` でテストによる保護を行う。
- **InferenceRouter から MLXLMBackend への `adapter_path` 伝搬のサポート。`config.toml` の `backends` セクションで `adapter_path` を指定することで、メインの会話推論時（`mlx_lm` バックエンド）に自動的に指定された LoRA アダプタ（例: `lora/adapters`）をロードして使用する設計を採用。**
- **dflash経由でのLoRAアダプタ適用について。現在の `dflash` パッケージは、APIリクエストパーサ内に `adapters` の受け口を持つものの、内部の `DFlashModelProvider` や `load_runtime_bundle` のロード処理で `adapter_path` を無視する仕様となっており、直接 LoRA アダプタを適用して起動・推論することはできない。dflashで利用する場合は `mlx_lm fuse` コマンドを用いてベースモデルと LoRA アダプタを結合したマージ済みモデル（fused model）を生成し、それを dflash の `--model` 引数に指定して起動する回避策を採用。**

### 確定した判断: 最新の動的コンテキストを圧縮表現に変更
2026-06-11 時点では、最新ターンに含める動的コンテキスト（タスク、長期メモリ、短期メモリ、検索結果）の記述方式が、キー・バリュー区切りの冗長なメタデータや redundant な指示文を含み、トークンを過剰に消費していた状態を否定する。
各コンテキストについて以下の圧縮表現を採用する。
- タスク: `## TASKS`（`-[id] 優先: title (期限) : details`）
- 長期メモリ: `## MEMORIES`（`-[MM/DD HH:MM] speaker: text`）
- 短期メモリ: 指示英文をカットした `## SHORT MEMORY`（`- 暗記: text`）
- 検索結果: `## RESEARCH`（`-[MM/DD HH:MM] クエリ -> summary (ソース)`）
これによりプロンプトサイズはさらに約 500〜600 トークンほど削減され、LLM プレフィルの負荷とコストを劇的に下げることができる。

### 確定した判断: プロンプト肥大化防止のため過去の履歴は raw text 優先に戻す
2026-06-11 時点では、dflash prefix cache のために `llm_prompt_content` を過去履歴に使用する設計（2026-06-07採用）を否定する。
このアプローチでは、会話ターンが進むにつれてカレンダー（最大64件）やタスクなどの巨大なコンテキスト情報が重複して各履歴メッセージに蓄積され、プロンプトが自己増殖的に肥大化するためである。
過去の user メッセージは `turn.text` （生発話）として履歴に渡す設計に戻す。これによりプロンプトサイズは常に最小限に保たれ、無駄な繰り返しは完全に排除される。

### 確定した判断: PR1823 streaming_synthesis は audio/wav stream として扱う
2026-06-06 時点では、`voicevox_tsumugi_chunked` を旧 multipart `/streaming_synthesis`
contract だけに固定し続ける方針を否定する。
隣 repo `async-voicevox` の PR1823 launcher は `http://127.0.0.1:50122` で起動し、
`/streaming_synthesis` は single `audio/wav` response として WAV header 後の 24kHz mono PCM を逐次流す。

Tomoko の client 再生境界は WebSocket binary message ごとの `decodeAudioData()` のまま維持する。
そのため raw PCM は client に流さず、`VoicevoxChunkedBackend` が WAV header を読んで
受信 PCM 区間を complete WAV chunk に包み直し、
`AudioChunkOut(data, sequence, is_last)` として yield する。
PR1823 は 24kHz 以外の `outputSamplingRate` を 422 で拒否するため、
`voicevox_tsumugi_chunked` の `sample_rate` は 24kHz にする。
legacy multipart complete-WAV parts は比較・互換用に同じ backend で引き続き受けられる。

### 確定した判断: world observation 収集は research-operator の Chrome CDP tool に寄せる
2026-06-05 時点では、world observation の raw Markdown 収集を Codex / Computer Use の手動操作へ
依存させ続ける方針を否定する。
隣 repo `tomoko-research-operator` が `tomoko-research-mcp` で公開する `world.observe` tool を使い、
ログイン済み Chrome / Perplexity の CDP 操作は operator 側に隔離する。

Tomoko 側は `WorldObservationMcpClient` で subprocess JSON-RPC を投げ、
`informations/prompts/daily_world_observation.md` を指定日付 / observed_at に差し替え、
返ってきた provider text に deterministic frontmatter を付け直して
`informations/work/YYYY-MM-DD-world-observation.md` に保存する。

この収集は `/ws` / `TomoroSession` / 会話 hot path へ入れない。
保存後は既存の strict validator、`make information-ingest-dry-run`、
`make information-ingest-once`、`make information-interpret-once` の順で DB 側へ入れる。
operator は Tomoko DB へ書かず、Tomoko 側が validated artifact と ingest を所有する。

### 確定した判断: VOICEVOX default は streaming_synthesis chunked backend にする
2026-06-05 時点では、central / edge の default TTS を通常 `/synthesis` の
`voicevox_tsumugi` にして 1 WAV だけ返す方針を否定する。

`~/by-llms/async-voicevox` で用意した chunked VOICEVOX contract に合わせ、
Tomoko では `voicevox_tsumugi_chunked` を default にする。
backend は VOICEVOX Engine の `/streaming_synthesis` を
`chunk_min_accent_phrases=1` と `segment_length=0.6` で呼ぶ。
PR1823 では `audio/wav` stream を受けて complete WAV chunk に包み直し、
legacy multipart response では part ごとの complete WAV をそのまま
`AudioChunkOut(data, sequence, is_last)` として yield する。
central default は PR1823 launcher の既定に合わせ、`http://127.0.0.1:50122` を向く。
通常/stream 比較 backend の 16kHz 出力切り分け設定は残すが、PR1823 chunked は engine の 24kHz-only contract に合わせる。

client は従来どおり WebSocket binary message を `decodeAudioData()` するだけにし、
部分 PCM byte stream の解釈や playback 状態判断を持たせない。
`voicevox_tsumugi` / `voicevox_tsumugi_stream` は比較・fallback 用 backend として残す。

### 確定した判断: timer / alarm は DB row と TomoroSession gate で鳴らす
2026-06-02 時点では、timer / alarm を client-local `setTimeout` や client asset 再生へ寄せる方針を否定する。
単発 timer / alarm は `timer_alarm_entries` の scheduled row として PostgreSQL に保存し、
worker が due row を claim して `timer_due` / `alarm_due` 相当の `SessionEvent` として
`TomoroSession` に戻す。

TomoroSession は connected output / floor ownership / playback state を見て通知発話の最終 gate を持つ。
due 通知は既存の precomputed reply / TTS / audio chunk 経路へ流し、
...
この方針を否定し、重要なタスク管理は `task_ledger_entries` の structured row として DB 永続化する。

ContextSnapshotBuilder は `task_ledger` source を読み、active task を
`TomokoContextSnapshot.task_ledger_entries` として返す。
初段では fast / normal でも active task 先頭10件を低コストで復帰し、
deep / reflective ではより多い active task を読める policy 境界を持たせる。
short / deep の最終バランスは、実測 latency と体感を見て後続 Phase で調整する。

今回の Phase は task ledger の保存先・読み取り・prompt 復帰を作るだけに留める。
音声 transcript から task の追加 / 完了 / 取消を deterministic reducer で反映する処理は別 Phase とする。

### 確定した判断: persona_overlay.md の本文は unit test で固定しない
2026-06-01 時点では、`tests/unit/test_phase4_thinking.py` が `persona_overlay.md` に
`原作台詞` という具体語句が含まれることを期待していた。
persona overlay は体感調整のために文面が頻繁に変わるため、
具体語句を固定する test は壊れやすく、変更のたびに不要な friction になる。

この方針を否定し、repo-local `persona_overlay.md` の test は
ファイルが存在する場合に UTF-8 で読めることだけを確認する。
`ThinkFastMode` が sibling `persona_overlay.md` を system prompt に差し込む挙動は、
tmp fixture の overlay injection test で引き続き保証する。

### 確定した判断: Calendar context は時計質問へ出さず、現在 turn では二重投入しない
2026-06-01 の live log では、`今何時` が calendar request として扱われ、
未来30日の予定 42 件が prompt に入っていた。
さらに同じ予定が `CALENDAR CONTEXT` と `長期コンテキスト` の両方に入り、
LLM が時刻質問に予定表を混ぜたり、`雑談` を反復する壊れた応答を出していた。

この状態を否定する。
`今何時` / `何時くらい` / `現在時刻` のような純粋な時計質問は calendar cue ではない。
予定質問で deep calendar context を読んだ現在 turn では、予定は `CALENDAR CONTEXT` にだけ載せる。
同一会話内 follow-up 用の carryover は維持し、次 turn が fast context になった時だけ
`長期コンテキスト` として短命に使う。
TTS / VOICEVOX 自体はログ上では音声 chunk を返しており、声が聞こえない主因は
壊れた長文/反復応答による TTS 巨大化と audio queue overflow と判断する。

### 確定した判断: Calendar context は今日から未来30日を prompt に載せる
2026-06-01 時点では、`ContextSnapshotBuilder(depth="deep")` の calendar context は
過去1日 + 14日先、最大8件だけを読む実装だった。
これだと 2026年6月のように予定が 42 件入っている月では、月内の予定が prompt へ十分に載らない。

この方針を否定し、会話中に予定を聞かれた時の deep calendar context は
今日から未来30日、最大64件を読む。
calendar context は引き続き PostgreSQL `calendar_events` 由来の参照情報であり、
ユーザー発話や会話原本として扱わない。
`ContextSnapshotBuilder` には test 用の `now_provider` を持たせ、
calendar window の unit test が実日付に依存しないようにする。

### 確定した判断: Research follow-up は operator の `full_text` を優先して発話する
2026-06-01 時点では、tomoko-research-operator が Perplexity raw response を増やして
`full_text` として返していても、Tomoko 側は MCP structuredContent から `short_answer` だけを読み、
`結果を教えて` follow-up でも短い冒頭だけを発話していた。

この状態を否定し、Tomoko 側の `ResearchResult` も `full_text` を保持する。
`research_answer_requested` / `start_research_answer_reply` の発話本文は `full_text` を優先し、
`full_text` がない場合だけ `short_answer` と `bullets` から発話本文を組み立てる。

`short_answer` は引き続き speakable 判定、result-ready emission、DB metadata 用の短い答えとして残す。
deep context への保存は従来どおり raw 本文ではなく LLM summary を使う。
実 operator smoke では `short_answer` 45 文字に対して `answer_reply_text` 549 文字となり、
Perplexity 側の増量出力が Tomoko の follow-up 発話まで届くことを確認した。

### 確定した判断: Browser audio device selection は client-only UI に閉じる
2026-06-01 に UI へ input device / output device selector を追加した。
これはブラウザの `navigator.mediaDevices.enumerateDevices()`、`getUserMedia({ deviceId })`、
`HTMLMediaElement.setSinkId()` の範囲に閉じる。

Tomoko の `/ws` payload、`TomoroSession`、VAD / STT / TTS hot path、playback telemetry contract、
DB write ordering は変更しない。
output device は Web Audio の playback mixer を `MediaStreamDestination` と hidden audio element へ流し、
`setSinkId` 対応ブラウザだけ選択先へ出す。
非対応ブラウザでは select を disabled にし、default output のまま動かす。

### 気づき: ChatGPT 生成 PNG は透明背景に見えても RGB 市松背景の場合がある
2026-06-01 に `assets/tmp/` 配下のキャラクター素材を確認したところ、
`base_body.png` / `face_*.png` / `mouce_closed.png` / `mouse_open.png` は PNG だが alpha channel を持たず、
透明背景に見える市松模様が RGB として焼き込まれていた。

単純な `<img>` 重ねでは差分 layer の背景が下 layer を覆うため、
素材確認用の `assets/tmp/preview.html` では Canvas で「画像端からつながる明るい無彩色背景」を flood fill で抜き、
その後に face / mouth layer の位置・scale を調整できる形にした。

今後本番 UI 用に画像生成する場合は、透明 PNG を明示的に要求するだけでなく、
alpha channel が本当に入っているか `file` / `identify` / Canvas 上の alpha sample で確認する。

### 確定した判断: Research MCP は常駐 make process ではなく subprocess 起動を観測する
2026-05-31 時点では、Tomoko の Research MCP integration は別 terminal で `make` などを起動しておく
常駐 server 前提ではない。
central runtime は `TOMOKO_RESEARCH_MCP_COMMAND` があればそれを使い、未指定なら
隣 repo の `tomoko-research-operator` に対して
`uv --directory ... run tomoko-research-mcp` を research request ごとに subprocess 起動する。

Tomoko の一次応答は通常 reply pipeline の待機応答であり、MCP subprocess の成功とは独立している。
そのため live 調査不調の診断では、`research_request_accepted` や待機発話だけで成功扱いにせず、
`TomoroSession` の research transition handler schedule / finish、
`ResearchCommandRunner` の request start / finish / ingestion、
`ResearchMcpClient` の subprocess start / timeout / failure / completion を server log で追う。
...
### [2026-06-11] プロンプト圧縮適用後の WebSocket レイテンシー実測
メッセージ履歴の生発話化（turn.text）と最新コンテキストの圧縮フォーマット化（TASKS, MEMORIES, SHORT MEMORY, RESEARCH）を行った後の `make smoke-ws-voice-latency` 3ターン対話測定結果。

#### 測定1回目（ウォームアップなしの初回走行）
- **TURN 1**: `voice_end_to_first_reply_text: 4812.7ms`, `transcript_to_first_reply_text: 3545.3ms` (LLM推論時間: 3544.7ms)
- **TURN 2**: `voice_end_to_first_reply_text: 3732.8ms`, `transcript_to_first_reply_text: 2610.4ms` (LLM推論時間: 2609.9ms)
- **TURN 3**: `voice_end_to_first_reply_text: 3599.1ms`, `transcript_to_first_reply_text: 2449.6ms` (LLM推論時間: 2449.0ms)

#### 測定2回目（1回走行させてモデルと推論キャッシュをウォームアップさせた後の走行）
- **TURN 1**: `voice_end_to_first_reply_text: 3728.7ms`, `transcript_to_first_reply_text: 2588.2ms` (LLM推論時間: 2588.2ms)
- **TURN 2**: `voice_end_to_first_reply_text: 3560.1ms`, `transcript_to_first_reply_text: 2447.9ms` (LLM推論時間: 2447.9ms)
- **TURN 3**: `voice_end_to_first_reply_text: 3812.4ms`, `transcript_to_first_reply_text: 2658.5ms` (LLM推論時間: 2658.5ms)

#### 評価
ウォームアップ完了後の2回目の結果を見ると、特に **TURN 1** において、`transcript_to_first_reply_text` が **2588.2ms**（圧縮前 3143.1ms から **554.9ms短縮**。未ウォームアップ状態の 3545.3ms からは **957.1ms短縮**）、`voice_end_to_first_reply_text` が **3728.7ms** と劇的に高速化していることが確認された。
プロンプト全体を約90%（約2600文字）に圧縮した効果は、モデルと推論キャッシュが完全に温まった状態で極めて顕著に表れる。ローカル推論環境（LM Studio経由）での初期ロード/デコードオーバーヘッドをウォームアップによって前払いしておくことで、劇的なレイテンシーの短縮が達成された。
また、プロンプト肥大化による破綻リスクの回避、コンテキストの重複削減によるLLMの応答品質向上といった設計上の恩恵も完全に享受できているため、この健全なプロンプト設計は維持すべきである。
...
### 確定した判断: Research 開始通知は通常 reply pipeline の LLM 応答にする
2026-05-31 時点では、research request を受けた時に無言で background 調査だけを走らせる方針を否定する。
また、固定文の precomputed reply で済ませる方針も採らない。

Tomoko は `submit_research_request` を background drain に渡した後、
通常の conversation reply pipeline を使って短い待機応答を生成する。
この時 `ThinkingInput.response_directive` を通じて `ThinkFastMode` の system prompt に
...
### 確定した判断: Research MCP は operator repo を cwd にして起動する
2026-06-01 の live `logs/server-debug.log` では、Research request detection と
`Research MCP subprocess starting` までは動いていたが、
command が `/Users/seijiro/Sync/sync_work/by-llms/tomoko/tomoko-research-operator` を指し、
`No such file or directory` で即失敗していた。

原因は default operator path を `WORK_DIR.parent / "tomoko-research-operator"` で作っていたこと。
`WORK_DIR` は repo root 配下の `work/` なので、`WORK_DIR.parent` は Tomoko repo root になる。

実運用では sibling repo `/Users/seijiro/Sync/sync_work/by-llms/tomoko-research-operator` を
working directory にして `uv run tomoko-research-mcp` を起動する。
これにより operator 側の相対 artifact / 設定 / uv project 解決も operator repo 基準で安定する。

### 確定した判断: Research result ready は短い完了通知を発話する
2026-06-01 の live `logs/server-debug.log` では、MCP subprocess は
`status=completed speakable=True` で完走し、result ingest も成功していた。
しかしその後に `reply_text` / `tts_start` が出ておらず、
`research_result_ready.notice_text` は UI emission に留まっていた。

この状態を否定し、speakable な `research_result_ready` は
`調べ終わったよ。聞く？` を `start_research_notice_reply` command として返す。
background handler 経路では `ResearchCommandRunner` がこの command を
`TomoroSession.start_precomputed_reply(reason="research_result_notice")` に通す。

調査結果本文そのものは従来どおり pending result として保持し、
ユーザーの「教えて」「聞かせて」などの follow-up で
`research_answer_requested` / `start_research_answer_reply` として発話する。

### 確定した判断: Research failed result も短い失敗通知を発話する
2026-06-01 の live `日本の首相について調べて` では、MCP subprocess は起動したが
`status=failed speakable=False` で戻った。
同じ operator cwd から手動で同じ query を投げると `status=completed` だったため、
path / cwd の恒久問題ではなく、その回の operator result が failed だった。

この状態で無音にする方針を否定し、`speakable=False` の
`research_result_ready` でも `調べきれなかったみたい。` を
`start_research_notice_reply` として発話する。
また `ResearchMcpClient` / `ResearchCommandRunner` の完了ログには
`error_reason` を含め、次回 failed になった時に operator 側の理由を追えるようにする。

### 確定した判断: topic 付き Research follow-up は pending query と照合する
2026-06-01 の live log では、`調べ終わったよ。聞く？` 後の
`ともこ教えて` は partial では見えていたが final transcript が `ともこ` になり、
STT 側で `教えて` が落ちていた。
一方で `主称について教えて` / `手書について教えて` /
`ともこ 日本の首相について教えて` は final transcript として通っていた。

従来の `is_research_answer_request()` は `教えて` が含まれると
pending query との overlap を見ずに pending result を返していた。
このため `日本の首相について教えて` に対して、直前 pending の
`手書` result を誤って返していた。

この状態を否定し、topic 付きの `教えて` / `聞かせて` / `結果` 系 follow-up は
pending query と topic overlap する時だけ `research_answer_requested` にする。
単独の `教えて` / `ともこ教えて` のように topic がない follow-up は、
pending result があれば引き続き直前 result を読む。

### 確定した判断: Research follow-up の合言葉は `結果を教えて` に寄せる
2026-06-01 の live 音声では、ユーザーは `教えて` と発音しているつもりでも
Whisper final transcript が `そして` や `ともこ` になり、follow-up 判定まで届かないことがあった。

このため result-ready notice は `聞く？` ではなく
`調べ終わったよ。結果を教えてって言ってね。` と発話し、
ユーザー側の follow-up 合言葉を `結果を教えて` に寄せる。

`結果を教えて` は topic がない direct answer cue として扱い、
pending research result があれば `research_answer_requested` に落として
`short_answer` を発話する。

### 確定した判断: `結果を教えて` は低音量短文でも STT filter を通す
2026-06-01 の live log では、`結果を教えて` は Whisper final transcript として
正しく認識されていたが、`audio_level_db=-36.6/-37.1` で
`low_audio_short_text` と判定され drop されていた。

このため `結果を教えて` は明示 command phrase として扱い、
時計 query と同様に低音量短文フィルタの例外として accept する。

### 確定した判断: central realtime STT は Apple Speech に戻す
2026-06-01 時点では、`local_whisper_mlx_large_turbo_q4` を active STT にする判断を否定し、
central realtime の active `stt_backend` を `local_apple_speech_ja` に戻す。

MLX Whisper large turbo q4 は引き続き精度比較候補として残すが、
現時点の live runtime では macOS Speech framework の on-device lane を active にする。

### 確定した判断: engaged から cooldown までは 20 秒待つ
2026-06-01 の live log では、Tomoko の playback end から約 9.5 秒で
`attention changed from engaged to cooldown` が出ていた。
これは従来の `engaged_timeout_ms=8000` による仕様どおりの挙動だったが、
会話の余韻や follow-up 待ちとしては短すぎる。

そのため、既定の engaged timeout は 8 秒を否定し、2.5 倍の 20 秒にする。
cooldown から ambient へ戻る `cooldown_timeout_ms=8000` は今回は変更しない。

### 確定した判断: engaged follow-up の低音量 blanket drop は 6 文字以下に限る
2026-06-01 の live log では、`今何時` への返答後に
`もうかなり夜遅いやん` と `もうかなり遅いやん` が STT / transcript filter を通った。
しかし `audio_level_db=-30.1/-31.9` かつ 20 文字以下だったため、
`WakeWordJudge` の `low_confidence_followup` が observer に落とし、Tomoko は応答しなかった。

この 20 文字境界は日本語の自然な短文を巻き込みすぎるため否定する。
低音量だけを理由に follow-up を落とす blanket rule は 6 文字以下に狭める。
`お疲れ様です` などの既知 hallucination phrase、短い未完 fragment、長い未完 continuation tail は
個別 rule で引き続き observer にする。

### 確定した判断: task ledger の会話操作は create / complete のみに限定する
2026-06-02 の task ledger voice Phase では、update / cancel を会話仕様から落とす。
タスク内容を変えたい場合は、既存 task を complete にして新しい task を create する運用にする。

これにより task ledger の authoritative state は active / completed の単純な遷移に保ち、
曖昧な自然言語 update を LLM に解釈させて既存 row を破壊する経路を作らない。

### 確定した判断: 曖昧な task complete は LLM 候補抽出 + deterministic validator にする
complete 発話は、まず active task title の exact / normalized partial match で決める。
それで一意に決まらない場合だけ、memory_extraction backend の structured extractor に
active task id 候補を返させる。
...
### 確定した判断: macOS daily automation は user LaunchAgent + repo-local wrapper にする
2026-06-06 時点では、`make daily` の日次実行を cron や root LaunchDaemon に寄せる方針を否定する。
`make daily` は `information-collect-world` でログイン済み Chrome / Perplexity / CDP 経路を使うため、
macOS の user login session で動く LaunchAgent として扱う。

repo 配下には `_tools/run_daily_launchagent.sh` と
`_tools/launchagents/com.tomoko.daily.plist` を置く。
人間が plist を `~/Library/LaunchAgents` にコピーして bootstrap する。
wrapper は repo root を自分の位置から解決し、`mise` を含む PATH を明示し、
`logs/daily-launchagent.log` に詳細ログを残す。
`make daily` は world observation の 600 秒 timeout を含むため、重複起動防止は
`${TMPDIR:-/tmp}/tomoko-daily.lock` の atomic directory lock で行う。

### 確定した判断: world observation provider text は rendered innerText として検証する
2026-06-05 の `make information-collect-world` live 検証では、Perplexity / Chrome CDP から取得する
provider text は Markdown source ではなく rendered `innerText` として返ることが確認された。

このため、保存前 validation は `# 外界観測レポート` や `## news` のような Markdown 記号に依存しない。
`外界観測レポート`、各 topic heading、`事実` / `推測・含意` / `source_hint` label を見る。
日本語生成では `事実：` / `source_hint：` のように全角コロンになることがあるため、
半角 `:` 固定にも依存しない。

Perplexity が document / file / canvas を作って「作成しました」「共有しました」だけを返す場合は
Tomoko 側では work Markdown に保存しない。
一方、provider が loose frontmatter や `以下が本文です。` を含めても、本文が十分であれば
Tomoko 側でそれらを剥がし、deterministic frontmatter を付け直して strict validator へ渡す。

### 確定した判断: world observation は MCP timeout と provider timeout を分けて渡す
2026-06-05 の失敗 `timed out waiting for Perplexity response to settle; chars=5` は、
Tomoko 側の subprocess timeout ではなく operator 内部の Perplexity provider timeout が 90 秒既定のままだったことが原因だった。

`make information-collect-world` は `TOMOKO_WORLD_OBSERVATION_MCP_TIMEOUT_SEC=240` に加え、
`TOMOKO_WORLD_OBSERVATION_PROVIDER_TIMEOUT_SEC=240` を operator subprocess に渡す。
outer MCP timeout と browser/provider settle timeout は別の境界なので、長文 world observation では両方を明示する。

### 確定した判断: world observation normalizer fallback は rendered text 形式も読む
2026-06-05 の `make information-ingest-once` では、収集済み Markdown は strict validation を通ったが、
LLM normalizer が長い raw body を受けて context length 超過で失敗した。

この失敗自体は許容し、LLM normalizer が落ちた場合は deterministic fallback で代表項目を保存する。
ただし CDP / Perplexity 由来の本文は Markdown source ではなく rendered text になりうるため、
fallback parser は `## news` / `### 1. title` だけに依存しない。

topic 単独行（`news` / `economy` など）と `観測1：title` / `観測項目 1：title` を heading として扱い、
`source_hint：` のような全角コロン label も source hint として抽出する。
raw Markdown 全体は DB に保存されるので、fallback items は網羅ではなく代表項目の traceable index として扱う。

### 確定した判断: world observation 収集 timeout は 600 秒にする
2026-06-05 の live collection では、Perplexity が 240 秒時点でもまだ本文生成中のことがあった。
このため `TOMOKO_WORLD_OBSERVATION_MCP_TIMEOUT_SEC=240` /
`TOMOKO_WORLD_OBSERVATION_PROVIDER_TIMEOUT_SEC=240` を既定とする直前判断を否定する。

`make information-collect-world` の既定は outer MCP timeout / provider response timeout ともに 600 秒にする。
operator 側の `world.observe` default provider timeout も 600 秒に揃える。
また、生成中の CDP snapshot では page text に `blocked` などの文字列が混ざっても
即 `needs_human` へ倒さず、`isGenerating=false` になってから human/block 判定を見る。

### 確定した判断: world observation fallback は番号付き rendered heading も読む
2026-06-05 の追加 live ingest では、Perplexity rendered text が `観測1：title` ではなく
topic 単独行の下に `1. title` / `2. title` を置く形式で返った。

このため deterministic fallback parser は `観測1：title` だけでなく、
`1. title` / `1) title` / `1：title` も observation title として扱う。
LLM normalizer が context length 超過で失敗しても、raw Markdown 全体は保存しつつ、
代表項目 index を deterministic に作れる状態を優先する。

### 確定した判断: attention timeout は engaged 120 秒 / cooldown 60 秒にする
2026-06-06 時点では、`engaged -> cooldown` 20 秒、`cooldown -> ambient` 8 秒の既定値は
live 体感として短すぎる。

この方針を否定し、既定値を `engaged_timeout_ms=120000`、
`cooldown_timeout_ms=60000` に延長する。
調査中や follow-up 待ちでも、Tomoko の発話再生が終わって idle 無音 chunk が続けば
attention idle は進むため、follow-up 余白は約 3 分へ広げる。
timeout の進行条件は従来どおり `TomoroSession.state == "idle"` かつ
`AudioTurnController.playback_state == "idle"` の無音 chunk だけとし、
再生中や listening 中には進めない。

### 確定した判断: PR1823 chunked VOICEVOX は first chunk 即時 yield と segment_length 0.2 を既定にする
2026-06-06 時点では、Tomoko backend 側で PR1823 `audio/wav` stream の complete WAV chunk を
1 個 pending し、次 chunk 到着まで初回音声を遅らせる方針を否定する。

PR1823 `audio/wav` stream では、受信 PCM が complete WAV chunk 1 個分に達した時点で
`AudioChunkOut(is_last=False)` として即 yield する。
stream 終了時に残った final chunk だけ `is_last=True` として扱う。
client は引き続き WebSocket binary message ごとの `decodeAudioData()` のみを行い、
raw PCM decode や playback 判断を持たない。

`voicevox_tsumugi_chunked` の central / edge default `segment_length` は 0.6 秒を否定し、0.2 秒にする。
PR1823 launcher 直叩き smoke では、同じ text `うん、わかった。少し待ってね。` で
0.6 秒設定が first 1512.2ms / total 2283.6ms / 5 chunks、
0.2 秒設定が first 166.0ms / total 2203.1ms / 15 chunks だった。
生成 WAV は 24kHz mono として valid だが、主観的な音質確認は別途 local listening で行う。

### 気づき: dflash Gemma 4 は reasoning_content と content を分けて返す
2026-06-07 に LM Studio remote `192.168.11.66:1234` から local dflash ports
`localhost:8081` / `localhost:8082` へ変えた状態で 31B / 26B を直接 probe した。

dflash の OpenAI-compatible SSE は早い段階で `delta.reasoning_content` を返すが、
Tomoko の現行 `LMStudioBackend.parse_sse_content()` は `delta.content` だけを本文として扱う。
そのため、体感 latency は first any token ではなく first content で見る必要がある。

短文 `了解。` probe 3 runs では、26B config name `gemma-4-26b-a4b-it-mlx` が
first any avg 362.8ms / first content avg 3645.9ms / total avg 3679.7ms、
31B config name `gemma-4-31b-it-mlx` が
first any avg 990.3ms / first content avg 8616.5ms / total avg 8661.5ms だった。

過去の LM Studio 26B は `_docs/latency.md` で first 0.31〜0.41s after load、
MEMORY の 2026-05-27 比較では 31B が first content 905〜1369ms / total 2376〜2942ms だった。
したがって現行 Tomoko parser のままでは、dflash は raw token start は速いが
Tomoko visible content は 26B / 31B とも悪化している。
31B は背景 task の過去実測 `1.8〜3.2 秒程度` や task ledger smoke 8480.82ms と比べても、
短文 first content が約 8.6 秒なので hot path へ戻す根拠にはならない。

### 確定した判断: dflash Gemma 4 は chat_template_kwargs で no-think にする
2026-06-07 の追加 probe では、dflash の OpenAI-compatible request body に
`chat_template_kwargs = {"enable_thinking": false}` を入れると `delta.reasoning_content` が止まり、
`delta.content` が即時に返ることを確認した。

`think=false` / `thinking=false` は効かなかった。
dflash CLI の `--enable-thinking` は `enable_thinking=true` を入れるための shortcut であり、
request 単位では `chat_template_kwargs.enable_thinking` が正しい。
Gemma 4 chat template は system message があると thought channel 用 prefix を入れるため、
Tomoko のように system prompt を必ず送る経路では request body で明示的に no-think を渡す必要がある。

Tomoko では `BackendSpec.chat_template_kwargs` を追加し、`LMStudioBackend` が payload に forward する。
central dflash 26B / 31B backend は `chat_template_kwargs = { enable_thinking = false }` を持つ。
実 `InferenceRouter` probe では 26B conversation が first 438.0ms / total 504.8ms、
31B memory_extraction が first 1372.2ms / total 2085.3ms で `了解。` を返した。

### 気づき: dflash server 側 no-think 起動後は reasoning_content が出ない
2026-06-07 に dflash 31B / 26B を `--chat-template-args '{"enable_thinking": false}'` 付きで
再起動した後の probe では、request body の `chat_template_kwargs` なしでも `reasoning_content` は 0 件だった。

同じ短文 `了解。` probe で、request body 側にも `chat_template_kwargs = {"enable_thinking": false}` を入れた
3 runs direct average は、26B が first content 276.6ms / total 369.1ms、
31B が first content 306.7ms / total 984.0ms だった。
Tomoko `InferenceRouter` 経由の単発 probe は、26B conversation が first 541.4ms / total 643.1ms、
31B memory_extraction が first 1064.8ms / total 1723.5ms だった。

したがって dflash の thinking 有効状態と比べると大幅に改善している。
過去の `_docs/latency.md` 上の LM Studio 26B short probe とは同等からやや速い程度だが、
31B は過去の LM Studio 31B short response より明確に速い。
ただし live 会話の E2E first audio は LLM 以外に context build / TTS が乗るため、
最終判断には `make server-debug` での実会話計測が必要。

### 気づき: dflash は長文 prompt の first content で LM Studio より強い
2026-06-07 に LM Studio `http://192.168.11.66:1234` と dflash `localhost:8082` / `localhost:8081` を
direct OpenAI-compatible streaming で比較した。
dflash request には `chat_template_kwargs = {"enable_thinking": false}` を入れ、
全 run で reasoning chunks は 0 だった。

短文 `了解。` の warm run 2-3 では、LM Studio 26B が first 72.4ms / total 73.0ms、
dflash 26B が first 223.4ms / total 350.4ms で、短文 26B は LM Studio が速い。
31B 短文では LM Studio が first 820.5ms / total 821.8ms、
dflash が first 290.5ms / total 1028.9ms で、dflash は first content は速いが total はやや遅い。

中長文 Tomoko-like prompt（system 2705 chars、messages 398 chars、9 messages）の warm run 2-3 では、
26B は LM Studio first 879.6ms / total 1839.4ms に対し、
dflash first 341.2ms / total 2903.0ms だった。
つまり 26B 長文では dflash が first content を早く返すが、短い出力全体の完了は LM Studio の方が速い場面がある。

31B 中長文では LM Studio first 4803.6ms / total 13033.4ms に対し、
dflash first 480.7ms / total 10657.3ms だった。
31B では dflash が first content も total も明確に速い。

より実 Tomoko prompt に寄せた 10k prompt（system 9966 chars、messages 1304 chars、25 messages）では、
LM Studio 26B / 31B は HTTP 200 を返したが `delta.content` を 1 件も返さなかった。
dflash は完走し、warm run 2-3 で 26B first 251.7ms / total 3541.3ms、
31B first 363.3ms / total 9402.0ms だった。
このため、長い context を持つ Tomoko 実運用では dflash の prefix cache / 長文 prefill 経路が有利と判断する。
ただし dflash は cold first run が非常に重く、サーバー起動後や新規 prefix では初回遅延を別に見る必要がある。

### 確定した判断: dflash backend は startup で Tomoko persona prefix を warm-up する
2026-06-07 の LM Studio / dflash 比較では、dflash は cold first run が重い一方、
warm run では長文 prompt の first content が大きく改善した。
このため central startup warm-up では、通常の短い conversation backend warm-up に加えて、
dflash no-think backend へ Tomoko の固定 persona / overlay prompt prefix を投げる。

対象は `BackendSpec.type == "lm_studio"` かつ
`chat_template_kwargs.enable_thinking == false` の backend に限定する。
現行 central config では 26B conversation 系と 31B memory/persona 系が対象になる。
conversation / session_summary / memory_extraction / persona_update / candidate_gen / diary の role を見て、
同じ backend は一度だけ warm-up する。

warm-up request は `ThinkFastMode.system_prompt` の固定部分と
`起動時 warm-up です。短く返事して。` の短い user message にし、`max_tokens=4` で返答生成を抑える。
...
### 確定した判断: TOPIC RETURN HINT は今回の latency probe 目的から外す
2026-06-07 に `TOPIC RETURN HINT` を撤去した。
今回の目的は「3 turn 目で時刻を聞き、4 turn 目で通常会話に戻る時に、
時刻後置の latency がどう変わるか」を見ることであり、話題復帰の意味論を上げることではない。

撤去後も current user utterance first / `CURRENT LOCAL TIME` last の prompt order は維持する。
同じ4 turn scenario の再測では、時刻質問は `今は午後3時56分だよ` と答え、
4 turn 目も特別 hint なしで `これくらい短めがちょうどいい感じ？` と概ね速度/短さの話に戻った。

平均 voice-end to first binary audio は 6413.0ms。
最初の baseline 6257.6ms からは +155.4ms、前回の time-last-only 6002.1ms からは +410.9ms。
この差は dflash の full prefill miss と生成揺れの範囲として扱い、
time-last を「明確に高速化」とは断定しない。

### 確定した判断: dflash prefix cache 用に LLM user prompt content を派生保存する
2026-06-07 の raw dflash A/B で、前 turn の user content が次 turn の履歴で
生 transcript に戻ると prefix cache が効かないことを確認した。
同じ 3066 token 規模の payload でも、履歴側が前回の wrapped user content を完全に保つと
cached tokens が 3729 まで動き、elapsed は約 1.05 秒まで落ちた。

このため `conversation_logs.transcript` を会話原本として生発話のまま残し、
LLM に実際に渡した user message content は `conversation_logs.llm_prompt_content` に
派生データとして保存する。
`ThinkFastMode` は過去 user turn の prompt content が存在する時だけそれを履歴に使い、
存在しない古い row では従来どおり transcript を使う。

この列は原本ではなく dflash prefix cache と再現性のための prompt audit である。
embedding、会話要約、ユーザーへの transcript 表示は引き続き `transcript` を使う。

### 確定した判断: active session 中は same-session context を先頭で安定させる
2026-06-07 の runtime simulation では、same-session turns に global recent turns を補完すると、
turn が進むたびに補完数が変わり、dflash が見る prompt prefix が揺れた。
この状態では `llm_prompt_content` を保存しても live prompt は full prefill miss になった。

active conversation session がある時は、直近文脈は same-session turns だけを使う。
別 session の recent turns は、active session がまだ空の時の補助に留める。
これにより turn3 以降で prefix cache hit が継続し、
runtime 6 turn simulation では turn3-6 の first text が約 2 秒前後まで落ちた。

same-session turns は会話中に毎 turn 増える source なので process-local TTL cache しない。
...
### 確定した判断: structured stable context は current utterance の前へ置ける
2026-06-07 に `ThinkFastMode` の prompt order を比較した。
候補は最後の user message 内を次の順にする構造である。

1. `SESSION STABLE STRUCTURED CONTEXT`
   - persona slice
   - calendar context
   - task context
2. `CURRENT USER UTTERANCE`
3. `TURN DYNAMIC / RECALL CONTEXT`
   - response directive
   - short memory
   - long term memory
   - research context
...
この構造は、現在発話を完全に最後へ押し込むのではなく、
session 中に安定しやすい structured context だけを current user utterance の前へ寄せる。
memory / research / current time は発話後ろに置く。

実DB / 実dflash 26B simulation では、fast depth は calendar/task が0件だったため差は小さかった。
current avg first 2249.5ms / total 4306.8ms に対し、
stable-first は avg first 2241.3ms / total 4052.8ms だった。

deep depth で calendar 41件を含めると、
task なしでは first は 6303.1ms -> 6710.4ms と悪化し、total は 9678.5ms -> 9286.5ms と改善した。
temporary task 3件を追加した deep では、first が 7619.5ms -> 7273.1ms、
total が 9880.5ms -> 9137.8ms と改善した。
一時 task は `source='codex_prompt_order_bench'` で作成し、測定後に削除済みである。

意味論は3条件とも大きな破綻は見られなかった。
ただし deep context build は 100ms budget に張り付いており、retrieval/source timeout の影響も混ざる。
したがってこの order は採用可能だが、「first text が常に速くなる」とは断定しない。
採用理由は、structured context がある条件で total latency が改善し、
current utterance への追従性も実測上は維持されたためである。

### 気づき: システムプロンプト削減（Flag ON）による人格保持のトレードオフ
2026-06-11 のセッション8において、dflashポート8082のLoRAフューズモデル（fused_model）に対して、システムプロンプトのトモコ設定（基本性格など）を削減するフラグ（Flag ON）と削減しないフラグ（Flag OFF）を用いて応答検証を行った。
- Flag OFF（長文プロンプト）：TTFTは約1.8秒〜2.1秒。応答口調は「トモコ（生意気な後輩、タメ口）」として正しく動作し、感情出力や「えー、今さら？」といったカジュアルな応答が返る。
- Flag ON（短文プロンプト）：TTFTは約0.4秒〜0.5秒。プレフィル時間の削減によりTTFTは1.4秒〜1.6秒（約75%）大幅に高速化した。しかし、応答内容は「はじめまして。私はAIアシスタントです...」と完全にデフォルトの丁寧語かつトモコの名乗らないアシスタントに戻ってしまった。

この検証から、LoRA焼き込みモデルであっても、システムプロンプト内の人物設定を完全に除去（Flag ON）すると、トリガーが外れてトモコの人格（口調）が失われてしまう重大なトレードオフがあることが確認された。
したがって、会話中の口調や感情表現を保つためには、ある程度の人格指定・トリガー指示をプロンプト内に維持する必要がある。

### 確定した判断: 100文字上限の minimal_persona.md を用いたキャラクター性のトリガー
2026-06-11 の追加検証において、100文字未満の極小の人格設定（`prompts/minimal_persona.md`）を作成し、これをプロンプトに読ませて検証した。
- 設定（89文字）: 「トモコ：生意気でフランクな後輩AI。常にユーザーにタメ口で話し、感情を先頭に [EMOTION:happy] や [EMOTION:angry] などの形式で1行出力した後に返答する。」
- 応答結果: `[EMOTION:smug]\nやっと聞いてくれた？遅すぎ。\n\n私はトモコ。あんたの作業をサポートするために作られた、ちょっと生意気な後輩AIだよ...` のように、トモコとしての口調・生意気後輩のキャラクター性・感情出力フォーマットが完全に復活・動作した。

これにより、約2,700文字のフル人格プロンプト（Flag OFF）を使用せずとも、約500文字程度（Flag ON相当 of 基礎ルール＋100文字以下の minimal_persona.md）で、LoRAによるトモコの人格を十分にトリガー可能であることが実証された。システムプロンプト肥大化によるプレフィル遅延を削減する上で、極小人格設定の付与は極めて有効なアプローチであると決定する。

### 気づき: フルプロンプト+ベースモデル vs ミニマルプロンプト+LoRAフューズモデルの比較
2026-06-11 のセッション8において、Gemma 26Bベースモデル（`mlx-community/gemma-4-26b-a4b-it-4bit`）とLoRAフューズモデル（`fused_model`）を切り替えて、プロンプトの削減効果を比較した。
- **フルプロンプト ＋ Gemma 26B (ベースモデル)**:
  - TTFT: **約 1825 ms**
  - 応答内容: 「えー、今さら？ 私はトモコ。あんたのそばでいろいろ手伝っちゃう...」と正しくトモコとして応答。感情出力形式も指定通り（`EMOTION:happy`）実行された。
- **ミニマルプロンプト ＋ LoRAモデル (フューズモデル)**:
  - TTFT: **約 766 ms** (約 1059 ms / 58% の高速化)
  - 応答内容: `[EMOTION:smug]\nやっと聞いてくれた？遅すぎ。\n\n私はトモコ。あんたの作業をサポートするために作られた...` と、トモコとしての口調・感情出力（[EMOTION:smug]）が十分に機能。

この検証から、マージしたLoRAモデルに対して約500文字のミニマルプロンプト（`minimal_persona.md` のトモコ設定＋静的ルール）を適用する構成は、ベースモデルに対してフルプロンプト（約2,700文字）を適用した場合と比較して、トモコの人格・応答品質を高いレベルで維持したまま、TTFTを **1秒以上（約58%）高速化できる** 非常に強力な構成であることが実証された。

### 気づき: E2E WebSocket音声対話テスト（3ターン）による実測
2026-06-11 のセッション8において、`make server-debug` (相槌 MaAI オフ設定) および `make smoke-ws-voice-latency` を用いて、実アプリサーバーを介した3ターンのダミー音声会話テストを実行した。
- **測定値 (E2E音声遅延 `voice_end_to_first_binary_audio`)**:
  - TURN 1 (智子短く返事して): **4348.9 ms** (LM処理 `transcript_to_first_reply_text` = 2727.8 ms)
  - TURN 2 (うん今の速度感はどう): **4576.3 ms** (LM処理 = 2696.1 ms)
  - TURN 3 (ありがとうもう一言だけお願い): **4664.3 ms** (LM処理 = 2923.1 ms)

以前の測定結果（prefillが5秒前後発生していた頃の `voice_end_to_first_binary_audio` 7.8s〜9.3s）と比較して、E2E遅延が **約4.3〜4.6秒とほぼ半減** しており、大幅な高速化が実証された。
また、応答内容（「了解。これくらいでいい？」「いい感じじゃない？これくらいがちょうどいい？」など）のキャラクター性（タメ口、生意気な後輩らしさ）も完全に機能しており、モデルマージ（fused_model）やdflash移行に伴う「意味の変更（崩壊）」や応答の劣化は一切起きていないことを確認した。

### 確定した判断: 音声合成エンジンとして Kokoro MLX (kokoro_mlx) を正式採用
2026-06-11 の追加検証において、`voicevox_tsumugi_chunked` から `kokoro_mlx` への切り替えによる速度的・アーキテクチャ的メリットを評価した。
- `kokoro_mlx` の実測値:
  - ファーストチャンク生成時間: **134.6 ms** (VOICEVOXの166.0 msと同等に高速)
  - 音声生成完了時間 (Total): **134.8 ms** (VOICEVOXの2,203.1 msに対し、約2.0秒高速)
  - RTFx: **約 21.3 倍速** (VOICEVOXの約 1.3 倍速に対し圧倒的)
  - 分割チャンク数: **1 チャンク** (VOICEVOXの15チャンクに対し、1個に集約)

この検証により、`kokoro_mlx` は会話最初の発話開始遅延（TTFTに連動する部分）において同等以上の速度を維持しつつ、音声の全体生成完了速度を「秒単位」で劇的に短縮できることが実証された。これにより、ネットワーク負荷の軽減、音声途切れ（プツプツ）リスクの解消、およびブラウザ側での割り込み（Barge-in）ハンドリングの簡素化が実現できる。
以上を評価し、`config/central_realtime.toml` および `config/edge_kitchen.toml` の `tts_backend` 設定を `"kokoro_mlx"` へ変更し、本番の音声合成エンジンとして正式採用した。

### 気づき: Gemma-4-12B vs Gemma-4-26B の応答速度・品質の実測比較
2026-06-11 のセッション12において、Gemma-4-12B (ローカル `mlx-community/gemma-4-12b-it-4bit`)、Gemma-4-26B (ローカル `mlx-community/gemma-4-26b-a4b-it-4bit`)、および Gemma-4-26B (dflash 8082ポート / fused_model) の応答速度（TTFT / デコード速度）と品質を実測比較した。

- **測定結果**:
  - **Gemma-4-26B (dflash @ 8082)**:
    - TTFT: **約 400 ms** (※競合プロセスのないクリーン状態時。同時推論時はメモリスラッシングにより20s超まで悪化)
    - デコード速度: **約 30〜31 tok/s**
    - 応答品質: 「絶好調だよ。そっちはどう？」 (生意気な後輩、タメ口。完璧)
  - **Gemma-4-26B (MLX local via mlx_vlm)**:
    - ロード時間: 5.84秒
    - GPUメモリ消費: 14.54 GB (ピーク14.54 GB)
    - TTFT: **5219.06 ms (約5.2秒)**
    - デコード速度: **21.59 tok/s**
    - 応答品質: 「絶好調だよ。てか、そんな急かさなくてもずっとここにいるから。何か手伝うことある？」 (後輩らしさが最も良く表現されている)
  - **Gemma-4-12B (MLX local via mlx_vlm)**:
    - ロード時間: 4.05秒
    - GPUメモリ消費: 7.32 GB (ピーク15.57 GB)
    - TTFT: **3625.88 ms (約3.6秒)**
    - デコード速度: **20.78 tok/s**
    - 応答品質: 「まあまあかな。正直、あんたの顔見て元気出たかも。何か手伝えることある？」 (後輩のツンデレ感や口調は良好に維持)

- **分析と考察**:
  - **速度**: 12B に切り替えることでモデルサイズが小さくなり、純粋なローカルプレフィル時間（TTFT）は 5.2秒から3.6秒に短縮され、メモリ消費も約半分（7.3GB）に抑えられる。しかし、音声対話において **3.6秒** のプレフィル遅延は依然として遅すぎる。
  - **dflash の影響力**: 一方、26B は `dflash` (Draft model + Prefix cache) の speculative decoding 最適化が機能しているため、TTFT を **400ms台** と圧倒的に高速化できている。現行の `dflash` は 12B（`gemma4_unified` マルチモーダルアーキテクチャ）をサポートしておらず起動できないため、12B に切り替えると dflash による最適化の恩恵を受けられず、応答性能が劇的に悪化する（400ms -> 3.6s）。
  - **結論**: メモリ消費の観点では 12B が有利であるものの、リアルタイム音声対話における応答レイテンシー（TTFT）を最優先する本システムの設計上、**dflash による 400ms 台 of 応答が可能な Gemma-4-26B の構成を維持すべきであり、Gemma-4-12B への切り替えは非推奨**であると決定する。品質についても 26B の方がキャラクター表現の深みで優位性がある。

### 気づき: Apple Speech STT (Speech.framework) のリアルタイムストリーミング（部分出力）動作確認
2026-06-11 のセッション18において、macOS の `say -v Kyoko` コマンドで生成した日本語音声（約 6.06秒、96989 サンプル）を用いて、Apple Speech STT のリアルタイム書き起こし検証スクリプトを実行した。
- `stream_min_audio_ms=1000` / `stream_interval_ms=1000` 設定において、32ms（512サンプル）チャンクのストリーミング投入により、1.27s, 2.60s, 3.94s, 5.30s, 6.70s と、1秒蓄積ごとに段階的に部分出力（Partial output）が得られることを確認した。
- 出力結果は、音声が進行するにつれて `"智子"` -> `"智子3 +3 ="` -> `"智子3 +3はいくつですか"` と正しく拡張されていった。
- 最後の約 0.94 秒分の音声（5.12s〜6.06s）は、6回目の発火しきい値である 98304 サンプルに達しなかったため `process_stream_chunk` では出力されなかったが、ループ終了後の最終一括文字起こし（`transcribe`）によって完全に補完され、`"智子3 +3はいくつですか今日の晩御飯は何にしようか悩むよね"` として正しくファイナルテキストが得られた。
- これにより、スライディングウィンドウ方式による Apple STT リアルタイム部分出力エミュレーションが期待通りに正常動作していることが実証された。

### 気づき: Turn-taking v2 shadow lane initial scaffold による非同期 wakeup notification の実現
2026-06-11 のセッション19において、Turn-taking v2 shadow lane の足場（Scaffold）構築を完了し、PostgreSQL LISTEN/NOTIFY を用いた非同期通知とバックグラウンドワーカーの連携を確認した。
- メインの会話スレッド (TomoroSession) を一切ブロックせずに、部分的な文字起こし (partial transcript) が得られるたびに `partial_transcript_observations` にレコードを保存し、`turn_taking_v2_observation` チャネルへ NOTIFY を発火する仕組みが正常に機能することを確認。
- `TurnTakingV2Worker` は通知をトリガーに即座に観測データを読み出してダミーのアドバイザリーを生成し、`turn_taking_v2_advisories` へ保存して `turn_taking_v2_advisory` を NOTIFY する。
- メイン側は `turn_taking_v2_advisory` 通知を受信してアドバイザリー情報を読み込み、メインの会話制御ロジックや応答レイテンシー（ホットパス）に一切影響を与えずに非同期にログ出力することに成功した。
- データベースの外部キー制約に対処するため、インテグレーションテストで親テーブル `conversation_sessions` にダミーデータを適切に作成・クリーンアップし、さらに pytest でのモジュール名衝突を避けるためにテストファイル名を `test_turn_taking_v2_db.py` と命名することが妥当であった。

### 確定した判断: シャドウワーカー（turn_taking_v2_worker.py）の分析スキーマを 2 キーに縮減し構造化出力を強制
2026-06-11 時点の段階では、シャドウワーカーによる意味的完了度分析で LLM に要求する JSON スキーマとして、5 つのキー（`semantic_saturation`, `remaining_info_risk`, `semantic_split_risk`, `safe_response_level`, `confidence`）をプロンプトベースの指示のみで出力させていた。この「構造化出力を強制せず、多くの冗長なパラメータを LLM から直接取得する」設計を否定する。重いスキーマを出力させるとモデルの挙動が不安定になり JSON のパース崩れを引き起こしやすいため、またローカル推論時に構造化出力を強制しないと形式の安定性が確保できないためである。

したがって、LLM に要求する JSON キーを `semantic_saturation` と `remaining_info_risk` の 2 つのみに縮減し、他のパラメータ（`safe_response_level` など）は LLM から得られた結果に基づいてプログラム側でルールベースで算出・補完するハイブリッド設計に移行した。
また、ローカル MLX 推論バックエンド（`GemmaMLXBackend` と `MLXLMBackend`）に `chat_stream_structured` メソッドを追加実装し、JSON Schema の指示をシステムプロンプトに動的に付加する構造化出力強制の仕組みを導入した。これにより、ローカル環境においても構造化出力と同等の安定した JSON 生成形式が強制的に確保される。

### 確定した判断: LoRA 一式は `loras/lora/` 配下で管理する
2026-06-12 時点で、プロジェクト直下に `lora/` を置く構成は否定する。
今後は複数 LoRA や派生モデルを同じ親ディレクトリで扱えるよう、既存の LoRA プログラム一式を `loras/lora/` に移動し、既定の `data` / `adapters` / `fused_model` パスも `loras/lora/...` に統一する。
`train.sh` / `evaluate.py` / `generate_data.py` / dflash 起動スクリプト / LoRA unit test は、この配置を前提にする。

### 確定した判断: Gemma 4 E2B MLX は短い即応テキストなら 0.5s 以内に入る
2026-06-12 の `memo.md` にある「E2B 級の小モデルが `うん` / `あー、それね` / `マジで？` 級の短い反応を 0.5s 以内に返せるか」を、このマシン上の `mlx-community/gemma-4-e2b-it-4bit` で実測した。

`_tools/bench_short_reaction_mlx.py` を追加し、`mlx_vlm` runner、`max_tokens=8`、warm-up 2 回後に短い相槌 prompt 10 本を測定した結果:
- model load: 7683.1ms（cold 起動は即応不可。startup warm-up / 常駐必須）
- warm first_text: p50 400.9ms / p95 475.9ms
- warm total: p50 412.5ms / p95 490.8ms
- 10/10 samples が first_text と total の両方で 500ms 以内

出力例は `それは大変`, `そうかも`, `速いね`, `意外だね`, `了解` で、短い backchannel / short_confirmation 用としては成立する。
同じ HF model を `mlx_lm` runner で読み込む比較も試したが、重み key 非互換で失敗したため、Tomoko の config 通り `gemma_mlx` / `mlx_vlm` 経路を使う。

ただしこの結論は LLM テキスト生成だけの話である。0.5s 以内に「発声」まで到達するには、短い相槌音声を cached/pre-generated audio として持つか、TTS を別途 100ms 台で確実に返すレーンへ寄せる必要がある。

### 確定した判断: Qwen3 1.7B MLX は Tomoko short persona LoRA 候補から外す
2026-06-12 に `Qwen/Qwen3-1.7B-MLX-4bit` を、Tomoko の persona / output format 焼き込み LoRA 候補として Step 1 評価した。

`tokenizer.apply_chat_template(..., enable_thinking=False)` を渡し、`prompts/base_persona.md` + `prompts/persona_overlay.md` を system prompt として 8 本の音声応答 prompt を評価したが、結果は以下だった。
- `EMOTION:` starts: 0/8
- valid emotion: 0/8
- `<think>` leak: 8/8
...
`enable_thinking=False` は API 引数として受理されたものの、実出力は `<think>` reasoning で始まり、英語または中国語混じりの reasoning が出た。
Tomoko の音声応答 format lane は先頭 `EMOTION:<label>`、許可 emotion、日本語本文、短文性が入口条件なので、このモデルは LoRA 学習前の prompt following 段階で不合格とする。

このため、`loras/short/` は作成せず、LoRA 学習と fuse には進まない。
次に小型 LoRA 候補を試すなら、thinking mode のない `mlx-community/Qwen2.5-1.5B-Instruct-4bit` または品質寄りの `mlx-community/Qwen2.5-3B-Instruct-4bit` を Step 1 から評価する。

### 確定した判断: Gemma 4 E2B OptiQ は short persona LoRA の本命候補
2026-06-12 に `mlx-community/gemma-4-e2b-it-OptiQ-4bit` を `mlx_lm` 経路で確認した。

`mlx_lm.load` / `mlx_lm.generate` は成功し、`mlx_lm lora` smoke も 2 iter で adapter 保存まで成功した。
これにより、以前の `mlx-community/gemma-4-e2b-it-4bit` が `mlx_lm` で重み key 非互換だった問題は、
Gemma E2B 一般ではなく、その MLX artifact と `mlx_lm` の相性問題として扱う。

Gemma 4 の tokenizer は default だと `<|think|>` を入れやすいが、
`tokenizer.apply_chat_template(..., enable_thinking=False)` を渡すと non-thinking lane で短い応答を生成できる。
一方、`mlx_lm` の chat dataset loader は `enable_thinking=False` を渡さないため、
Gemma 4 E2B OptiQ 用の `loras/short/data` は `messages` 形式ではなく、事前に non-thinking でレンダリングした
`text` JSONL とする。

本学習は 58 train / 10 valid、120 iter、12 LoRA layers、batch 2、peak memory 5.230GB で完了した。
adapter 適用ロードでは `EMOTION:` 12/12、許可 emotion 12/12、基本 pass 11/12。
短文速度は adapter 適用、max_tokens=16、5 prompt で avg 451.7ms、4/5 が 500ms 未満だった。

単一モデル化は注意が必要。
通常の quantized fuse は作成できるが、極小 prompt 評価で `EMOTION:` 7/10、許可 emotion 6/10 へ崩れたため非推奨。
`--dequantize` fuse は 8.7GB になり、`EMOTION:` 10/10、許可 emotion 10/10、基本 pass 9/10 で成立するが、
...
### 確定した判断: LFM2.5 8B A1B MLX は現行 Tomoko メイン会話 backend には採用しない
2026-06-13 に `LiquidAI/LFM2.5-8B-A1B-MLX-4bit`、Gemma 4 E4B 4bit、Gemma 4 26B A4B 4bit を
Tomoko の `ThinkFastMode` prompt 形状で比較した。
LM Studio 上の LFM model id は `lfm2.5-8b-a1b-mlx`、Gemma は
`gemma-4-e4b-it-mlx` / `gemma-4-26b-a4b-it-mlx` を使った。

LFM は HTTP request としては成功するが、OpenAI-compatible response の `content` が空で、
出力が `reasoning_content` にだけ入った。Tomoko の現行 `LMStudioBackend` は `delta.content` だけを
本文として扱うため、メイン会話に入れると無言になる。
`chat_template_kwargs.enable_thinking=false` や `think=false` は効かず、`max_tokens=256` でも
completion token は reasoning token のままだった。
Hugging Face model card でも assistant turn に explicit chain of thought を含む reasoning model と
説明されており、これは Tomoko の `EMOTION:<label>` 先頭契約と相性が悪い。

同じ測定で、warm run だけを見ると Gemma 4 E4B は first delta 平均 338.5ms / total 1391.6ms、
Gemma 4 26B A4B は first delta 平均 570.7ms / total 1695.6ms だった。
E4B は `EMOTION:` と valid emotion は全件守ったが、本文に `26B/E4B/LFM` のような model 名英字を
そのまま含む出力が 2/8 あったため、厳密な「本文は日本語だけ」契約では弱い。
26B は 8/8 で `EMOTION:`、valid emotion、日本語本文、reasoning leak なしを満たし、
会話の受け方も Tomoko の本番メイン推論として最も安定していた。

したがって、現時点のメイン会話 LLM は引き続き Gemma 4 26B A4B を基準線にする。
E4B は速度優先の rollback / 比較候補として残すが、LFM2.5 8B A1B は reasoning/content 分離を
Tomoko 側で扱う新しい backend を作るまでは採用しない。
artifact: `logs/main-llm-format-20260613.json`

### 確定した判断: LFM2-350M-Extract は shadow worker の意味飽和度判定には現状採用しない
2026-06-13 に `LiquidAI/LFM2-350M-Extract` と `mlx-community/gemma-4-e2b-it-4bit` を、
turn-taking v2 shadow worker の `semantic_saturation` / `remaining_info_risk` 判定に近い
16 ケースで比較した。

LFM2-350M-Extract は Hugging Face 上では JSON/XML/YAML などの structured extraction 向けと説明されているが、
Tomoko の現行 `chat_stream_structured` と同じ `name` / `strict` / `schema` を含む prompt では、
値入り JSON ではなく schema 自体を返しやすかった。
structured prompt では parse/shape/range が 0/16、compact prompt でも平均 first delta は約 290ms と速い一方で、
余計なキーや途中切れが多く strict shape は 0/16 だった。

Gemma 4 E2B は同じ測定で structured prompt が strict shape 16/16、finished accuracy 15/16、
compact prompt が strict shape 16/16、finished accuracy 16/16 だった。
したがって shadow worker の意味飽和度判定は、現時点では Gemma 4 E2B を基準線にする。
LFM2-350M-Extract を使うなら、現行 backend への単純差し替えではなく、grammar / constrained decoding /
strict postprocess を持つ専用 Extract backend として再評価する。
artifacts: `logs/turn-taking-semantic-lfm2-350m-extract-structured-20260613.json`,
`logs/turn-taking-semantic-lfm2-350m-extract-compact-20260613.json`,
`logs/turn-taking-semantic-gemma4-e2b-structured-20260613.json`,
`logs/turn-taking-semantic-gemma4-e2b-compact-20260613.json`

### 確定した判断: shadow worker の Gemma E2B semantic 判定は compact prompt へ移行する
2026-06-13 のベンチ結果では、`mlx-community/gemma-4-e2b-it-4bit` の turn-taking semantic 判定は、
現行 `chat_stream_structured` 相当より compact prompt の方が速く、strict shape / accuracy も良かった。
このため、上の「シャドウワーカー（turn_taking_v2_worker.py）の分析スキーマを 2 キーに縮減し構造化出力を強制」
という判断のうち、「2 キーに縮減」は維持するが、「構造化出力を強制」は Gemma E2B semantic lane では否定する。

`server/background/turn_taking_v2_worker.py` は `chat_stream_structured_with_trace_role` ではなく
`chat_stream_with_trace_role` で compact JSON prompt を流す。
採用条件は引き続き strict にし、`semantic_saturation` / `remaining_info_risk` の2キーぴったり、
かつ両方が 0.0..1.0 に収まる場合だけ LLM 結果として採用する。
余計なキー、JSON なし、範囲外は rule-based fallback とする。

### 確定した判断: v2-alpha は runtime 接続前の offline initiative motivation sandbox として扱う
2026-06-14 時点では、自発発話や横槍の調整を声出し実会話だけで行う方針を否定する。
声を出して会話しながら threshold / curiosity / teasing / attachment を調整するのは再現性と体力コストが高い。

まずは既存 `logs/turn-taking-main.jsonl` / `logs/turn-taking-v2-shadow.jsonl` と
DB の `utterance_candidates` / `arrival_candidates` を read-only snapshot として使い、
1 秒刻みの motivation waveform、fire marker、prompt preview、選択 marker の LLM dry-run を
offline artifact として生成する。

この v2-alpha sandbox は `/ws`、`TomoroSession` final gate、TTS、candidate lifecycle、
conversation lifecycle を変更しない。本番発話には接続せず、hardcoded gain / threshold による
simulation と HTML slider UI で「この値ならここで話したがる」を確認するための分析器として扱う。
UI 上で候補 JSON を見てコピペできることを優先し、候補手編集を browser-side 再計算へ反映する polish は
次段階で扱う。

### 確定した判断: initiative motivation sandbox は直近 session selector を既定 UI にする
2026-06-14 の実ログでは、単一の最新 window だけを見るより、
`conversation_session_id` ごとに直近 session を選べる UI の方が自発発話 threshold 調整に向いていた。

`make initiative-sim` は既定で `INITIATIVE_RECENT_SESSIONS=100` を `simulate_initiative_motivation` へ渡し、
main / v2 ログを session ごとに grouped replay した multi-session JSON / HTML を生成する。
実際の現在ログでは session_count は 6 件だった。
fire marker id は session 間で衝突しないよう `s1-marker-1` のように session index prefix を付ける。

### 確定した判断: initiative-sim の session selector は DB conversation_logs を主入力にする
2026-06-14 の確認で、`logs/turn-taking-v2-shadow.jsonl` だけから `conversation_session_id` を拾うと、
session selector に `v2_shadow/speech_decision_score` だけの session が並び、会話本文が timeline に出ないことが分かった。

会話本文の source of truth は `conversation_logs` なので、
`make initiative-sim` の recent session UI は DB `conversation_logs` の直近 session を主入力にする。
`logs/turn-taking-v2-shadow.jsonl` は同じ `conversation_session_id` がある場合だけ補助イベントとして合流する。
これにより現在 DB では session_count 100 を生成でき、先頭 session の timeline に user / tomoko turn が表示される。

### 確定した判断: initiative motivation UI は生成済み pressure を再利用せず browser-side で再積分する
2026-06-14 の確認で、HTML UI の slider 再計算が Python 生成済み `curiosity_pressure` /
`teasing_pressure` / `attachment_pressure` / `unspoken_pressure` を足し直しているだけだったため、
`curiosity_gain` などを変更しても pressure 波形そのものが変わらなかった。

UI で parameter tuning をする場合、browser-side でも events / candidates / params から
candidate selection、pressure decay、candidate pressure、silence attachment、score を再計算する。
また `silence_attachment_gain` は `attachment_gain` とは別の無音時 attachment 増分なので UI に明示する。

dead candidate 由来の `unspoken` 加算も `unspoken_gain` 配下に置く。
全 gain / silence attachment / floor / freshness を 0 にしたとき score が 0 になることを
initiative motivation sandbox の単体テストで保証する。

### 確定した判断: thinker2 は外界・人間状態の background cognition worker として扱う
2026-06-16 時点では、カレンダー・タイマー・world information・カメラ・スクリーンショット由来の観測を
turn-taking / stop / barge-in 系 shadow worker へ混ぜる方針を否定する。

shadow worker は会話 hot path 近傍の低遅延 advisory に限定し、thinker2 は raw frame / observation /
user context snapshot / candidate generation を扱う別 worker とする。

camera / screenshot の raw image は短命 artifact として保持し、DB には `human_presence_observations`、
`human_activity_observations`、`screen_activity_observations`、`user_context_snapshots` のような構造化結果を保存する。
発話候補は既存 `utterance_candidates` / `arrival_candidates` に載せ、TomoroSession の final gate は動かさない。

初期実装計画は `thinkerv2.md` にまとめた。

### 確定した判断: thinker2 T2.1 は calendar reminder だけを candidate source にする
timer / alarm reminder を thinker2 の通常 candidate source として重複実装する方針は否定する。
既存の timer / alarm は `timer_alarm_entries` の due claim と TomoroSession gate によって、
通常の自発発話より強い通知として扱われる。

T2.1 では Google Calendar 由来の予定開始 15 分前 / 5 分前 / 開始時刻だけを
`CalendarReminderSource` から deterministic `CandidateSeed` として生成する。
dedupe key は calendar event の `source_key` と reminder window に固定し、
`available_at` は metadata、`expires_at` は次 window または開始後 10 分として扱う。

### 確定した判断: T2.2 perception frame retention は retained flag で始める
camera / screenshot の古い raw artifact を T2.2 store が即座に物理削除する方針は否定する。
初段では `perception_frames.retained=false` に落として、削除や外部 artifact cleanup は後続 worker の責務にする。

`PerceptionFrame` DTO は `server/shared/models.py` に置き、store は `server/shared/perception.py` に分ける。
retention は source ごとに `captured_at DESC, created_at DESC` の最新 100 件を残す。

### 確定した判断: T2.3 presence は latest-frame 優先で backlog を処理しない
camera frame backlog を古い順に全件 VLM 処理する方針は否定する。
presence は「今いるか」の信号なので、`PresenceInferenceWorker` は retained camera frame の最新未処理を 1 件だけ処理し、
同時に残っている古い未処理 frame は backlog skip として数える。

stale frame は `stale_after` を超えたら推論しない。
Gemma / VLM 接続は `HumanPresenceInferenceBackend` protocol と `MlxVlmPresenceBackend` に閉じ込め、
TomoroSession や `/ws` hot path へ raw image を渡さない。

### 確定した判断: T2.4 activity は raw label と coherent label を分ける
`present=false` の時に activity row 自体を捨てる方針は否定する。
VLM が矛盾した `activity_label` を返しても `human_activity_observations` には raw observation として保存し、
snapshot 合成側で `coherent_activity_label(present=false, ...) == "away"` に丸める。

activity 推論も latest-frame 優先とし、backlog camera frame を古い順に処理しない。

### 確定した判断: T2.5 screen activity は screenshot source 専用に分ける
camera activity と screenshot activity を同じ observation table に混ぜる方針は否定する。
screen activity は `screen_activity_observations` に保存し、`screen_activity_label` と optional
`app_hint` / `document_hint` / `url_hint` を持つ。

screenshot も capture / inference とも latest-frame 優先にし、backlog screenshot を古い順に処理しない。

### 確定した判断: T2.6 user context snapshot は deterministic rule 初段にする
`user_context_snapshots` の `interaction_readiness` を初段から LLM synthesis 必須にする方針は否定する。
T2.6 では latest presence / activity / screen / calendar / world を読み、
deterministic rule で `away` / `do_not_disturb` / `low_intrusion_ok` / `chat_ok` /
`needs_help_maybe` を決める。

LLM synthesis は後続で `UserContextSnapshotBuilder` の境界に差し込む。
snapshot build は elapsed ms、source counts、skipped sources、source errors を trace と log に残す。

### 確定した判断: T2.7 context candidate は既存 thinker source として積む
screen / activity context candidate を TomoroSession へ直接渡す方針は否定する。
T2.7 では `ScreenContextSource` / `ActivityContextSource` を通常の thinker `InformationSource`
として追加し、既存 `utterance_candidates` の seed / dedupe / evaluator / final gate 経路に載せる。

`interaction_readiness` が `away` / `do_not_disturb` の snapshot は candidate を出さない。
`needs_help_maybe` は low intrusion の高め priority、`chat_ok` は通常 priority とする。

### 確定した判断: T2.8 world collection は既存 raw/ingest/interpret 境界を使う
thinker2 が world observation の normalizer / interpreter / candidate DB write を直接再実装する方針は否定する。
T2.8 では deterministic topic seeds を daily prompt template に追記し、
既存 `world.observe` operator から raw Markdown artifact を保存するだけにする。

raw artifact 後の normalizer / interpretation / candidate 化は既存 pipeline を使う。
candidate source 側では low confidence、stale、sensitive / private / do_not_speak を落とす。

### 確定した判断: T2.9 thinker2 runtime は snapshot/candidate 統合から始める
`thinker2` runtime 初段で実カメラ capture と VLM inference を常時起動する方針は否定する。
T2.9 では既存 DB の observation rows から `UserContextSnapshotBuilder` を動かし、
screen / activity context sources で candidate seed を作り、inspection HTML を生成する。

capture / inference workers は T2.3-T2.5 の部品として残し、常駐統合は後続で段階的に有効化する。
`run_thinker2.py --once` は新規 perception/snapshot schema を ensure してから実行する。
