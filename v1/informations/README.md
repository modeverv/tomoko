...
`information-collect-world` は隣 repo `tomoko-research-operator` の
`tomoko-research-mcp` を subprocess 起動し、`world.observe` tool で
ログイン済み Chrome / Perplexity を操作します。
生成された provider text は Tomoko 側で frontmatter を付け直してから
`work/YYYY-MM-DD-world-observation.md` に保存します。
