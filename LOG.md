...
### やったこと
- `scratch/smoke_apple_stt_streaming.py` を実時間シミュレーション（32ms チャンク・32ms スリープ）に調整し、実行。
- `say -v Kyoko` による約 6.06 秒（96,989 サンプル）の合成音声が 32ms チャンクごとにストリーミング投入され、1.27s, 2.60s, 3.94s, 5.30s, 6.70s（実経過時間）の間隔で段階的な部分出力（Partial output）が逐次表示されることを実証した。
- ループの最後（残りの 0.94 秒分）は蓄積サンプル数が次の発火閾値（98,304 サンプル）に満たなかったため `process_stream_chunk` では出力されなかったが、終了後の `transcribe` によって完全な文（「智子3 +3はいくつですか今日の晩御飯は何にしようか悩むよね」）として正しくファイナルテキスト化され、仕様通りの補完挙動となることを確認した。
- 検証に基づき、`MEMORY.md` の「気づき」に検証結果を追記した。
- ユーザーの「gitは触らないで」の指示を遵守し、Git 関連のコマンド実行やコミット、プッシュは一切行わなかった。

### 詰まったこと・解決したこと
- 特になし（計算通りの周期で期待される出力が得られた）。

### 次のセッションでやること
- ユーザー指示に従い、`PLAN.md` の `Turn-taking v2 shadow lane initial scaffold` フェーズに沿った実装（DDL定義や PostgreSQL NOTIFY 連携など）に移行する。

## 2026-06-11 セッション17

### やること（開始時に書く）
- リアルタイム出力（部分出力）が可能な Apple Speech STT バックエンドの追加（あるいは既存への機能追加）。
- 先行してテストコードを作成し、その後に実装を行う。
- テスト通過を確認してコミット・プッシュする。

### やったこと
- `AppleSpeechSTT` バックエンドクラス（`server/edge/pipeline/stt_apple.py`）にストリーミング（部分出力）サポートを追加：
  - `__init__` 引数に `streaming`, `stream_interval_ms`, `stream_min_audio_ms` を追加し、ストリームバッファ関連インスタンス変数を追加。
  - `process_stream_chunk` メソッドを実装し、一定音量・長さの音声が溜まるごとに一時WAVを書き出して `transcribe_audio`（AppleSpeechSTT.appの呼び出し）を実行し、以前のストリーミング出力と異なる場合に `Transcript(is_final=False)` として途中結果を逐次返却するよう設計。
  - `reset_stream` メソッドを実装し、ストリームバッファのリセットをサポート。
- `server/edge/pipeline/stt.py` の `create_stt_transcriber` 関数において、`apple_speech` タイプ初期化時にストリーミングオプションを正しく伝搬するよう改修。
- `tests/unit/test_stt_backends.py` に、Apple Speech STT のストリーミング（部分出力）と重複テキストの抑制を検証するユニットテストを追加。
- `pytest -m unit` により、新しく追加したテストを含む 659 件 of ユニットテストがすべて正常に通過することを確認。
- ユーザーの指示（「gitは触らないで」）に従い、Git コミットおよびプッシュは行わず、コード修正とテスト検証完了のみとした。

### 詰まったこと・解決したこと
- 特になし。

### 次のセッションでやること
- ユーザーからフィードバックを得て、他のストリーミングバックエンドへのチューニングや、v2 連携テストを行う。

## 2026-06-11 セッション16

### やること（開始時に書く）
- `v2.md` の設計思想と ChatGPT（GPT-4o リアルタイムAPIなど）の音声対話システムのアプローチの共通点・相違点を考察し、技術的類似性について評価する。

### やったこと
- `v2.md` アーキテクチャと ChatGPT（GPT-4o / Realtime API）の技術的アプローチを比較した。
  - **共通点**: 単なる音響VAD（無音検出）依存からの脱却、意味情報（テキストコンテキスト）主導での発話決定、フライング処理（仮推論）、割り込み時のキャンセル処理など。
  - **相違点**: ChatGPT が End-to-End のネイティブ・マルチモーダルモデルであるのに対し、Tomoko はカスケード型（STT -> LLM -> TTS）であり、体験の近さをソフトウェアアーキテクチャ（シャドウレーン、リアクティブ制御）で疑似再現しようとしている点。また、Tomoko は `TomoroSession` が厳格な制御主権を保持する。
- 結論として、「カスケード接続した OSS / ローカル環境において、ChatGPT のような E2E モデルが提供する高度な会話の呼吸をいかにシミュレート・再現するか」という極めて先鋭的なアプローチであることを整理した。

### 詰まったこと・解決したこと
- 特になし。

### 次のセッションでやること
- ユーザーに比較分析結果を報告し、今後の設計・開発のモチベーションを共有する。
...
### やること（開始時に書く）
- `v2.md` を読み、提案されているアーキテクチャが「Gemma 12B/26Bの応答速度」や「VAD/会話のフットプリント」といった現在の悩みを解決する抜本的な解決策になり得るかを評価・考察する。

### やったこと
- `v2.md` のアーキテクチャ設計を詳細に確認し、以下の観点から現在の課題に対する評価・考察をまとめた：
  - **VAD短縮による破綻問題の解決**: 音響無音（VAD）を主役から一要素に格下げし、Partial STT に基づく意味的飽和度（Semantic Saturation）で発話判定を行うため、200msといった短い息継ぎでも未完判定（participation gate）で無視されず会話が維持される。
  - **プレフィル遅延問題の解決**: `Provisional Inference`（仮推論・フライング推論）によってユーザーが話し終える前に推論を開始し、12Bローカルなどの重いプレフィル時間を見かけ上相殺する。
  - **懸念点**: Partial STTの出力速度と精度への依存性、およびフライング推論の無駄撃ちによるGPU/メモリ資源の競合（スラッシング）リスク。
  - **手法の妥当性**: 実装への直接投入を避け、ホットパスを絶対に壊さない `Shadow Lane`（影の観測レーン）としてスキャフォールドを作成する設計思想は極めて安全かつ適切。
- 結論として、「VAD依存の限界」と「ローカル推論のレイテンシー限界」を意味論的に乗り越えるための「極めて抜本的かつ合理的な作り替えである」と評価した。

### 詰まったこと・解決したこと
- 特になし。

### 次のセッションでやること
- ユーザーからのフィードバックに基づき、v2.mdのフェーズScaffold（LISTEN/NOTIFYを用いたShadow Lane開発）への着手等を検討する。

## 2026-06-11 セッション13

### やること（開始時に書く）
- VAD の無音閾値（vad_silence_ms）を 200ms に設定した場合の影響・破綻リスクの考察およびコード側の関連ロジックの調査。
- 200ms 設定時の挙動シミュレーションや、`participation gate` との相互作用について評価する。

### やったこと
- VAD 200ms 設定時のシステム全体の動作影響および破綻シナリオをコードロジック（`VADProcessor` および `WakeWordJudge` / 未完判定）に基づいて分析した。
- 以下の破綻リスクを特定：
  1. **発話の細切れ化**: 200ms は日常会話の息継ぎ（ブレス）や微小な休止（「ともこ、[250ms] 明日の予定は？」等）より短いため、発話が強制分断される。
  2. **ウェイクワードと本発話の分断による応答不全**: 「ともこ（200ms）〜」のタイミングで別セグメント化されるため、第2セグメントにウェイクワードが入らず、会話が開始できない。
  3. **未完判定（participation gate）による全シグナル無視**: 分断された短いセグメント（助詞「の」「で」止めやフィラー）が、`WakeWordJudge._looks_like_low_confidence_followup` に引っかかり、全て「observer」モード（返答なし）として処理され、ユーザーの意図が無視され続ける。
- 現行の「VAD 400ms ＋ participation gate」が、会話の結合性と即時応答 balances を取るベストエフォートな構成であることを再確認した。

### 詰まったこと・解決したこと
- 特になし（調査・考察タスクのため）。

### 次のセッションでやること
- ユーザーに調査結果を報告し、VAD 200ms への設定変更は破綻リスクが極めて高いため見送る（400ms維持）ことを提案する。
...
### やること（開始時に書く）
- 本番サーバーの再起動を行い、実際の音声対話テストにて 400ms VAD 設定と未完判定（participation gate）＋相槌の実際の動作感を確認・微調整する。

### やったこと
- PyPIより `maai==0.1.16` を本環境へ新規インストールし、相槌（MaAI）バックチャネル機能のインポートエラーを解決。
- ハングしていたdflashサーバーを再起動し、元の `lora/fused_model` が8082ポートで正常起動・応答することを確認。
- `make screen-stop && make screen-runtime` を実行し、Uvicornサーバー（8000番ポート）を含む対話システムプロセス群を一括でクリーン起動。
- `make smoke-ws-voice-latency` によるE2E音声対話テスト（3ターン）を実行。
- 実測ログより、VADが 400ms で迅速に発話終了を検知し、音声終了からSTT確定までが約 300〜400ms（VAD400ms含む全体でも700ms前後）と大幅に高速化していることを確認。
- MaaiBackchannelTap が CPU デバイス上で正常に初期化・起動し、対話中もエラーを発生させることなく正常に動作していることを検証。
- 会話中のウェイクワード検知および engaged follow-up 判定も、400ms の短いVAD判定時間と競合することなく安定して行われていることを確認。

### 詰まったこと・解決したこと
- 前回サーバーがポート8000を掴んだまま終了していたため `Address already in use` エラーとなった。`pkill -9 -f uvicorn` を用いてプロセスを強制終了させてポートをクリーンに解放し、再度起動することで解決した。

### 次のセッションでやること
- ユーザーによるブラウザ等を用いた実機での動作確認を待ち、フィードバックに基づくパラメータ調整を行う。
...
### 次のセッションでやること
- ユーザーにE2Eテスト結果を報告し、指示に基づく次の作業または本番移行への調整。
...
- RuffによるE501（行の長さが100文字超過）エラーが発生したため、追加したテストコード内の引数リストを改行して整形

### 次のセッションでやること
- LoRA適用によるプレフィルレイテンシーの改善効果の実測および評価

## 2026-06-11 セッション6

### やること（開始時に書く）
- 音声処理およびSTT不安定に関するユーザーの質問について調査し回答する

### やったこと
- `server/edge/pipeline/stt_gate.py` および `client/main.js`、`config/central_realtime.toml` などを調査
- クライアント側・サーバー側の音声処理（SttSignalGate, bandpass フィルタ, ブラウザのエコーキャンセル等）の現状を整理し、STT不安定の原因と対策についてユーザーに回答

### 次のセッションでやること
- ユーザーからのフィードバックをもとに、必要に応じて音声処理設定のチューニングやバグ修正を行う
...
- `pytest -m unit` で全てのユニットテスト（654 passed）が通過することを確認
- `ruff check lora` でリントチェックが全てパスすることを確認
...
### やること（開始時に書く）
- `make server-debug` でサーバーを起動し、起動時ウォームアップの完了を待つ
- `make smoke-ws-voice-latency` を1回目として実行し、LM StudioやTTSのキャッシュなどを温める（ウォームアップ測定）
- `make smoke-ws-voice-latency` を連続で2回目として実行し、こちらの応答速度（レイテンシー）を本測定データとして記録する
- 測定終了後、測定結果を整理し、サーバーを停止する

### やったこと
- デバッグサーバープロセス (`e09ca3b7-e421-4889-8791-b9f35307f833/task-268`) を立ち上げ、起動時のウォームアップ（Gemmaモデル、BGE Embedding、VAD/TTSのロードなど）完了を確認した。
- `make smoke-ws-voice-latency` によるテストを1回（ウォームアップ用）走行させたのち、連続でもう1回走行（2回目、本測定用）させた。
- 2回目の測定結果を抽出し、圧縮前および圧縮直後（ウォームアップなし状態）の数値と比較評価した。
- 測定完了後、バックグラウンドのデバッグサーバープロセスを停止した。

### 詰まったこと・解決したこと
- ウォームアップ完了後の2回目のテストにおいて、特に **TURN 1** の応答遅延が大幅に減少することを確認した。
  - `transcript_to_first_reply_text`: **2588.2ms** （圧縮前 3143.1ms から **554.9ms** の短縮。圧縮直後の未ウォームアップ状態 3545.3ms からは **957.1ms** の短縮）
  - `voice_end_to_first_reply_text`: **3728.7ms** （圧縮前 4447.3ms から **718.6ms** の短縮）
- プロンプト圧縮の効果（プロンプト全体を約90%削減し約2,662文字にしたこと）が、2回目以降の推論キャッシュが完全に効く状態（ウォームアップ後）において顕著に表れることが実証された。

### 次のセッションでやること
- この高速化したレイテンシー（特に初回ターンの2.5秒台）を基準値（baseline）とし、さらに実機や別モデルでの最適化を検討する。

## 2026-06-11 セッション3

### やること（開始時に書く）
- `make smoke-ws-voice-latency` を実行して、3ターンの対話における応答速度の変化を測定・評価する
- 測定終了後、測定結果とプロンプト圧縮前との比較を行い、ボトルネックなどを評価する
- バックグラウンドで動作しているデバッグサーバーを正常に停止する

### やったこと
- `make smoke-ws-voice-latency` を実行し、3ターンの対話における WebSocket 音声対話レイテンシーを計測した。
- 測定したレイテンシーを前回のプロンプト圧縮前（メッセージ履歴の生発話化および最新コンテキストの圧縮表現化を適用する前）と比較し、パフォーマンスの変化を評価した。
- デバッグサーバープロセス (`e09ca3b7-e421-4889-8791-b9f35307f833/task-223`) を正常に停止した。

### 詰まったこと・解決したこと
- プロンプトサイズを劇的に削減（履歴の重複排除により初回ターンで約90%の約25,000文字から2,662文字へ）したにも関わらず、LLMの応答（ファーストトークン生成）速度は前回の測定値と同等か、微増（+100ms〜400ms）という結果となった。
- サーバーログの解析から、LLMの推論処理自体の時間 (`reply_start_to_first_reply_text_ms`) が支配的（2.4秒〜3.5秒）であり、プレフィルトークン量の削減だけではローカル推論環境（LM Studio経由）でのファーストトークン生成の大幅な高速化には結びつかないことが分かった。

### 次のセッションでやること
- 今後さらに応答レイテンシーを削るために、ローカル推論バックエンドの設定や prefix caching の有効性（LM Studio側での設定など）を確認する。
- または、LLMへのコンテキストの投入自体をさらに見直し、不要なカレンダー情報やシステムプロンプトの冗長部分の切り詰め、あるいは軽量モデルへの動的スイッチを検討する。
...
### やること（開始時に書く）
- メイン推論プロンプト肥大化の原因（llm_prompt_content を履歴としてそのまま送信していること）を確認し、過去のユーザーメッセージを raw text (生発話) に戻す対策を実施する
- テストコードを更新し、関連ユニットテストが全て通過することを確認する

### やったこと
- `ThinkFastMode.prepare_prompt` において、過去の `user` メッセージ履歴の content として `turn.llm_prompt_content` を使わず、常に生発話 `turn.text` を使用するように変更した
- 過去の各ターンにおいてカレンダー予定（最大64件）やタスク、メモリなどの巨大なコンテキスト情報が重複して蓄積され、プロンプトが自己増殖的に肥大化・重複していた問題を解消した
- `test_think_fast_uses_saved_llm_prompt_content_for_user_history` テストを `test_think_fast_uses_raw_text_for_user_history_ignoring_saved_llm_prompt_content` に更新し、生発話を期待する形に変更した

### 詰まったこと・解決したこと
- dflash prefix cache 用に `llm_prompt_content` を使用する最適化がなされていたが、会話ターン進行による累積的なプロンプトサイズ爆発のリスクが極めて高かったため、生発話に戻した。これにより最新ターンのみにカレンダーなどの動的コンテキストが注入される本来の健全なプロンプト設計に戻した。

### 次のセッションでやること
- 実ブラウザで `make server-debug` を走らせ、会話中のプロンプトの肥大化が解消されていること、および応答性能を確認する
...
### やること（開始時に書く）
- 人間体感に近い `/ws` 実測を継続できるよう、macOS `say` 音声を 16kHz mono float32 chunk として送る外部 dummy client を追加する
- 有音チャンク送信後に無音チャンクを送り、VAD の speech end から transcript / reply_text / first audio までを記録する
- unit test と実 `/ws` smoke を行い、結果を `_docs/latency.md` に追記する

### やったこと
- `_tools/smoke_ws_voice_latency.py` を追加し、macOS `say` で生成した 16kHz mono WAV を 512 sample の float32 chunk として `/ws` に送るようにした
- 最後の有音 chunk は Silero VAD が受け付けるよう 512 samples に padding し、発話後は 1200ms の trailing silence を送るようにした
- `transcript_final` / `first_reply_text` / `audio_start` / first binary audio / `reply_done` を記録し、`logs/ws-voice-latency-*.json` に artifact を残すようにした
- `make smoke-ws-voice-latency` を追加した
- `_docs/latency.md` / `MEMORY.md` / `PLAN.md` に今回の実測線と結果を追記した

### 詰まったこと・解決したこと
- 初回 smoke は最後の有音 chunk が 512 samples 未満で、Silero VAD が `Input audio chunk is too short` を出して WebSocket が切れた
- ブラウザの AudioWorklet と同じ固定 chunk contract に合わせ、末尾有音 chunk を zero padding して解決した
- 実測では STT は `智子短く返事して`、reply は `了解。短くいくね。` で意味的には問題なかった
- voice-end to first binary audio は 7821.3ms で、server / dflash log 上は 1532 token prompt の prefix-cache miss と約5.0s prefill が支配要因だった

### 検証
- `uv run pytest -m unit tests/unit/test_smoke_ws_voice_latency.py -q`: 4 passed
- `uv run ruff check _tools/smoke_ws_voice_latency.py tests/unit/test_smoke_ws_voice_latency.py`: pass
- `make -n smoke-ws-voice-latency`: pass
- `uv run pytest -m unit -q`: 637 passed, 23 deselected
- `/ws` real smoke artifact: `logs/ws-voice-latency-20260607-say-short.json`
- `/ws` real smoke metrics
  - voice-end to transcript_final: 1357.2ms
  - voice-end to first reply_text: 7150.3ms
  - voice-end to first binary audio: 7821.3ms
  - transcript_final to first binary audio: 6464.1ms
  - binary audio chunks: 10

### 次のセッションでやること
- `make smoke-ws-voice-latency` を dflash restart 直後 / warmed prompt / DB context 少なめの条件で複数回測り、prefix cache hit 率と first audio の関係を見る
- 実 browser decode / playback scheduling との差が気になる場合は、Chrome 側の performance mark で補助測定する

## 2026-06-07 セッション9

### やること（開始時に書く）
- `/ws` voice latency smoke を3 turn程度の連続会話に対応させる
- Tomoko 側の返答が不定でも破綻しにくい無難な follow-up 入力で実測する
- prefill warm-up と実 turn の cache miss の違いを、実測ログに基づいて整理する

### やったこと
- `_tools/smoke_ws_voice_latency.py` を同一 WebSocket session の複数 turn に対応させた
- built-in `three-turn` scenario と複数 `--turn-text`、turn ごとの artifact、playback telemetry emulation、inter-turn pause を追加した
- `make smoke-ws-voice-latency` の既定を three-turn smoke にし、単発は `WS_LATENCY_SCENARIO=single` で残した
- 3 turn の実 `/ws` smoke を live dflash 26B / 実 STT / 実 VOICEVOX で測定した
- `_docs/latency.md` / `MEMORY.md` / `PLAN.md` に3 turn実測と prefill warm-up の判断を追記した

### 詰まったこと・解決したこと
- 初期の複数 turn smoke は browser playback telemetry を送らず、turn 2 / 3 が `barge_in` になった
- binary audio chunk 受信ごとに `playback_started`、`audio_end` で対応する `playback_ended` を送る emulation を追加した
- 1500ms の inter-turn pause を入れても turn 2 / 3 の artifact には `barge_in` が残った
- latency 主値は取得できているが、人間が完全に聞き終わってから次を話す条件は `WS_LATENCY_INTER_TURN_PAUSE_MS=3000` 以上でも比較する余地がある
- dflash startup warm-up は 804/808 token restore のように固定 prefix へ効いていたが、実会話 prompt は 1338 / 1337 / 1155 token が restored 0.0 で約4.9-5.3s prefill していた
- したがって warm-up で prefill が完全解消したとは扱わず、live prompt shape の固定化を引き続き見る

### 検証
- focused unit: `uv run pytest -m unit tests/unit/test_smoke_ws_voice_latency.py -q`
- focused ruff: `uv run ruff check _tools/smoke_ws_voice_latency.py tests/unit/test_smoke_ws_voice_latency.py`
- dry-run: `make -n smoke-ws-voice-latency`
- full unit: `uv run pytest -m unit -q`: 639 passed, 23 deselected
- diff check: `git diff --check`
- `/ws` three-turn artifact: `logs/ws-voice-latency-20260607-three-turn-final.json`
- turn 1 input / STT / reply: `トモコ、短く返事して。` / `智子短く返事して` / `了解。これくらいでいい感じ？`
- turn 2 input / STT / reply: `うん、今の速度感はどう？` / `うん今の速度感はどう` / `んー、今の感じはどうかな？ちょっと速すぎた？`
- turn 3 input / STT / reply: `ありがとう。もう一言だけお願い。` / `ありがとうもう一言だけお願い` / `えー、そんなに急いでどうしたの？何かお手伝いする？`
- voice-end to first binary audio: 7819.0ms / 8030.0ms / 9359.6ms
- voice-end to transcript: 1355.4ms / 1180.8ms / 1255.1ms
- voice-end to first reply_text: 7102.3ms / 7189.8ms / 7365.0ms

### 次のセッションでやること
- three-turn smoke を `WS_LATENCY_INTER_TURN_PAUSE_MS=3000` または 5000 でも測り、`barge_in` が消えるか確認する
- live prompt の prefix cache miss を減らすため、会話ログ・日時・task context の配置と安定化をさらに測る

## 2026-06-07 セッション10

### やること（開始時に書く）
- 4 turn の `/ws` scenario で、3 turn 目に時刻を聞き、4 turn 目に元の速度会話へ戻れるかを測る
- 現状 prompt order と、current user utterance を先に置いて時刻/context を後ろに置く変更後を比較する
- 意味論が壊れないか、voice-end to first audio / dflash prefill が改善するかを `_docs/latency.md` に記録する

### やったこと
- 4 turn time-return scenario を同一 `/ws` session で baseline / after / after-v2 / after-v3 / after-v4 として実測した
- `ThinkFastMode` の最後の user message を `CURRENT USER UTTERANCE` 先頭、`TURN CONTEXT` 後続に変更した
- `TURN CONTEXT` 内でも `CURRENT LOCAL TIME` を最後へ移動した
- `さっきの...話に戻って` を検出した turn だけ `TOPIC RETURN HINT` を入れるようにした
- `_docs/latency.md` / `MEMORY.md` / `PLAN.md` に A/B 結果と判断を追記した

### 詰まったこと・解決したこと
- baseline は時刻には正しく答えたが、4 turn 目で速度話題へ戻れなかった
- 時刻後置だけの after は平均 first audio が 6257.6ms -> 6002.1ms に小幅改善し、時刻回答も壊れなかったが、4 turn 目の話題復帰は直らなかった
- 固定 system rule で話題復帰を促す after-v2 は効かなかった
- dynamic `TOPIC RETURN HINT` を入れた after-v4 は4 turn 目で `今のテンポ、ちょうどいい感じかな？` と速度話題へ戻れた
- after-v4 の平均 first audio は 8522.9ms で悪化したため、topic hint は意味論改善として扱い、latency benefit とは扱わない
- dflash log では live prompt は引き続き full prefill cache miss (`restored 0.0`) だった

### 検証
- baseline artifact: `logs/ws-voice-latency-20260607-time-return-before.json`
- time-last artifact: `logs/ws-voice-latency-20260607-time-return-after.json`
- topic-hint artifact: `logs/ws-voice-latency-20260607-time-return-after-v4.json`
- baseline avg voice-end to first binary audio: 6257.6ms
- time-last avg voice-end to first binary audio: 6002.1ms
- topic-hint avg voice-end to first binary audio: 8522.9ms
- focused unit: `uv run pytest -m unit tests/unit/test_phase4_thinking.py -q`: 20 passed
- focused ruff: `uv run ruff check server/gateway/thinking/fast.py tests/unit/test_phase4_thinking.py`
- full unit: `uv run pytest -m unit -q`: 640 passed, 23 deselected
- focused ruff final: `uv run ruff check server/gateway/thinking/fast.py tests/unit/test_phase4_thinking.py tests/unit/test_phase8_memory.py`
- diff check: `git diff --check`

### 次のセッションでやること
- time-return scenario を scenario runner 化し、複数回の平均で topic hint の latency cost を判断する
- live prompt の full prefill miss を減らすため、recent conversation context の汚染と prompt cache 一致率を切り分ける

## 2026-06-07 セッション11

### やること（開始時に書く）
- `TOPIC RETURN HINT` を撤去する
- semantic steering なしで、current user utterance first / `CURRENT LOCAL TIME` last の latency を同じ4 turn scenario で再測する
- baseline と概ね同等の意味論として扱い、速度差を `_docs/latency.md` に追記する

### やったこと
- `server/gateway/thinking/fast.py` から `TOPIC RETURN HINT` / cue extraction を削除した
- 専用 unit test を削除し、prompt order の既存 unit は維持した
- 同じ4 turn `/ws` scenario を `logs/ws-voice-latency-20260607-time-return-no-topic-hint.json` として測定した
- `_docs/latency.md` / `MEMORY.md` / `PLAN.md` に撤去後の判断と結果を追記した

### 詰まったこと・解決したこと
- topic hint は今回の目的である latency 比較には余計な意味論改善だったため撤去した
- 撤去後も時刻回答は壊れず、4 turn 目は特別 hint なしで概ね速度/短さの話に戻った
- 平均 first audio は baseline より +155.4ms で、time-last が明確に高速とは断定しない

### 検証
- focused unit: `uv run pytest -m unit tests/unit/test_phase4_thinking.py tests/unit/test_phase8_memory.py -q`: 29 passed
- focused ruff: `uv run ruff check server/gateway/thinking/fast.py tests/unit/test_phase4_thinking.py tests/unit/test_phase8_memory.py`
- full unit: `uv run pytest -m unit -q`: 639 passed, 23 deselected
- diff check: `git diff --check`
- `/ws` no-topic-hint artifact: `logs/ws-voice-latency-20260607-time-return-no-topic-hint.json`
- avg voice-end to first binary audio: 6413.0ms
- turn 3 reply: `今は午後3時56分だよ。何か予定入ってるの？`
- turn 4 reply: `了解。これくらい短めがちょうどいい感じ？`

## 2026-06-07 セッション12

### やること（開始時に書く）
- dflash prefix cache が live Tomoko prompt で外れる原因を、raw payload / 本番 prompt / 実 `/ws` の順で切り分ける
- 1→2→3 の段階ごとに cache restored が動くか確認し、効き始めた段階で本番実装へ進める
- 実装した場合は unit / ruff / 実測を通し、`_docs/latency.md` に記録する

### やったこと
- Step1 raw dflash A/B で、履歴 user が raw transcript に戻ると cached_tokens=0 のまま、前回 wrapped content を保つと cached_tokens=3729 まで動くことを確認した
- `conversation_logs.llm_prompt_content` を追加し、`transcript` は raw 原本のまま、LLM に渡した user content だけ派生保存するようにした
- `ThinkFastMode` に prompt preparation / audit path を追加し、prior user turn は `llm_prompt_content` があればそれを履歴 content に使うようにした
- `TomoroSession` は user turn row id を reply task へ渡し、LLM 送信直前に同じ current user content を row に保存するようにした
- `_tools/bench_runtime_thinkfast_conversation.py` も prompt audit path を使うようにし、実DB simulation で同じ挙動を測れるようにした
- active session 中は same-session turns だけを recent context とし、global recent turns の補完で prompt prefix が揺れないようにした
- same-session turns の TTL cache を無効化し、fast context budget を 20ms から 50ms に広げた
- live DB に `ALTER TABLE conversation_logs ADD COLUMN IF NOT EXISTS llm_prompt_content TEXT NULL` を適用した
- 検証用 server を `127.0.0.1:8010` で起動し、`/ws` three-turn smoke を実行後に停止した

### 詰まったこと・解決したこと
- Step2 では `llm_prompt_content` 保存だけでは cache hit しなかった
  - 原因は same-session に global recent turns を補完し、turn ごとに補完数が変わって prompt 先頭がずれることだった
- same-session turns の TTL cache を無効化しただけでは fast 20ms budget で DB read が timeout し、turn3以降の context が0件になることがあった
  - baseline 文脈を落とす方が cache と意味論を壊すため、fast budget を 50ms に広げた
- 既存の 8000 番 server は `/ws` handshake が timeout したため、検証用に 8010 番で別 server を起動して確認した

### 検証
- Step1 raw artifact: `logs/dflash-prefix-step1-history-shape-20260607-161119.json`
  - raw history: cached_tokens=0
  - wrapped history: cached_tokens=3729, elapsed 1054.2ms
- Step2 runtime artifact: `logs/runtime-thinkfast-26b-step2-prompt-content-cache-20260607.json`
  - avg first 5983.3ms / total 7890.9ms, still restored 0.0
- Step3 runtime artifact: `logs/runtime-thinkfast-26b-step3-fast50-cache-20260607.json`
  - avg first 3409.6ms / total 5551.5ms
  - recent turns: 12 / 2 / 4 / 6 / 8 / 10
  - turn3-6 first text: 2092.9 / 1954.9 / 2217.4 / 2051.6ms
  - dflash prefix hits included 1602/2019, 2015/2436, 2432/2859, 2855/3282
- `/ws` artifact: `logs/ws-voice-latency-20260607-162554.json`
  - voice-end to first binary audio: 4705.8 / 7624.5 / 2938.6ms
  - turn3 transcript-to-first-reply-text: 1076.6ms
  - turn3 reply: `いいよ、何て言えばいい？`
  - dflash log: prefix cache hit 1534/1916, prefill restored 1926.2 tok/s
- focused unit: `uv run pytest -m unit tests/unit/test_phase88_context_snapshot.py tests/unit/test_phase85_conversation_sessions.py tests/unit/test_phase4_thinking.py -q`
  - 54 passed
- context perf: `uv run pytest -m perf tests/perf/test_context_snapshot_latency.py -q`
  - 1 passed
- focused ruff: pass
- full unit: `uv run pytest -m unit -q`
...
### 次のセッションでやること
- 4 turn time-return scenario でも新しい prompt cache 構造の意味論と speed topic 復帰を再確認する
- 31B 側の memory_extraction / summary lanes でも `llm_prompt_content` 履歴が悪影響を出さないか確認する

## 2026-06-07 セッション13

### やること（開始時に書く）
- same-session conversation_logs read が本当に SQL で遅いのか、インデックス利用と Python 側接続込み時間を切り分ける
- `EXPLAIN ANALYZE` と `PostgresConversationLogWriter.read_recent_turns_for_session()` の実測を比較する

### やったこと
- latest conversation_session_id に対して `EXPLAIN (ANALYZE, BUFFERS)` を実行し、`conversation_logs_session_recorded_at_idx` が使われていることを確認した
- SQL execution は 0.077ms、reused connection の query は avg 0.469ms / max 1.899ms だった
- `PostgresConversationLogWriter.read_recent_turns_for_session()` は毎回 connection を作るため、connection+ensure+query で初回 24.525ms、以後 7.2〜9.6ms 程度だった

### 詰まったこと・解決したこと
- インデックス未使用ではなく、接続作成込みのアプリ側時間が context build の 10ms 前後として見えていた
- same-session SQL 自体は十分速く、次に詰めるなら connection reuse / pool 化、または schema ensure を startup/migration 側へ寄せるのが筋

### 次のセッションでやること
- context build をさらに 20ms へ戻したい場合は、Postgres store の connection pool 化を小さく実装して実測比較する

## 2026-06-07 セッション14

### やること（開始時に書く）
- PostgreSQL hot path の毎回 connection 作成をやめ、既存ライブラリの connection pool を使う
- まず `PostgresConversationLogWriter` / session store の same-session context read/write を pool 化し、実測で context read が 1ms 台へ寄るか確認する
- unit / ruff / 実DB microbench を通し、必要な docs を追記する

### やったこと
- `psycopg_pool.AsyncConnectionPool` を使う `server/shared/db_pool.py` を追加し、PostgreSQL の connection pool を DSN ごとに共有するようにした
- `PostgresAmbientLogWriter` / `PostgresConversationLogWriter` / `PostgresConversationSessionStore` の connection 作成を pool 経由へ置き換えた
- `psycopg-pool` dependency を追加した
- connection pool の再利用と close を unit test で固定した
- pool 化後に fast context budget を 50ms から 20ms へ戻した

### 詰まったこと・解決したこと
- same-session SQL 自体は index scan で 0.077ms、reused connection query でも avg 0.469ms だった
- 遅さは SQL / index ではなく、既存 writer が毎回 PostgreSQL connection を作る app 側 cost だった
- pool 化後の `read_recent_turns_for_session()` は初回 pool open 込み 16.378ms、warm avg 0.936ms / max 1.217ms まで下がった
- runtime simulation では fast 20ms budget でも same-session recent turns が 12 / 2 / 4 / 6 / 8 / 10 と維持された

### 検証
- 実DB microbench: pooled writer warm avg 0.936ms / max 1.217ms
- runtime artifact: `logs/runtime-thinkfast-26b-dbpool-fast20-20260607.json`
  - avg first text 4150.2ms / avg total 5764.1ms
  - context elapsed 15.65 / 11.40 / 11.76 / 20.28 / 20.28 / 21.17ms
  - dflash prefix hits: 1579/1992, 1988/2407, 2403/2824, 2820/3238
- focused unit + perf: `uv run pytest -m unit tests/unit/test_db_pool.py tests/unit/test_phase88_context_snapshot.py tests/unit/test_phase85_conversation_sessions.py tests/unit/test_phase4_thinking.py -q && uv run pytest -m perf tests/perf/test_context_snapshot_latency.py -q`
  - 56 passed, perf 1 passed
- focused ruff: pass
- full unit: `uv run pytest -m unit -q`
  - 642 passed, 23 deselected

### 次のセッションでやること
- pool warm 初回 16ms を server startup warm-up / readiness で吸収できているか、`make server-debug` の起動直後 first turn で確認する
- schema ensure を migration/startup 側へ寄せると、hot path の初回 jitter をさらに減らせるか検討する

## 2026-06-07 セッション15

### やること（開始時に書く）
- 作業ツリーがクリーンであることを確認してから、context layer の session scoped cache を実装する
- `calendar_events` と `task_ledger` は active conversation session 中に session_id 単位で保持する
- `same_session_turns` は引き続き cache せず、task ledger write 成功時は session cache を invalidate する
- unit / perf / ruff / 必要な docs を更新して commit / push する

### やったこと
- `ContextSnapshotBuilder` に session scoped cache を追加し、`calendar_events` / `task_ledger` を active session 中だけ session_id key で保持するようにした
- `active_session_id=None` の ambient / startup 側は従来の TTL cache のままにし、会話外 context を無期限保持しないようにした
- session_id 変更時は session scoped cache を prune し、key に session_id を含めて並行 build でも別 session の値を誤用しないようにした
- `ContextSnapshotBuilder.invalidate_session_cache_source()` を追加し、task ledger update が `created` / `completed` で確定した時だけ `task_ledger` session cache を消すようにした

### 詰まったこと・解決したこと
- 既存の `calendar_events` 60秒 TTL だけでは、session_id が変わっても同じ日付なら cache hit してしまう
  - active session では通常 TTL cache を bypass し、session scoped cache を使うことで解決した
- task ledger は会話中に Tomoko 自身が更新するため、session cache だけだと stale になる
  - `task_ledger_update_recorded` の reducer で source invalidate を同期的に呼ぶようにした

### 検証
- focused unit: session scoped cache / session change miss / task invalidation
  - 4 passed
- related unit + perf: `uv run pytest -m unit tests/unit/test_phase88_context_snapshot.py tests/unit/test_task_ledger_voice.py tests/unit/test_phase85_conversation_sessions.py tests/unit/test_phase4_thinking.py -q && uv run pytest -m perf tests/perf/test_context_snapshot_latency.py -q`
  - 65 passed, perf 1 passed
- 実DB context microbench:
  - same session turn1: elapsed 19.317ms, `calendar_events=False`, `task_ledger=False`
  - same session turn2: elapsed 0.117ms, `calendar_events=True`, `task_ledger=True`
  - same session turn3: elapsed 0.090ms, `calendar_events=True`, `task_ledger=True`
  - new session: elapsed 10.487ms, `calendar_events=False`, `task_ledger=False`
- runtime artifact: `logs/runtime-thinkfast-26b-session-context-cache-20260607.json`
  - avg first text 2479.1ms / avg total 4794.8ms
- full unit: `uv run pytest -m unit -q`
  - 646 passed, 23 deselected
- focused ruff: pass

### 次のセッションでやること
- `/ws` three-turn smoke でも calendar/task context を含む実会話で session cache hit が trace に見えるよう、smoke artifact に `cache_hits` を保存するか検討する

## 2026-06-07 セッション16

### やること（開始時に書く）
- 現行 prompt order と `SESSION STABLE STRUCTURED CONTEXT -> CURRENT USER UTTERANCE -> TURN DYNAMIC / RECALL CONTEXT` order を実DB/実dflash 26B runtime simulation で比較する
- 速度だけでなく、応答が現在発話へ追従しているか、calendar/task/persona/memory に引っ張られていないかを確認する
- 有意な差があれば `_docs/latency.md` / `MEMORY.md` に記録する

### やったこと
- `ThinkFastMode` の最後の user message を、`SESSION STABLE STRUCTURED CONTEXT` / `CURRENT USER UTTERANCE` / `TURN DYNAMIC / RECALL CONTEXT` の順に変更した
- stable structured 側には persona slice / calendar / task を置いた
- dynamic recall 側には response directive / short memory / long term memory / research / current time を置いた
- current order と proposed order を fast / deep / deep+temporary task の3条件で実DB/実dflash 26B simulation 比較した

### 詰まったこと・解決したこと
- fast depth では calendar が無効で、実DB active task も0件だったため、提示構造の本命である calendar/task の効果は見えなかった
  - deep depth で calendar 41件を含めて再測した
  - さらに `source='codex_prompt_order_bench'` の active task 3件を一時投入して deep を再測し、最後に削除して `bench_task_rows 0` を確認した
- deep は context build が 100ms budget 付近で timeout するため、prompt order だけでなく retrieval/source timeout の影響も混ざる
  - first text の単発差はノイズを含むため、意味論と total も合わせて判断する

### 検証
- fast, real DB / dflash 26B:
  - current avg first 2249.5ms / total 4306.8ms
  - stable-first avg first 2241.3ms / total 4052.8ms
  - 意味破綻なし、calendar/task は0件
- deep, calendar 41件 / task 0件:
  - current avg first 6303.1ms / total 9678.5ms
  - stable-first avg first 6710.4ms / total 9286.5ms
  - first は +407.3ms 悪化、total は -392.0ms 改善、意味破綻なし
- deep, calendar 41件 / temporary task 3件:
  - current avg first 7619.5ms / total 9880.5ms
  - stable-first avg first 7273.1ms / total 9137.8ms
  - first は -346.4ms 改善、total は -742.7ms 改善、意味破綻なし
- artifacts:
  - `logs/runtime-thinkfast-26b-prompt-order-current-20260607.json`
  - `logs/runtime-thinkfast-26b-prompt-order-stable-first-20260607.json`
  - `logs/runtime-thinkfast-26b-prompt-order-current-deep-20260607.json`
  - `logs/runtime-thinkfast-26b-prompt-order-stable-first-deep-20260607.json`
  - `logs/runtime-thinkfast-26b-prompt-order-current-deep-with-tasks-20260607.json`
  - `logs/runtime-thinkfast-26b-prompt-order-stable-first-deep-with-tasks-20260607.json`
- focused unit: `uv run pytest -m unit tests/unit/test_phase4_thinking.py tests/unit/test_phase8_memory.py -q`
  - 30 passed
- full unit: `uv run pytest -m unit -q`
  - 646 passed, 23 deselected
- context perf: 1 passed
- focused ruff: pass

### 次のセッションでやること
- deep context build が 100ms で張り付く原因を source timing / timeout 別に分け、calendar/task prompt order とは別に最適化する

## 2026-06-11 セッション1

### やること（開始時に書く）
- VAP (Voice Activity Projection) 予測モデルを追加で動かし、その予測確度（ターン譲渡確率）に応じて VAD の無音判定時間（`silence_ms`）を動的制御するハイブリッド制御（Phase TT-v2.5）を実装する。
- 150, 650 (max 800), 90% という閾値を config 設定に外だしする。
- `MaaiBackchannelTap` において `bc_2type` モデルと並行して `vap` 予測モデルを起動し、推奨無音時間を算出する。
- `TomoroSession` の音声処理ホットパスで、VAPの推奨値を VAD に適用する。
- VADProcessor リセット時にデフォルトの無音値に復元する。
- VAPの動的無音制御の動作を保証するユニットテストを追加する。

### やったこと
- `config/central_realtime.toml` と `config/edge_kitchen.toml` に `vap_hybrid_enabled = true`, `vap_hybrid_min_silence_ms = 150`, `vap_hybrid_max_silence_ms = 800`, `vap_hybrid_threshold_probability = 0.90` を追加した。
- `server/shared/config.py` の `AudioSection` にこれらの設定を追加した。
- `server/edge/pipeline/vad.py` の `VADProcessor` に `default_silence_ms` を持たせ、`reset()` 時に `silence_ms` をデフォルト値に戻すようにした。
- `server/gateway/maai_backchannel.py` で、`vap_hybrid_enabled` が有効な場合、`bc_2type` に加えて `mode="vap"` の Maai インスタンスも同時に起動し、予測値 `p_future[1]` が 90% 以上の際に `min_silence_ms + (1.0 - p_yielding) * delta_silence_ms` で算出した推奨無音時間を `get_recommended_silence_ms()` として提供する機能を実装した。
- `server/edge/main.py` の `audio_interaction_tap_factory` に `config.audio` を渡すようにリファクタリングした。
- `server/session.py` の `process_audio_chunk` で、VAPの推奨時間があれば VADProcessor の `silence_ms` に適用するようにした。
- `tests/unit/test_maai_backchannel_tap.py` に `test_session_applies_recommended_silence_from_audio_tap` ユニットテストを追加した。

### 詰まったこと・解決したこと
- `Maai` インスタンスから取得できる `vap` モードの出力キー構造（`p_now`, `p_future`, `vad` が `[user, system]` の 2 要素リストであること）を事前に python ワンライナーで検証し、確度 `p_future[1]` から逆数計算を行うようにした。
- ユニットテストを実行する際、最初は python が `ModuleNotFoundError` になったが、`.venv` 内の `pytest` を呼び出すことで dependency エラーなく全テストが通ることを確認した。

### 検証
- 新規テスト: `.venv/bin/pytest tests/unit/test_maai_backchannel_tap.py`
  - 5 passed
- 全ユニットテスト: `.venv/bin/pytest -m unit`
  - 676 passed, 24 deselected

### 次のセッションでやること
- 実会話テスト環境（`make server-debug`等）にて、発話終了時のテンポ（確信度が高い場合は 150ms 程度で極小になり、言い淀み時は 800ms 近くたっぷり待ってくれること）の体感性能と音声対話での自然さを微調整・評価する。

## 2026-06-11 セッション25

### やること（開始時に書く）
- 150, 650, 90% というパラメータについて、単に max_silence_ms から delta_silence_ms を算出するのではなく、設定ファイル (config/central_realtime.toml, config/edge_kitchen.toml) に delta_silence_ms = 650 として直接外だしできるよう修正する。
- 逆数的判定への VAD 適用と VAP 並行起動の整合性を改めて確認し、テストを追加する。

### やったこと
- config/central_realtime.toml と config/edge_kitchen.toml で `vap_hybrid_max_silence_ms = 800` を削除し、`vap_hybrid_delta_silence_ms = 650` を直接指定するように変更した。
- server/shared/config.py の AudioSection および NodeConfig.load にて、`vap_hybrid_delta_silence_ms` パラメータを追加し、従来の max_silence_ms 設定に対しても相互にフォールバックされる互換ロジックを実装した。
- server/gateway/maai_backchannel.py にて、`vap_hybrid_delta_silence_ms` が設定にある場合はそれを優先し、ない場合に max_silence_ms から delta_silence_ms を計算するように修正した。また環境変数 `TOMOKO_MAAI_VAP_HYBRID_DELTA_SILENCE_MS` からの上書き設定にも対応した。
- tests/unit/test_maai_backchannel_tap.py に、新しく追加した delta_silence_ms パラメータが正しくパースされ、MaaiBackchannelTap の config に設定されることを検証するユニットテスト `test_create_maai_backchannel_tap_from_config` を追加した。
- pytest -m unit を実行し、全テスト (677 passed) が正常にパスすることを確認した。
...
### やること（開始時に書く）
- ボタンと Space キー押下でシグナルを UI から送ってログに記録する「Phase TT-v2.6」の対応を PLAN.md に追加し、実装を行う。
- クライアント側から WebSocket 経由でデバッグ用の発話終了マーカーイベントを送信する仕組みと、サーバー側で受信して turn-taking-main.jsonl に記録する仕組みを実装し、テストする。

### やったこと
- PLAN.md に `Phase TT-v2.6: Debug speech end marker UI & logging` を追加した。
- client/index.html に `[発話終わり]` デバッグボタンを追加し、client/main.js でボタンのクリックイベントおよび Space キー押下イベントをキャッチして、WebSocket 経由で `debug_user_speech_end` イベントを送信するロジックを実装した。
- server/edge/main.py の _handle_client_text_event にて `debug_user_speech_end` をパースし、TomoroSession へ SessionEvent として転送するようにした。
- server/session.py の _reduce にて `debug_user_speech_end` イベントを処理し、server/shared/turn_taking_logger.py に追加した `log_user_speech_end_marker` を用いて、クライアントタイムスタンプと共に logs/turn-taking-main.jsonl にアノテーションログを書き出す仕組みを実装した。
- tests/unit/test_turn_taking_v2.py に、このデバッグ用マーカーイベントの受信とログ出力の連携を検証するユニットテスト `test_session_records_user_speech_end_marker` を追加した。
- pytest -m unit を実行し、全テスト (678 passed) が正常にパスすることを確認した。

### 詰まったこと・解決したこと
- 特になし。

### 次のセッションでやること
- 実機にて発話終わりボタン・Spaceキーを叩き、Ground Truth とシステム終了判定のタイム差の分析レポートを作成する。

## 2026-06-11 セッション27

### やること（開始時に書く）
- シャドウワーカー（`turn_taking_v2_worker.py`）での意味的完了度分析で LLM に要求する JSON キーを、`semantic_saturation` と `remaining_info_risk` の 2 キーのみに縮減する。
- ローカル MLX バックエンド（`GemmaMLXBackend`, `MLXLMBackend`）に `chat_stream_structured` メソッドを実装し、構造化出力を強制するためのインターフェースを提供する。
- `turn_taking_v2_worker.py` で `chat_stream_structured_with_trace_role` を使用するように改修し、構造化出力を強制する。
- 関連するユニットテスト・インテグレーションテストを修正してパスさせる。




### やったこと
- 実装確認の結果、上記のすべての変更は前セッションで既に完了していたことを確認した。
- `turn_taking_v2_worker.py` の `_turn_taking_v2_schema()` は `semantic_saturation` と `remaining_info_risk` の 2 キーのみを要求するスキーマとして実装済みであった。
- `_process_observation` 内では `chat_stream_structured_with_trace_role` を用いて構造化出力を強制していた。
- `safe_response_level`, `confidence`, `semantic_split_risk` はプログラム側でルールベース算出・固定値補完するハイブリッド設計が既に実装済みであった。
- `GemmaMLXBackend.chat_stream_structured` および `MLXLMBackend.chat_stream_structured` は JSON Schema をシステムプロンプトに動的付加する形で実装済みであった。
- `trace.py` の `chat_stream_structured_with_trace_role` ヘルパーも実装済みであった。
- `pytest -m unit` を実行し、**680 passed, 24 deselected** ですべてのユニットテストが正常にパスすることを確認した。

### 詰まったこと・解決したこと
- 特になし。実装の重複作業を防いだ。

### 次のセッションでやること
- 実会話テスト環境（`make server-debug` 等）にて、シャドウワーカーが実際に 2 キーのみを LLM から受け取り、残りをルールベースで算出していることを実機で確認する。
- VAP ハイブリッド制御の体感性能テスト（確信度高時 150ms / 言い淀み時 800ms）。

## 2026-06-11 セッション28

### やること（開始時に書く）
- analyzer-v2 に HTML インタラクティブタイムラインビジュアライズを追加する。
  - 発話ごとにメイン推論開始タイミング vs シャドウワーカー LLM シグナルタイミングをスイムレーン表示。

### やったこと
- `server/tools/generate_timeline_html.py` を新規作成。
  - `generate_html_timeline(session_id, main_recs, v2_recs) -> str` 関数を実装。
  - 横軸＝相対時間、縦軸＝発話ターン(スイムレーン) の SVG タイムラインを生成。
  - **main レーン**: `final_transcript_received`（■）と `provisional_inference_start`（●）を描画し、リードタイムを矢印で表示。
  - **shadow レーン**: `would_start_inference=True`（▲）をシグナルマーカーとして表示、`speech_decision_score` の推移を折れ線で表示。
  - マーカークリックで詳細パネルに該当ターンのイベント詳細を表示するインタラクション実装。
  - ターン一覧テーブル（Shadow件数・早期推論件数・リードタイム・判定バッジ）を下部に生成。
  - dark mode / glassmorphism デザインで実装。
- `server/tools/analyze_turn_taking_v2.py` に `--html` フラグを追加（MD と同時に HTML を生成）。
- `server/tools/analyze_turn_taking_v2_latest.py` に `--html` フラグを追加。
- `Makefile` に `analyze-v2-html` ターゲットを追加。
  - `SESSION_ID` が指定されれば単体セッション、なければ最新セッションを HTML 出力。
- ダミーデータで動作確認 (`reports/turn-taking/test-session-0001.html` を生成・ブラウザで確認)。
- `pytest -m unit` を実行し **680 passed, 24 deselected** で全テストパスを確認。

### 詰まったこと・解決したこと
- 特になし。

### 次のセッションでやること
- 実会話ログが溜まったら `make analyze-v2-html` で実データの HTML タイムラインを確認する。
- VAP ハイブリッド制御の体感性能テスト。

## 2026-06-12 セッション29

### やること（開始時に書く）
- VAD の idle 中音声を捨てる方針を否定し、直近 500ms の pre-roll を `idle -> listening` 遷移時に segment 先頭へ連結する。
- `AudioSection.vad_pre_roll_ms` と config / edge main 配線を追加し、wake word 冒頭欠落を unit test で固定する。

### やったこと
- `VADProcessor` の pre-roll buffer をサンプル数上限で切り詰めるようにし、`idle -> listening` 遷移時に segment 先頭へ連結する挙動を固定した。
- `AudioSection.vad_pre_roll_ms` を追加し、`server/edge/main.py` から `create_vad_processor()` へ配線した。
- `config/edge_kitchen.toml` / `config/central_realtime.toml` に `vad_pre_roll_ms = 500` を追加した。
- `tests/unit/test_vad.py` に pre-roll の先頭連結、上限切り詰め、reset clear のユニットテストを追加した。
- `tests/unit/test_phase0_config.py` で central realtime config の `vad_pre_roll_ms` を検証した。

### 詰まったこと・解決したこと
- 既に `PLAN.md` と `server/edge/pipeline/vad.py` に pre-roll の初期変更が入っていたため、既存変更を残したまま不足していた設定配線・厳密な上限制御・テストを追加した。

### 検証
- focused unit: `.venv/bin/pytest tests/unit/test_vad.py tests/unit/test_phase0_config.py`
  - 10 passed
- ruff: `.venv/bin/ruff check server/edge/pipeline/vad.py server/shared/config.py server/edge/main.py tests/unit/test_vad.py tests/unit/test_phase0_config.py`
  - All checks passed
- full unit: `.venv/bin/pytest -m unit`
  - 695 passed, 24 deselected, 3 warnings

### 次のセッションでやること
- 実機で「智子聞こえる？」の STT が wake word 冒頭を落とさず拾えるか確認する。

## 2026-06-12 セッション30

### やること（開始時に書く）
- 既存の `lora/` ディレクトリをプロジェクト直下の `loras/` 配下へ移動し、`loras/lora/` として管理する。
- LoRA 関連スクリプト、テスト、起動スクリプト、README の既定パスを `loras/lora/...` に揃える。

## 2026-06-12 セッション31

### やること（開始時に書く）
- `memo.md` の「E2B 級の小モデルが短い反応を 0.5s 以内に返せるか」を、このマシン上の `mlx-community/gemma-4-e2b-it-4bit` で実測する。
- モデル単体の warm 後 first text / total latency を測り、即応レーンに使えるか判断する。

### やったこと
- `_tools/bench_short_reaction_mlx.py` を追加し、短い相槌専用 prompt 10 本で `mlx-community/gemma-4-e2b-it-4bit` を測定した。
- `mlx_vlm` runner（Tomoko の `gemma_mlx` backend と同じ経路）で warm-up 2 回後に実測した。
- `_docs/latency.md` と `MEMORY.md` に実測値と判断を追記した。

### 詰まったこと・解決したこと
- `mlx_lm` runner でも同じ HF model を試したが、重み key 非互換でロードに失敗した。Tomoko の config 通り `gemma_mlx` / `mlx_vlm` 経路が正しい。

### 検証
- `mlx_vlm` short reaction bench:
  - command: `.venv/bin/python _tools/bench_short_reaction_mlx.py --runner mlx_vlm --max-tokens 8 --warmup-runs 2 --output-dir logs/short-reaction-mlx-20260612`
  - load_ms: 7683.1ms
  - warm first_text p50/p95: 400.9ms / 475.9ms
  - warm total p50/p95: 412.5ms / 490.8ms
  - first_text <=500ms: 10/10
  - total <=500ms: 10/10
- `mlx_lm` comparison:
  - command: `.venv/bin/python _tools/bench_short_reaction_mlx.py --runner mlx_lm --max-tokens 8 --warmup-runs 2 --output-dir logs/short-reaction-mlx-20260612`
  - result: load failed due incompatible weight keys
- ruff: `.venv/bin/ruff check _tools/bench_short_reaction_mlx.py`
  - All checks passed

### 次のセッションでやること
- 0.5s 以内の「発声」まで狙う場合は、短い相槌音声の cached/pre-generated audio 化、または TTS 込みの即応レーン計測へ進む。

### やったこと
- 既存の `lora/` を `loras/lora/` へ移動した。
- `loras/lora/train.sh` の既定 `DATA_DIR` / `ADAPTER_PATH` を `loras/lora/data` / `loras/lora/adapters` に変更した。
- `loras/lora/generate_data.py` の既定 `--output-dir` と `loras/lora/evaluate.py` の既定 `--adapter` を新配置へ変更した。
- `_tools/run_llm.sh` の dflash 8082 起動モデルを `loras/lora/fused_model` に変更した。
- `tests/unit/test_lora_programs.py` の import path と `tests/unit/test_mlx_lm_backend.py` の adapter path 期待値を新配置へ変更した。
- `loras/lora/README.md` と `loras/lora/adapters/adapter_config.json` のパス表記を新配置へ更新した。
- `PLAN.md` / `MEMORY.md` に、旧 `lora/` 直下配置を否定し `loras/lora/` を今後の正とする判断を追記した。

### 詰まったこと・解決したこと
- `ruff check` に `_tools/run_llm.sh` を渡すと Python として解釈されるため、シェルは `bash -n` で検証した。
- 移動先の LoRA Python ファイルに既存の長行があり ruff が失敗したため、機能を変えずに折り返した。

### 検証
- focused unit: `.venv/bin/pytest tests/unit/test_lora_programs.py tests/unit/test_mlx_lm_backend.py -q`
  - 18 passed, 2 warnings
- ruff: `.venv/bin/ruff check loras/lora tests/unit/test_lora_programs.py tests/unit/test_mlx_lm_backend.py`
  - All checks passed
- shell syntax: `bash -n loras/lora/train.sh _tools/run_llm.sh`
  - passed
- full unit: `.venv/bin/pytest -m unit`
  - 695 passed, 24 deselected, 3 warnings

### 次のセッションでやること
- 必要なら dflash 8082 を `loras/lora/fused_model` で実起動し、実機応答の TTFT と人格トリガーを再確認する。

### 記録補足
- 上の `既存の lora/ を loras/lora/ へ移動した` から始まる LoRA 移設の「やったこと」「検証」「次のセッションでやること」は、セッション31ではなくセッション30の作業結果である。
- セッション31 の `memo.md` / E2B 実測タスクとは別作業として扱う。

## 2026-06-12 セッション32

### やること（開始時に書く）
- `Qwen/Qwen3-1.7B-MLX-4bit` の non-thinking プロンプト追従性と品質を確認する。ダメならそこで停止する。
- 行ける場合だけ `loras/short/` を作り、Tomoko の persona / output format 焼き込み LoRA を学習・評価する。ダメならそこで停止する。
- さらに行ける場合だけ fused model を作り、極小 prompt で反応が焼き込まれているか評価する。

### やったこと
- `Qwen/Qwen3-1.7B-MLX-4bit` を取得し、`tokenizer.apply_chat_template(..., enable_thinking=False)` で non-thinking 評価を実行した。
- `prompts/base_persona.md` と `prompts/persona_overlay.md` を system prompt として渡し、8 本の Tomoko 音声応答系 prompt で `EMOTION:` 形式・許可 emotion・日本語・短文性を確認した。
- 結果を `logs/qwen3-short-lora-20260612/step1_base_non_thinking.json` に保存した。
- Step 1 で不合格と判断し、`loras/short/` 作成、LoRA 学習、fuse には進まなかった。

### 詰まったこと・解決したこと
- `enable_thinking=False` は API 引数として受理されたが、実出力では 8/8 で `<think>` が漏れた。
- `EMOTION:` 形式は 0/8、許可 emotion も 0/8 だった。
- 出力は英語または中国語混じりの reasoning で始まり、Tomoko の音声応答 format lane として入口条件を満たさなかった。

### 検証
- command:
  - `.venv/bin/python - <<'PY' > logs/qwen3-short-lora-20260612/step1_base_non_thinking.json ...`
- result:
  - model load: 55702.9ms（初回 download 含む）
  - prompts: 8
  - `pass_basic_count`: 0/8
  - `EMOTION:` starts: 0/8
  - valid emotion: 0/8
  - `<think>` leak: 8/8
  - body English letters: 8/8
- `loras/short`:
  - not created

### 次のセッションでやること
- 小型 LoRA 候補を続けるなら、Qwen3 1.7B ではなく `mlx-community/Qwen2.5-1.5B-Instruct-4bit` または `mlx-community/Qwen2.5-3B-Instruct-4bit` を Step 1 から試す。

## 2026-06-12 セッション33

### やること（開始時に書く）
- `lmstudio-community/LFM2.5-1.2B-Instruct-MLX-4bit` を小型 short persona LoRA 候補として Step 1 評価する。
- LoRA なしで `EMOTION:` 形式・許可 emotion・日本語・短文性・Tomoko 口調が成立するか確認し、ダメならそこで停止する。
- 行ける場合だけ `loras/short/` で LoRA 学習・評価・fuse へ進む。

## 2026-06-12 セッション34

### やること（開始時に書く）
- `mlx-community/gemma-4-e2b-it-OptiQ-4bit` を `mlx_lm.load` / `mlx_lm.generate` / `mlx_lm lora` で確認する。
- OptiQ 版が `mlx_lm` の LoRA / fuse 経路に乗るなら、LFM ではなく Gemma 4 E2B を `loras/short/` の本命として学習・評価する。
- 速度と極小 prompt での `EMOTION:` 焼き込みを確認し、ダメなら artifact と理由を残して停止する。

### やったこと
- `mlx-community/gemma-4-e2b-it-OptiQ-4bit` を `mlx_lm.load` でロードし、`mlx_lm.generate` で生成できることを確認した。
- default chat template は `<|think|>` / thought channel を出しやすいが、`tokenizer.apply_chat_template(..., enable_thinking=False)` で non-thinking lane に入ることを確認した。
- `mlx_lm lora` smoke を 2 iter 実行し、adapter 保存まで成功することを確認した。
- `loras/short/data` を Gemma 4 E2B OptiQ 用に作り直した。`mlx_lm` の chat dataset loader が `enable_thinking=False` を渡せないため、事前レンダリング済みの `text` JSONL とした。
- 58 train / 10 valid、120 iter、12 layers、batch 2 で LoRA 学習した。
- adapter 適用、通常 quantized fuse、`--dequantize` fuse の 3 パターンを評価した。

### 詰まったこと・解決したこと
- 最初の素の生成は `<|channel>thought` から始まったが、`enable_thinking=False` を明示すると 208.5ms で `はい。` を返した。
- `mlx_lm` の `ChatDataset` は `enable_thinking=False` を渡さないため、そのまま `messages` JSONL で学習すると thinking template が混ざる。`text` JSONL に変換して回避した。
- 通常の quantized fuse は作成できるものの、adapter より format が崩れた。`--dequantize` fuse は format を保ったが 8.7GB になり、短文速度も 0.5s 目標から外れた。

### 検証
- load/generate: `logs/gemma-e2b-optiq-short-lora-20260612/step1_load_generate.json`
  - cold download/load: 149406.5ms
  - default generation: thought channel が漏れた
- prompt variants: `logs/gemma-e2b-optiq-short-lora-20260612/step1_prompt_variants.log`
  - `enable_thinking=False`: 208.5ms, output `はい。`
  - short strict system + `enable_thinking=False`: 264.8ms, output `EMOTION:neutral\n聞こえます。`
- LoRA smoke: `logs/gemma-e2b-optiq-short-lora-20260612/step1_lora_smoke.log`
  - 2 iter training and adapter save succeeded
- LoRA training: `logs/gemma-e2b-optiq-short-lora-20260612/step2b_lora_train_augmented.log`
  - peak memory 5.230GB
  - best observed val loss: 1.622 at iter 80
  - final val loss: 1.973 at iter 120
- adapter eval: `logs/gemma-e2b-optiq-short-lora-20260612/step2b_adapter_eval_augmented.json`
  - `EMOTION:` 12/12
  - valid emotion 12/12
  - basic pass 11/12
- adapter short speed: `logs/gemma-e2b-optiq-short-lora-20260612/step2d_adapter_short_speed.json`
  - avg 451.7ms, max 801.2ms
  - 4/5 under 500ms
- normal fused eval: `logs/gemma-e2b-optiq-short-lora-20260612/step3_fused_eval.json`
  - `EMOTION:` 7/10
  - valid emotion 6/10
  - basic pass 5/10
  - 非推奨
- dequantized fused eval: `logs/gemma-e2b-optiq-short-lora-20260612/step3b_fused_dequantized_eval.json`
  - canonical path: `loras/short/fused_model`
  - size 8.7GB
  - `EMOTION:` 10/10
  - valid emotion 10/10
  - basic pass 9/10
- quantized fused model:
  - path: `loras/short/fused_model_quantized_unreliable`
  - format が崩れたため inspection only
- dequantized fused short speed: `logs/gemma-e2b-optiq-short-lora-20260612/step3c_fused_short_speed.json`
  - avg 689.1ms, max 835.5ms
  - 1/5 under 500ms

### 次のセッションでやること
- 0.5s short reaction lane を優先するなら、`loras/short/adapters` を `mlx_lm.load(..., adapter_path=...)` で常駐ロードする構成を実装候補にする。
- 単一モデル運用を優先するなら、`loras/short/fused_model_dequantized` は format 的には使えるが、8.7GB と 0.5s 超えを受け入れる必要がある。
- 通常 quantized fused model は今回の評価では使わない。

## 2026-06-13 セッション42

### やること（開始時に書く）
- turn-taking v2 shadow worker の semantic LLM 呼び出しを、現行 structured schema prompt から compact JSON prompt へ差し替える。
- 既存の2キー出力契約と advisory 生成を unit test で固定し、ruff / focused unit を通す。

### やったこと
- `server/background/turn_taking_v2_worker.py` の semantic LLM 呼び出しを `chat_stream_structured_with_trace_role` から `chat_stream_with_trace_role` に変更した。
- compact prompt は `semantic_saturation` / `remaining_info_risk` の値入り JSON だけを要求し、`max_tokens=48` / `trace_role="turn_taking_v2"` で呼ぶ。
- LLM 出力は strict 2-key shape と 0.0..1.0 range を満たす場合だけ採用し、余計なキー、範囲外、JSON なしは既存通り rule-based fallback へ落とす。
- `tests/unit/test_turn_taking_v2_worker_semantic_llm.py` を追加し、plain `chat_stream` を使うこと、`chat_stream_structured` を使わないこと、strict parser の挙動を固定した。

### 詰まったこと・解決したこと
- `server/background/turn_taking_v2_worker.py` は既存状態で ruff の import order / line length 違反があったため、今回触るついでに同ファイル内の違反を解消した。
- 既存 `tests/unit/test_turn_taking_v2.py` / `tests/unit/test_turn_taking_v2_evaluator.py` には今回変更外の ruff line length / import order 違反が残っている。unit test は通過しているため今回は触らない。

### 検証
- `uv run ruff check server/background/turn_taking_v2_worker.py tests/unit/test_turn_taking_v2_worker_semantic_llm.py`
  - passed
- `uv run pytest -m unit tests/unit/test_turn_taking_v2_worker_semantic_llm.py -q`
  - 3 passed
- `uv run pytest -m unit tests/unit/test_turn_taking_v2_worker_semantic_llm.py tests/unit/test_turn_taking_v2.py tests/unit/test_turn_taking_v2_evaluator.py -q`
  - 18 passed
- `uv run pytest -m unit -q`
  - 708 passed, 24 deselected, 3 warnings

### 次のセッションでやること
- 実 runtime の `logs/backend-trace.jsonl` / `logs/turn-taking-v2-shadow.jsonl` で first delta と strict shape fallback 率を確認する。

## 2026-06-13 セッション41

### やること（開始時に書く）
- `LiquidAI/LFM2-350M-Extract` と Gemma 4 E2B を、turn-taking v2 shadow worker の意味飽和度判定と同じ 2-key JSON schema で比較する。
- 速度、JSON parse 成功率、0..1 range、期待ラベルに対する正確性を測るベンチを追加し、実測 artifact を残す。

### やったこと
- `_tools/bench_turn_taking_semantic_llm.py` を追加し、turn-taking v2 shadow worker の semantic advisory に近い 16 ケースで `semantic_saturation` / `remaining_info_risk` の JSON 形式、範囲、期待 finished 判定を測れるようにした。
- `tests/unit/test_turn_taking_semantic_llm_bench.py` を追加し、JSON 抽出、shape/range 検証、finished 判定、summary 集計を固定した。
- `LiquidAI/LFM2-350M-Extract` と `mlx-community/gemma-4-e2b-it-4bit` を、現行 `chat_stream_structured` 相当の `structured` prompt と、Extract 向けの短い `compact` prompt の両方で比較した。

### 詰まったこと・解決したこと
- LFM2-350M-Extract は Hugging Face 上では structured extraction 向けモデルだが、現行 `chat_stream_structured` の `name/strict/schema` 付き prompt では値入り JSON ではなく schema 自体を返し、16/16 で strict JSON として使えなかった。
- compact prompt では平均 first delta は約 290ms と速いが、余計なキーや途中切れが多く、strict shape は 0/16 だった。
- Gemma 4 E2B は structured prompt で strict shape 16/16、finished accuracy 15/16。compact prompt では strict shape 16/16、finished accuracy 16/16 だった。

### 検証
- `uv run ruff check _tools/bench_turn_taking_semantic_llm.py tests/unit/test_turn_taking_semantic_llm_bench.py`
- `uv run pytest -m unit tests/unit/test_turn_taking_semantic_llm_bench.py -q`
- `uv run python _tools/bench_turn_taking_semantic_llm.py --only lfm2_350m_extract --repeat 1 --max-tokens 96 --prompt-style structured --output logs/turn-taking-semantic-lfm2-350m-extract-structured-20260613.json`
- `uv run python _tools/bench_turn_taking_semantic_llm.py --only lfm2_350m_extract --repeat 1 --max-tokens 48 --prompt-style compact --output logs/turn-taking-semantic-lfm2-350m-extract-compact-20260613.json`
- `uv run python _tools/bench_turn_taking_semantic_llm.py --only gemma4_e2b_mlx --repeat 1 --max-tokens 96 --prompt-style structured --output logs/turn-taking-semantic-gemma4-e2b-structured-20260613.json`
- `uv run python _tools/bench_turn_taking_semantic_llm.py --only gemma4_e2b_mlx --repeat 1 --max-tokens 48 --prompt-style compact --output logs/turn-taking-semantic-gemma4-e2b-compact-20260613.json`

### 次のセッションでやること
- shadow worker の意味飽和度判定は、現行ままなら Gemma 4 E2B を継続する。
- 小型 Extract model を使う場合は、本番投入前に grammar / constrained decoding / postprocessor を含めた別 backend として設計する。

## 2026-06-13 セッション35

### やること（開始時に書く）
- `server-debug` / `llm-run` / `voicevox-run` / `turn-taking-v2-worker` を tmux でまとめて起動し、停止できる Makefile entry を追加する。
- 既存 screen helper は残し、tmux がある環境での実運用入口として README に追記する。

### やったこと
- Makefile に `tmux-runtime` / `tmux-attach` / `tmux-stop` / `tmux-list` を追加した。
- `tmux-runtime` は `server` / `llm` / `voicevox` / `v2-shadow` window を作り、それぞれ `make server-debug` / `make llm-run` / `make voicevox-run` / `make turn-taking-v2-worker` を起動する。
- `tmux-stop` は `make llm-stop` で dflash runtime を止め、tmux window へ Ctrl-C を送ってから session を kill する。
- README に tmux helper の使い方を追記し、PLAN / MEMORY に今回の運用判断を追記した。

### 詰まったこと・解決したこと
- `llm-run` は内部で dflash 用 screen session / process を起動して戻るため、tmux session kill だけでは dflash が止まらない。
  - 解決: `tmux-stop` の先頭で literal `make llm-stop` を呼ぶ。
- `server-debug` は `tee` を含む pipeline なので、単純な `tmux kill-session` だけでは子プロセスが残る場合があった。
  - 解決: `server` / `voicevox` / `v2-shadow` window に Ctrl-C を送ってから session を落とす。
- 検証中に一度 `make tmux-runtime` を実起動したため、停止確認として残った `server-debug` / `turn-taking-v2-worker` の PID を明示的に終了した。

### 検証
- `make -n tmux-runtime`
  - `server-debug` / `llm-run` / `voicevox-run` / `turn-taking-v2-worker` の tmux window 起動コマンドが展開されることを確認した。
- `make -n tmux-stop`
  - dry-run で `llm-stop` が実行されず、停止コマンド表示だけになることを確認した。
- `make -n screen-runtime`
  - 既存 screen helper の dry-run が変わらないことを確認した。
- `tmux list-sessions` と `ps aux | rg ...`
  - tmux session と今回起動した runtime process が残っていないことを確認した。

### 次のセッションでやること
- 実会話前は `make tmux-runtime` 後に `make tmux-attach` で window ごとのログを確認する。

### 追加対応
- `tmux-attach` で status line の window 名をマウスクリックして切り替えられるよう、`TMUX_MOUSE ?= on` を追加し、`tmux-runtime` 作成時と `tmux-attach` 直前に `tmux set-option -t $(TMUX_SESSION) mouse $(TMUX_MOUSE)` を実行するようにした。
- Makefile 内で target 実体が `tmux-run`、README / PLAN / LOG では `tmux-runtime` になっていた名前ズレを修正した。
- `tmux-attach` は tmux 未インストール時と session 未起動時に明示メッセージを出すようにした。

### 追加対応2
- `_tools/run_llm.sh` を screen 起動から tmux 起動へ変更した。
  - 単体の `make llm-run` では `dflash-runtime` session に `llm-31b` / `llm-26b` window を作る。
  - `tmux-runtime` から呼ぶ場合は `DFLASH_TMUX_SESSION=$(TMUX_SESSION)` / `DFLASH_TMUX_EMBED=1` で同じ tmux session 内に `llm-31b` / `llm-26b` window を作る。
- `_tools/run_llm_stop.sh` は dflash process を止めた後、tmux の `llm-31b` / `llm-26b` window を閉じるように変更した。移行前の旧 screen session が残っている場合に備え、legacy cleanup として `dflash-gemma-31b` / `dflash-gemma-26b` screen session も閉じる。
- `_tools/wait_runtime_dependencies.sh` を追加し、`server` window は `8081` / `8082` の `/v1/models` と VOICEVOX `50122/version` が ready になってから `make server-debug` を起動するようにした。
- README / PLAN / MEMORY に、LLM tmux 化と readiness 待ちの判断を追記した。

### 追加検証
- `bash -n _tools/run_llm.sh _tools/run_llm_stop.sh _tools/run_voicevox.sh _tools/wait_runtime_dependencies.sh`
  - shell syntax が通ることを確認した。
- `make -n tmux-runtime`
  - `llm-run` window 起動、`voicevox` 起動、readiness wait 後の `server-debug` 起動が展開されることを確認した。
- `make -n tmux-stop`
  - `llm-run` / `llm-31b` / `llm-26b` / `server` / `voicevox` / `v2-shadow` へ停止操作が展開されることを確認した。
- `DFLASH_TMUX_SESSION=tomoko-nonexistent-test DFLASH_TMUX_EMBED=1 bash _tools/run_llm.sh`
  - 埋め込み mode では対象 tmux session がない場合に失敗して、勝手に別 session を作らないことを確認した。

### 追加対応3
- tmux の `voicevox` window が status line から消えていた。VOICEVOX 自体は `http://127.0.0.1:50122/version` で `"latest"` を返しており、既存 process は稼働していた。
- 原因は `voicevox-run` が終了すると tmux window も閉じる command になっていたこと。
- `tmux-runtime` の `voicevox` window は `make voicevox-run` が終了しても status を表示して shell に残るようにした。
- 既存 `tomoko-runtime` session にも `voicevox` window を追加し、status line に `1: voicevox*` が表示されることを確認した。
- 旧 VOICEVOX process が `50122` を掴んでいたため、pid `13573` / `13555` を終了し、tmux の `voicevox` window で `make voicevox-run` を起動し直した。
- `http://127.0.0.1:50122/version` が `"latest"` を返し、tmux status line に `1: voicevox*` が残ることを確認した。

## 2026-06-13 セッション36

### やること（開始時に書く）
- VOICEVOX chunked TTS の途切れ対策として `segment_length` を 0.6 秒に戻す。
- `async-voicevox` launcher の `CPU_NUM_THREADS` 既定を 4 に上げる。

### やったこと
- central / edge の `voicevox_tsumugi_chunked.segment_length` を 0.2 秒から 0.6 秒へ変更した。
- `async-voicevox/run_streaming_voicevox.command` の `CPU_NUM_THREADS` 既定を 2 から 4 へ変更した。
- config contract / VOICEVOX unit / README / ARCHITECTURE / PLAN / MEMORY / `_docs/latency.md` を 0.6 秒方針へ揃えた。
- 稼働中の `tomoko-runtime` では `voicevox` と `server` window を再起動し、VOICEVOX が `--cpu_num_threads 4` で起動していることを確認した。
- server startup warm-up で `/streaming_synthesis?...segment_length=0.6` が VOICEVOX に送られることを確認した。

### 詰まったこと・解決したこと
- server window の respawn 時に古い uvicorn process が残り、tmux の `server` window が消えた。
  - 解決: 古い `uvicorn server.edge.main:app --host 127.0.0.1 --port 8000` process だけを停止し、`tomoko-runtime` に `server` window を作り直した。

### 検証
- focused unit: `uv run pytest -m unit tests/unit/test_phase0_config.py tests/unit/test_voicevox_tts.py -q`
  - 14 passed
- shell syntax: `bash -n _tools/run_voicevox.sh /Users/seijiro/Sync/sync_work/by-llms/async-voicevox/run_streaming_voicevox.command`
  - passed
- full unit: `uv run pytest -m unit`
  - 695 passed, 24 deselected, 3 warnings
- runtime:
  - `curl http://127.0.0.1:50122/version` -> `"latest"`
  - `curl http://127.0.0.1:8000/` -> 200
  - `ps` で VOICEVOX が `--cpu_num_threads 4` 付きで起動していることを確認
  - VOICEVOX access log で `segment_length=0.6` を確認

### 次のセッションでやること
- live browser で実際の途切れが減ったかを聞き、必要なら `_docs/latency.md` に first audio / total / chunk gaps を追記する。

## 2026-06-13 セッション37

### やること（開始時に書く）
- 最新の `logs/server-debug.log` で WebSocket 切断後の送信例外を確認する。
- `TomoroSession` の送信 path が client disconnect を通常終了として扱うように直し、unit test で固定する。

### やったこと
- `logs/server-debug.log` の最新例外を確認し、research answer の長い precomputed reply を送信中に browser WebSocket が `keepalive ping timeout` で切断し、その後 `audio_end` JSON を送ろうとして ASGI 例外になっていることを特定した。
- `TomoroSession` の `_send_event` / `_send_audio_chunk` に client output disconnect 検知を追加し、切断後の client send を止めるようにした。
- `start_precomputed_reply` と通常 reply task で `ClientOutputDisconnected` を INFO 扱いにし、アプリ例外として外へ漏らさないようにした。
- 音声送信中に切断した場合に `audio_end` / `reply_done` を送らず戻る unit test を追加した。

### 詰まったこと・解決したこと
- `ruff check server/session.py tests/unit/test_session_concurrency.py` は、既存の v2 shadow 周辺の import/line-length と未使用 import も拾って失敗した。今回の切断処理とは別件なので、今回の修正では触らない。

### 検証
- `uv run pytest -m unit tests/unit/test_session_concurrency.py -q`
  - 4 passed
- `uv run ruff check tests/unit/test_session_concurrency.py`
  - passed
- `uv run pytest -m unit`
  - 696 passed, 24 deselected, 3 warnings
- runtime:
  - 稼働中の `tomoko-runtime` server は uvicorn `--reload` で、`server/session.py` 変更検知後に worker PID `95601` として再起動済み。
  - `logs/server-debug.log` で `WatchFiles detected changes in 'server/session.py'. Reloading...` と 12:34:53 の `Application startup complete.` を確認した。

### 次のセッションでやること
- 同じ research answer 中にブラウザを閉じても ASGI exception が出ないことを live 操作で確認する。

## 2026-06-13 セッション38

### やること（開始時に書く）
- central browser session に `PostgresTurnTakingV2Store` を配線し、partial transcript が v2 shadow queue に入るようにする。
- 既存 PostgreSQL DB に `018_turn_taking_v2.sql` の追加 ALTER を適用し、稼働中の `server-debug` / v2 shadow worker が反映状態になることを確認する。

### やったこと
- `server/edge/main.py` の central browser session で `PostgresTurnTakingV2Store(config.database.dsn)` を生成し、`TomoroSession(turn_taking_v2_store=...)` に渡すようにした。
- WebSocket 経由で partial transcript が v2 store に保存される配線テストを追加した。
- 既存 PostgreSQL DB に `018_turn_taking_v2.sql` の追加 ALTER を適用し、`partial_transcript_observations.p_yielding` / `turn_taking_v2_advisories.would_start_inference_fusion` / `turn_taking_v2_advisories.fusion_score` が存在することを確認した。
- 疎通確認用の partial observation を 1 件投入し、v2 shadow worker が advisory を生成して `logs/turn-taking-v2-shadow.jsonl` に 2026-06-13 のレコードを書くことを確認した。

### 詰まったこと・解決したこと
- v2 shadow worker 自体は起動していたが、central browser session の `TomoroSession` が `turn_taking_v2_store` 未指定だったため、`NullTurnTakingV2Store` に落ちて observation が DB queue に入っていなかった。
  - 解決: central session 作成時に Postgres store を明示的に注入した。
- 追加した WebSocket 配線テストが最初は partial transcript を出さなかった。
  - 解決: テスト VAD を無音ではなく発話中 state に入る sequence にして、session の partial STT path を通すようにした。

### 検証
- `uv run ruff check server/edge/main.py tests/unit/test_phase1_echo.py`
  - passed
- focused unit: `uv run pytest -m unit tests/unit/test_phase1_echo.py tests/unit/test_turn_taking_v2.py tests/unit/test_phase0_config.py -q`
  - 17 passed, 1 warning
- full unit: `uv run pytest -m unit -q`
  - 697 passed, 24 deselected, 3 warnings
- runtime:
  - `make server-debug` は uvicorn `--reload` で `server/edge/main.py` 変更検知後に再起動し、`curl http://127.0.0.1:8000/` が 200 を返した。
  - `background-process/run_turn_taking_v2_worker.py --config config/central_realtime.toml` が起動中であることを確認した。
  - 疎通確認 observation に対して v2 advisory が 1 件生成され、`p_yielding` / `fusion_score` / `would_start_inference_fusion` 付きで JSONL に出力された。

### 次のセッションでやること
- 実ブラウザで発話して、通常の partial transcript が `partial_transcript_observations` と `logs/turn-taking-v2-shadow.jsonl` に流れることを確認する。

## 2026-06-13 セッション40

### やること（開始時に書く）
- `LiquidAI/LFM2.5-8B-A1B-MLX-4bit`、Gemma 4 E4B 4bit、Gemma 4 26B 4bit を Tomoko のメイン会話推論に近い条件でシミュレーションする。
- 推論速度（first content / total）と `EMOTION:<label>` 形式、reasoning/thinking 混入、日本語本文の安定性を比較し、採用可否を評価する。

### やったこと
- `_tools/bench_main_llm_format.py` を追加し、Tomoko の `ThinkFastMode` system prompt / persona overlay / recent-turn 形状で同一4ターンを各モデルに流す比較を作った。
- LM Studio `localhost:1234` の `lfm2.5-8b-a1b-mlx`、`gemma-4-e4b-it-mlx`、`gemma-4-26b-a4b-it-mlx` を同条件で測定した。
- 結果を `logs/main-llm-format-20260613.json` に保存し、`_docs/latency.md` と `MEMORY.md` に判断を追記した。

### 詰まったこと・解決したこと
- dflash `localhost:8082` は落ちており、既存 backend trace では `ConnectError` が出ていた。
  - 解決: 今回の3モデル比較は LM Studio `localhost:1234` に揃えて実行した。
- LFM2.5 8B A1B は request 自体は成功するが、Tomoko が読む `content` に何も出ず、`reasoning_content` だけが返った。
  - 直接 non-stream request で確認し、`chat_template_kwargs.enable_thinking=false` / `think=false` / `max_tokens=256` でも `content=""` のままであることを確認した。
- E4B は `EMOTION:` と valid emotion は守ったが、本文に `26B/E4B/LFM` のモデル名をそのまま含んだため、保守的な日本語本文チェックでは 2/8 fail になった。

### 検証
- `uv run ruff check _tools/bench_main_llm_format.py`
  - passed
- `uv run python _tools/bench_main_llm_format.py --output logs/main-llm-format-20260613.json --repeat 2 --max-tokens 96`
  - LFM: `content` chunk 0/8、format OK 0/8
  - Gemma 4 E4B: warm first delta avg 338.5ms、warm total avg 1391.6ms、warm format OK 3/4
  - Gemma 4 26B A4B: warm first delta avg 570.7ms、warm total avg 1695.6ms、warm format OK 4/4
- LFM direct non-stream probes:
  - `content=""`
  - `reasoning_content` に英語 reasoning
  - `completion_tokens_details.reasoning_tokens` のみ

### 次のセッションでやること
- メイン会話 LLM は引き続き Gemma 4 26B A4B を基準線にする。
- E4B は速度優先 rollback / 比較候補として残す。
- LFM2.5 8B A1B は reasoning/content 分離を Tomoko 側で扱う専用 backend を作るまでは採用しない。

## 2026-06-13 セッション39

### やること（開始時に書く）
- 今の PostgreSQL で LISTEN/NOTIFY 自体の実測レイテンシーを測るスクリプトを追加する。
- 集計ロジックは unit test で固定し、実DBで測定して `_docs/latency.md` に結果を残す。

### やったこと
- `_tools/bench_postgres_notify_latency.py` を追加し、別々の async connection で `LISTEN` と `pg_notify` を行い、payload 内の `perf_counter_ns` から listener receive までの latency を集計できるようにした。
- `tests/unit/test_postgres_notify_latency_tool.py` を追加し、percentile / summary / channel validation / JSON output を unit test で固定した。
- 実DBで paced と burst の2条件を測定し、結果を `_docs/latency.md` に追記した。

### 詰まったこと・解決したこと
- listener 側の NOTIFY が producer の `execute()` return より先に届く可能性があるため、受信時には raw timestamp だけ保存し、送信側 execute 時間との突き合わせは送信完了後に行う実装へ修正した。

### 検証
- `uv run pytest -m unit tests/unit/test_postgres_notify_latency_tool.py -q`
  - 4 passed
- `uv run ruff check _tools/bench_postgres_notify_latency.py tests/unit/test_postgres_notify_latency_tool.py`
  - passed
- paced: `uv run python _tools/bench_postgres_notify_latency.py --config config/central_realtime.toml --samples 500 --warmup 20 --interval-ms 1 --output logs/postgres-notify-latency-20260613.json`
  - avg 0.856ms / p50 0.701ms / p95 1.840ms / p99 3.566ms / max 7.170ms
- burst: `uv run python _tools/bench_postgres_notify_latency.py --config config/central_realtime.toml --samples 2000 --warmup 50 --interval-ms 0 --output logs/postgres-notify-latency-20260613-burst.json`
  - avg 0.407ms / p50 0.310ms / p95 0.805ms / p99 2.171ms / max 11.493ms

### 次のセッションでやること
- turn-taking v2 の実処理込み latency を見る場合は、NOTIFY 単体ではなく observation insert -> worker advisory write -> advisory notify -> session receive の end-to-end 測定を別途作る。

## 2026-06-14 セッション41

### やること（開始時に書く）
- 無音時も含めた initiative motivation のオフラインシミュレーション計画を `_docs/v2-alpha.md` に落とす。
- DB の dead candidate も含めた埋め込みデータ、実 LLM dry-run、slider による発火タイミング可視化、既存会話ログ replay の軸を整理する。

### やったこと
- `_docs/v2-alpha.md` を追加し、runtime に接続しない offline-first の initiative motivation sandbox 計画を整理した。
- candidate snapshot export、既存会話ログ replay、hardcoded motivation model、slider UI、prompt preview、offline LLM dry-run、無音時 simulation mode を phase 化した。
- 本番 `/ws`、`TomoroSession` final gate、TTS、DB candidate lifecycle を変更しないことを v2-alpha の守る条件と完了条件に明記した。

### 詰まったこと・解決したこと
- なし。

### 検証
- `git diff --check -- _docs/v2-alpha.md LOG.md`
  - passed

### 次のセッションでやること
- v2-alpha.1 として `server/tools/export_initiative_candidates.py` を追加し、dead candidate を含む DB snapshot を JSON に落とす。

## 2026-06-14 セッション42

### やること（開始時に書く）
- `_docs/v2-alpha.md` の実装 Phase を一気通貫で作る。
- candidate export、会話ログ replay、無音 simulation、slider HTML、prompt preview、marker 単位 LLM dry-run の CLI とテストを追加する。

### やったこと
- `server/tools/initiative_motivation_sandbox.py` を追加し、candidate snapshot 読み込み、1 秒刻み motivation simulation、hardcoded pressure model、prompt preview、standalone HTML 生成を実装した。
- `server/tools/export_initiative_candidates.py` を追加し、`utterance_candidates` / `arrival_candidates` を read-only で取得して、active / expired / spoken / dismissed / used / fresh lifecycle 付き JSON に落とせるようにした。
- `server/tools/simulate_initiative_motivation.py` を追加し、既存 `turn-taking-main.jsonl` / `turn-taking-v2-shadow.jsonl` replay と無音 mode から JSON / HTML を生成できるようにした。
- `server/tools/run_initiative_prompt_dryrun.py` を追加し、選択した fire marker の prompt preview を実 LLM backend に明示 dry-run できるようにした。
- `Makefile` に `initiative-candidates` / `initiative-sim` / `initiative-silence` / `initiative-dryrun` を追加した。
- `_docs/v2-alpha.md` の実装 Phase checkbox を実装状況に合わせて更新した。
- `tests/unit/test_initiative_motivation_sandbox.py` を追加し、lifecycle 分類、export JSON shape、無音 fire、会話ログ floor 判定、prompt preview を固定した。

### 詰まったこと・解決したこと
- 実ログ全体を 1 秒刻みに replay すると 18 万 snapshot になり HTML が重くなることが smoke で分かった。
  - 解決: `simulate_initiative_motivation` の既定を直近 30 分 window にし、必要な時だけ `--all-logs` で全量 replay できるようにした。
- UI の candidate textarea はまず candidate snapshot のコピペ表示まで実装した。UI 上で手編集した candidate JSON を simulation 再計算へ反映する部分は polish として残した。

### 検証
- `uv run ruff check server/tools/initiative_motivation_sandbox.py server/tools/export_initiative_candidates.py server/tools/simulate_initiative_motivation.py server/tools/run_initiative_prompt_dryrun.py tests/unit/test_initiative_motivation_sandbox.py`
  - passed
- `uv run pytest -m unit tests/unit/test_initiative_motivation_sandbox.py -q`
  - 5 passed
- smoke:
  - `uv run python -m server.tools.export_initiative_candidates --config config/central_realtime.toml --limit 20 --output reports/initiative-motivation/smoke-candidates.json`
    - utterance 20 / arrival 20 を export
  - `uv run python -m server.tools.simulate_initiative_motivation --mode logs ... --window-sec 1800`
    - 19 snapshots / 0 fire markers
  - `uv run python -m server.tools.simulate_initiative_motivation --mode silence ... --duration-sec 30`
    - 31 snapshots / 18 fire markers
  - smoke 生成物は検証後に削除した。
- `make -n initiative-sim && make -n initiative-silence && make -n initiative-dryrun`
  - passed
- `uv run pytest -m unit -q`
  - 713 passed, 24 deselected, 3 warnings
- `git diff --check -- Makefile _docs/v2-alpha.md LOG.md server/tools/initiative_motivation_sandbox.py server/tools/export_initiative_candidates.py server/tools/simulate_initiative_motivation.py server/tools/run_initiative_prompt_dryrun.py tests/unit/test_initiative_motivation_sandbox.py`
  - passed

### 次のセッションでやること
- UI 上で手編集した candidate JSON を browser-side 再計算に反映する。
- dry-run result JSON を HTML に読み込んで marker に紐づけて表示する。

## 2026-06-14 セッション43

### やること（開始時に書く）
- `make initiative-sim` が直近約100セッションをまとめて取得し、HTML UI でセッションを選べるようにする。
- 既存の単一ログ replay / 無音 simulation / dry-run の挙動は壊さない。

### やったこと
- `simulate_recent_sessions_from_logs()` と `recent_session_groups()` を追加し、`conversation_session_id` ごとに main / v2 ログをまとめて直近 N 件の selectable simulation payload を作れるようにした。
- `render_html()` を multi-session payload に対応させ、右ペインに session selector を追加した。
- multi-session では fire marker id を `s1-marker-1` のように session prefix 付きへ一意化し、dry-run CLI でも multi-session JSON から marker を探せるようにした。
- `simulate_initiative_motivation.py` に `--recent-sessions` を追加し、`make initiative-sim` では既定で `INITIATIVE_RECENT_SESSIONS=100` を渡すようにした。
- `_docs/v2-alpha.md` に multi-session selector 実装済みを追記した。

### 詰まったこと・解決したこと
- multi-session 化で `marker-1` が各セッションに重複する問題があった。
  - 解決: simulation 生成時に `s<index>-marker-*` へ prefix して dry-run 対象を一意にした。

### 検証
- `uv run ruff check server/tools/initiative_motivation_sandbox.py server/tools/simulate_initiative_motivation.py server/tools/run_initiative_prompt_dryrun.py tests/unit/test_initiative_motivation_sandbox.py`
  - passed
- `uv run pytest -m unit tests/unit/test_initiative_motivation_sandbox.py -q`
  - 7 passed
- `make initiative-sim`
  - candidate export: utterance 180 / arrival 500
  - multi-session simulation: session_count 6 / total_snapshot_count 66 / total_fire_marker_count 6
  - `reports/initiative-motivation/session-latest.html` を生成し `open` まで実行された。

### 次のセッションでやること
- UI 上で手編集した candidate JSON を browser-side 再計算へ反映する。
- session selector で選んだ session の dry-run marker id を UI からコピーしやすくする。

## 2026-06-14 セッション45

### やること（開始時に書く）
- initiative motivation UI で `curiosity_gain` / `teasing_gain` / `attachment_gain` / `unspoken_gain` が実質効いておらず、`floor_weight` と `threshold` だけで発火が決まって見える問題を調べて直す。

### やったこと
- HTML UI の `recompute()` が Python 生成済みの pressure snapshot を再利用しており、gain 変更時に pressure を再積分していなかった問題を修正した。
- browser-side recompute を、events / candidates / params から candidate 選択、pressure decay、candidate pressure、silence attachment、score 計算までやり直す実装に変更した。
- 隠れパラメータだった `silence_attachment_gain` を UI slider に追加した。
- `Candidate JSON` の手編集を browser-side 再計算に反映するようにした。
- Python / JS の両方で、dead candidate 由来の `unspoken` 固定加算を `unspoken_gain` 配下に移した。
- 全 gain / silence attachment / floor / freshness を 0 にしたとき score が 0 になる単体テストを追加した。

### 詰まったこと・解決したこと
- `attachment_gain=0` でも attachment pressure が伸びる原因は、別の `silence_attachment_gain` が hidden parameter として効いていたことだった。
- 全 gain を 0 にしても score が残る原因は、`dismissed` / `expired` / `spoken` candidate による `unspoken += 0.01` が `unspoken_gain` を通っていなかったことだった。

### 検証
- `uv run ruff check server/tools/initiative_motivation_sandbox.py tests/unit/test_initiative_motivation_sandbox.py`
  - passed
- `uv run pytest -m unit tests/unit/test_initiative_motivation_sandbox.py -q`
  - 9 passed
- `make initiative-sim`
  - HTML / JSON を再生成
  - multi-session simulation: session_count 100 / total_snapshot_count 7026 / total_fire_marker_count 3679
- generated HTML の JS を Node で構文確認し、DOM mock 上で再計算検証
  - all gains / silence attachment / floor / freshness = 0 のとき `maxZero=0`
  - floor_weight=0.54 だけ戻すと `maxFloor=0.54`
- `uv run pytest -m unit -q`
  - 717 passed, 24 deselected, 3 warnings
- `git diff --check -- server/tools/initiative_motivation_sandbox.py tests/unit/test_initiative_motivation_sandbox.py LOG.md MEMORY.md Makefile`
  - passed

### 次のセッションでやること
- session selector で選んだ session の dry-run marker id を UI からコピーしやすくする。
- UI の parameter set を保存/復元できるようにする。

## 2026-06-14 セッション47

### やること（開始時に書く）
- initiative motivation HTML の Timeline で、会話イベントの後ろに FIRE をまとめて append するのではなく、時系列に差し込んで表示する。

### やったこと
- `renderTimeline()` を、`current.events` と再計算後の `fire2` rows を `timelineItems` に混ぜ、`ts_ms` 順でソートして描画する形に変更した。
- 同時刻では会話イベントを FIRE より先に表示するようにした。
- FIRE 行クリックで prompt preview を表示する挙動は維持した。
- HTML が差し込み timeline を生成することを unit test に追加した。
...
- `make initiative-sim`
  - HTML / JSON を再生成
  - multi-session simulation: session_count 100 / total_snapshot_count 7026 / total_fire_marker_count 3679
- generated HTML の JS を `node --check` で構文確認
  - passed
- `uv run pytest -m unit -q`
  - 719 passed, 24 deselected, 3 warnings
...
### やること（開始時に書く）
- initiative motivation HTML の parameter slider を、人格・動機系とタイミング・ゲート系が見分けられるようにする。

### やったこと
- HTML の Parameters を `Persona / Motive` と `Timing / Gate` に分けた。
- `Persona / Motive` は紫系背景にして、`curiosity_gain` / `teasing_gain` / `attachment_gain` / `unspoken_gain` / `silence_attachment_gain` を配置した。
- `Timing / Gate` は緑系背景にして、`floor_weight` / `freshness_weight` / `intrusion_weight` / speaking penalties / `rejection_penalty` / `threshold` / `decay_sec` を配置した。
- HTML に分類が出ることを unit test に追加した。
...
- generated HTML の JS を `node --check` で構文確認
  - passed
- `uv run pytest -m unit -q`
  - 718 passed, 24 deselected, 3 warnings
...
### やること（開始時に書く）
- `make initiative-sim` の session selector で Timeline に会話本文が出ず、v2 shadow の decision score だけが表示される原因を調べて直す。

### やったこと
- 原因は `--recent-sessions` が `logs/turn-taking-*.jsonl` の `conversation_session_id` だけを母集団にしており、会話原本である DB `conversation_logs` を読んでいなかったことだった。
- `simulate_initiative_motivation.py` に DB `conversation_logs` から直近 N セッション分の user / tomoko turn を読み出して timeline record に変換する処理を追加した。
- `recent_session_groups()` を、会話本文のある main records を session selector の主入力にし、v2 shadow records は同じ session_id の補助イベントとして合流する形に変更した。
- session label を `turns=N events=N` 表示にして、本文 turn が取れているか UI 上でも見えるようにした。

### 詰まったこと・解決したこと
- スクショでは `events=2` の session に `v2_shadow/speech_decision_score` しかなく、会話が欠落しているように見えた。
  - 解決: DB を確認すると `conversation_logs` には session 付き会話が 1322 行存在したため、データ欠落ではなく simulator の取得元ミスとして修正した。

### 検証
- `uv run ruff check server/tools/initiative_motivation_sandbox.py server/tools/simulate_initiative_motivation.py tests/unit/test_initiative_motivation_sandbox.py`
  - passed
- `uv run pytest -m unit tests/unit/test_initiative_motivation_sandbox.py -q`
  - 8 passed
- `make initiative-sim`
  - candidate export: utterance 180 / arrival 500
  - multi-session simulation: session_count 100 / total_snapshot_count 7026 / total_fire_marker_count 3703
  - 先頭 session の timeline に `智子を聞こえますか` / `今日のニュース調べて` / `結果教えて` などの会話本文が出ることを JSON で確認した。
- `uv run pytest -m unit -q`
  - 716 passed, 24 deselected, 3 warnings
- `git diff --check -- Makefile server/tools/initiative_motivation_sandbox.py server/tools/simulate_initiative_motivation.py tests/unit/test_initiative_motivation_sandbox.py LOG.md MEMORY.md _docs/v2-alpha.md`
  - passed
...
### やること（開始時に書く）
- LoRA 済みメインモデル `loras/lora/fused_model` を、通常の MLX ロードと dflash ロードで同条件ベンチし、最初の応答までの時間と全応答までの時間を比較する。
- 既存 dflash 起動設定と MLX 推論経路を確認し、測定結果を artifact として残す。

### やったこと
- `_tools/run_llm.sh` を確認し、dflash 26B の対象が `loras/lora/fused_model` であることを確認した。
- `mlx_lm.load("loras/lora/fused_model")` + `stream_generate` と、`dflash serve --model loras/lora/fused_model --draft z-lab/gemma-4-26B-A4B-it-DFlash --chat-template-args '{"enable_thinking": false}'` + OpenAI-compatible streaming を同じ prompt 群で比較した。
- 短い prompt と、`prompts/base_persona.md` / `prompts/persona_overlay.md` / 固定会話履歴を含む長め prompt の2条件を測定した。

### 結果
- 短い prompt:
  - 通常 MLX: first content mean 362.5ms / median 399.8ms、total mean 1269.8ms / median 1222.1ms
  - dflash: first content mean 579.9ms / median 580.9ms、total mean 3168.9ms / median 2924.0ms
- 長め prompt（system 3122 chars、prompt 約3237 chars）:
  - 通常 MLX: first content mean 3030.8ms / median 2969.3ms、total mean 3978.4ms / median 3852.1ms
  - dflash: first content mean 4646.9ms / median 3563.2ms、total mean 7155.2ms / median 7171.5ms
- 今回の dflash log では `prefill restored 0.0` / prefix cache hit 0 で、prefix cache の利点は出ていなかった。

### 検証
- artifact:
  - `logs/lora-main-mlx-vs-dflash-20260615/mlx_lm_stream_bench.json`
  - `logs/lora-main-mlx-vs-dflash-20260615/dflash_stream_bench.json`
  - `logs/lora-main-mlx-vs-dflash-20260615/mlx_lm_long_context_stream_bench.json`
  - `logs/lora-main-mlx-vs-dflash-20260615/dflash_long_context_stream_bench.json`
  - `logs/lora-main-mlx-vs-dflash-20260615/dflash-26b.log`
  - `logs/lora-main-mlx-vs-dflash-20260615/dflash-26b-long.log`
- 測定後、`http://127.0.0.1:8082/v1/models` が接続不可で dflash 8082 が停止済みであることを確認した。

### 次のセッションでやること
- dflash の prefix cache hit が出ない原因を、chat template 展開後 prompt の接頭辞形状と snapshot 条件で切り分ける。

## 2026-06-15 セッション49

### やること（開始時に書く）
- LoRA 済みメインモデル `loras/lora/fused_model` を、単発 prompt ではなく会話履歴が turn ごとに積み上がる条件で通常 MLX と dflash 比較する。
- system prompt は固定し、過去 user / assistant turn を順に追加して、first content と total latency、dflash prefix cache hit を確認する。

### やったこと
- 通常 MLX は `mlx_lm.load("loras/lora/fused_model")` でロードし、8 turn のユーザー発話に対して、生成した assistant 返答を次 turn の履歴へ追加しながら `stream_generate` で測定した。
- dflash は `dflash serve --model loras/lora/fused_model --draft z-lab/gemma-4-26B-A4B-it-DFlash --chat-template-args '{"enable_thinking": false}' --port 8082` で起動し、同じ 8 turn シナリオを OpenAI-compatible streaming で測定した。
- system は `prompts/base_persona.md` + `prompts/persona_overlay.md` に固定し、会話が進むほど messages に user / assistant turn が積まれる条件にした。

### 結果
- 通常 MLX:
  - prompt tokens: 851 -> 1450
  - first content mean 2856.9ms / median 2927.3ms / p95 3344.3ms
  - total mean 4519.6ms / median 4635.4ms / p95 6111.9ms
  - turn1 first 2075.9ms / total 3493.1ms、turn8 first 3332.9ms / total 6111.9ms
- dflash:
  - prompt content chars: 1498 -> 2488
  - first content mean 851.2ms / median 631.5ms / p95 2604.1ms
  - total mean 4392.9ms / median 3974.6ms / p95 5988.2ms
  - turn1 first 2604.1ms / total 5833.4ms、turn8 first 665.4ms / total 5988.2ms
- dflash log では、warmup / turn1 相当の2回は miss だったが、その後は prefix cache hit が 7回出た:
  - 847/932, 928/1028, 1024/1109, 1105/1201, 1197/1292, 1288/1377, 1373/1448
  - 最終 `prefill_tokens_saved=7762`

### 検証
- artifact:
  - `logs/lora-main-conversation-growth-20260615/mlx_lm_conversation_growth.json`
  - `logs/lora-main-conversation-growth-20260615/dflash_conversation_growth.json`
  - `logs/lora-main-conversation-growth-20260615/dflash-26b.log`
- 測定後、`http://127.0.0.1:8082/v1/models` が接続不可で dflash 8082 が停止済みであることを確認した。

### 次のセッションでやること
- 実 `ThinkFastMode` / `ContextSnapshotBuilder` / DB conversation logs 経由でも同じ cache hit 形状が維持されるか測る。

## 2026-06-16 セッション50

### やること（開始時に書く）
- thinker2 / thinker v2 として、カレンダー・タイマー・world information・カメラ・スクリーンショット由来の観測と candidate 生成を整理した実装計画を `thinkerv2.md` に書き出す。

### やったこと
- `thinkerv2.md` を新規作成し、thinker2 の目的、shadow worker との境界、データフロー、DB schema 案、candidate source、worker 構成、実装 Phase、テスト方針、初期完了条件を書き出した。
- camera / screenshot は raw frame を短命 artifact として保持し、DB には presence / activity / screen activity / user context snapshot を構造化して保存する方針にした。
- calendar / timer / world information / screen / activity 由来の発話は既存 `utterance_candidates` / `arrival_candidates` に載せ、TomoroSession の final gate は動かさない方針にした。

### 詰まったこと・解決したこと
- `LOG.md` には作業開始前から未コミット差分があったため、既存内容を戻さず末尾追記だけで扱った。

### 検証
- `git diff --check -- LOG.md thinkerv2.md`
  - passed

### 次のセッションでやること
- `thinkerv2.md` を元に PLAN.md へ Phase T2.* の実装計画を append する。
- 実装に入る場合は Phase T2.1 の deterministic calendar / timer candidate から開始する。

## 2026-06-16 セッション51

### やること（開始時に書く）
- `thinkerv2.md` を順番に進める。
- まず Phase T2.0 として PLAN.md へ thinker2 実装 phase と完了条件を追記する。
- その後、Phase T2.1 の deterministic calendar / timer candidate source を既存 candidate store 境界に合わせて実装する。

### やったこと
- Phase T2.0 として PLAN.md に thinker2 / thinker v2 の Phase T2.0-T2.9 と完了条件を追記し、`thinkerv2.md` の T2.0 checklist を完了にした。
- Phase T2.1 として `CalendarReminderSource` を追加し、予定開始 15 分前 / 5 分前 / 開始時刻の deterministic `CandidateSeed` を生成するようにした。
- default thinker に `CalendarReminderSource(PostgresCalendarEventStore)` を配線した。
- timer / alarm は既存の due claim と TomoroSession gate が authoritative なので、通常 candidate source として重複実装しない境界を MEMORY.md に追記した。
- `thinkerv2.md` / PLAN.md の Phase T2.1 checklist を完了にした。
- Phase T2.2 として `perception_frames` DDL、`PerceptionFrame` DTO、InMemory / PostgreSQL store を追加した。
- retention は camera / screenshot source ごとに最新 100 件を残し、それ以前は `retained=false` にする形で実装した。

### 詰まったこと・解決したこと
- focused ruff で `server/thinker/main.py` の import order 指摘が出た。
  - `uv run ruff check --fix ...` で整列し、再検証した。

### 検証
- `uv run ruff check server/thinker/main.py server/thinker/sources/calendar_reminder.py tests/unit/test_phase91_deterministic_sources.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase91_deterministic_sources.py -q`
  - 8 passed
- `uv run pytest -m unit -q`
  - 723 passed, 24 deselected, 3 warnings
- `uv run ruff check server/shared/models.py server/shared/perception.py tests/unit/test_phase_t22_perception_frames.py tests/integration/test_phase_t22_perception_frames_db.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase_t22_perception_frames.py -q`
  - 3 passed
- `uv run pytest -m integration tests/integration/test_phase_t22_perception_frames_db.py -q`
  - 1 passed
- `uv run pytest -m unit -q`（T2.2 追加後の再実行）
  - 726 passed, 25 deselected, 3 warnings

### 次のセッションでやること
- Phase T2.3 camera presence へ進み、capture worker の前に presence JSON schema / stale frame discard / backlog skip の境界を固める。

## 2026-06-16 セッション52

### やること（開始時に書く）
- `thinkerv2.md` の Phase T2.3 camera presence を続ける。
- 実カメラ / VLM runtime に直結する前に、presence JSON schema、`human_presence_observations` store、stale frame discard / backlog skip をテスト先行で固定する。

### やったこと
- `human_presence_observations` DDL と `HumanPresenceObservation` DTO、InMemory / PostgreSQL store を追加した。
- `CameraCaptureWorker` を追加し、30 秒 interval loop で camera frame を `perception_frames` に保存できる足場を作った。
- presence JSON schema を `present: bool` / `confidence: 0.0-1.0` / optional reason に固定した。
- `MlxVlmPresenceBackend` を追加し、`mlx_vlm.stream_generate(..., image=frame_path)` から JSON を読む境界を作った。
- `PresenceInferenceWorker` を追加し、最新未処理 camera frame だけを処理し、stale frame と backlog を skip count として返すようにした。
- `thinkerv2.md` / PLAN.md の Phase T2.3 checklist を完了にした。

### 詰まったこと・解決したこと
- 既存 `GemmaMLXBackend` は `mlx_vlm` を使っているが、text chat 用 wrapper で画像 path を渡す共通 contract はなかった。
  - 解決: thinker2 専用の `HumanPresenceInferenceBackend` protocol と `MlxVlmPresenceBackend` を追加し、hot path / InferenceRouter には触れずに境界を分けた。

### 検証
- `uv run ruff check server/shared/models.py server/shared/perception.py server/thinker/perception/presence.py tests/unit/test_phase_t23_camera_presence.py tests/integration/test_phase_t23_camera_presence_db.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase_t23_camera_presence.py -q`
  - 7 passed
- `uv run pytest -m integration tests/integration/test_phase_t23_camera_presence_db.py -q`
  - 1 passed
- `uv run pytest -m unit -q`
  - 733 passed, 26 deselected, 3 warnings

### 次のセッションでやること
- Phase T2.4 camera activity へ進み、activity JSON schema と `human_activity_observations` store / worker を presence と同じ境界で追加する。

### 追加でやったこと
- Phase T2.4 として `human_activity_observations` DDL と `HumanActivityObservation` DTO、InMemory / PostgreSQL store を追加した。
- activity JSON schema を `activity_label` / `confidence` / optional reason に固定した。
- `MlxVlmActivityBackend` と `HumanActivityInferenceWorker` を追加した。
- `present=false` の時は raw activity row を保存し、snapshot 合成用 helper `coherent_activity_label()` で `away` に丸める方針にした。
- `thinkerv2.md` / PLAN.md の Phase T2.4 checklist を完了にした。

### 追加検証
- `uv run ruff check server/shared/models.py server/shared/perception.py server/thinker/perception/activity.py tests/unit/test_phase_t24_camera_activity.py tests/integration/test_phase_t24_camera_activity_db.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase_t24_camera_activity.py -q`
  - 7 passed
- `uv run pytest -m integration tests/integration/test_phase_t24_camera_activity_db.py -q`
  - 1 passed
- `uv run pytest -m unit -q`
  - 740 passed, 27 deselected, 3 warnings

### 次のセッションでやること（更新）
- Phase T2.5 screenshot activity へ進み、screenshot capture worker / screen activity JSON schema / `screen_activity_observations` store を追加する。

### さらに追加でやったこと
- Phase T2.5 として `screen_activity_observations` DDL と `ScreenActivityObservation` DTO、InMemory / PostgreSQL store を追加した。
- `ScreenshotCaptureWorker`、`ScreenActivityInferenceWorker`、`MlxVlmScreenActivityBackend` を追加した。
- screen activity JSON schema を `screen_activity_label` / `confidence` / optional `app_hint` / `document_hint` / `url_hint` に固定した。
- screenshot も latest-frame 優先で、古い backlog は skip count として扱うようにした。
- `thinkerv2.md` / PLAN.md の Phase T2.5 checklist を完了にした。

### さらに追加検証
- `uv run ruff check server/shared/models.py server/shared/perception.py server/thinker/perception/screen.py tests/unit/test_phase_t25_screenshot_activity.py tests/integration/test_phase_t25_screenshot_activity_db.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase_t25_screenshot_activity.py -q`
  - 5 passed
- `uv run pytest -m integration tests/integration/test_phase_t25_screenshot_activity_db.py -q`
  - 1 passed
- `uv run pytest -m unit -q`
  - 745 passed, 28 deselected, 3 warnings

### 次のセッションでやること（T2.5 後）
- Phase T2.6 user context snapshot へ進み、latest presence / activity / screen / calendar / world を合成する `user_context_snapshots` store と deterministic `interaction_readiness` rule を追加する。

## 2026-06-16 セッション53

### やること（開始時に書く）
- `thinkerv2.md` の Phase T2.6 user context snapshot を進める。
- latest presence / activity / screen / calendar / world を合成し、`user_context_snapshots` に保存する。
- 初段は deterministic `interaction_readiness` rule とし、LLM synthesis は後続で差し替えられる境界に留める。

### やったこと
- Phase T2.6 として `user_context_snapshots` DDL と `UserContextSnapshot` DTO、InMemory / PostgreSQL store を追加した。
- `UserContextSnapshotBuilder` を追加し、latest presence / activity / screen / calendar / world interpretation を合成するようにした。
- `interaction_readiness` は deterministic rule 初段として `away` / `do_not_disturb` / `low_intrusion_ok` / `chat_ok` / `needs_help_maybe` から選ぶようにした。
- `present=false` の時は activity を `away` に丸める挙動を snapshot builder 側でも固定した。
- snapshot build の elapsed ms / source counts / skipped sources / source errors を trace と log に残すようにした。
- `thinkerv2.md` / PLAN.md の Phase T2.6 checklist を完了にした。

### 検証
- `uv run ruff check server/shared/models.py server/shared/perception.py server/thinker/perception/context_snapshot.py tests/unit/test_phase_t26_user_context_snapshot.py tests/integration/test_phase_t26_user_context_snapshot_db.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase_t26_user_context_snapshot.py -q`
  - 3 passed
- `uv run pytest -m integration tests/integration/test_phase_t26_user_context_snapshot_db.py -q`
  - 1 passed
- `uv run pytest -m unit -q`
  - 748 passed, 29 deselected, 3 warnings

### 次のセッションでやること
- Phase T2.7 context-derived candidates へ進み、screen / activity context から existing `utterance_candidates` / `arrival_candidates` に candidate を積む source を追加する。

### 追加でやったこと
- Phase T2.7 として `ScreenContextSource` / `ActivityContextSource` を追加した。
- `interaction_readiness` が `away` / `do_not_disturb` の snapshot では candidate を出さないようにした。
- `needs_help_maybe` は low intrusion の高め priority、`chat_ok` は通常 priority として candidate seed を作るようにした。
- default thinker に `PostgresUserContextSnapshotStore` と screen / activity context sources を配線した。
- candidate final gate は TomoroSession から動かさず、既存 `utterance_candidates` 経路に載せる形にした。
- `thinkerv2.md` / PLAN.md の Phase T2.7 checklist を完了にした。

### 追加検証
- `uv run ruff check server/thinker/main.py server/thinker/sources/context_snapshot.py tests/unit/test_phase_t27_context_candidates.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase_t27_context_candidates.py -q`
  - 4 passed
- `uv run pytest -m unit -q`
  - 752 passed, 29 deselected, 3 warnings

### 次のセッションでやること（T2.7 後）
- Phase T2.8 world information autonomous collection へ進み、deterministic topic seed と既存 world observation operator 境界を thinker2 から起動できる形にする。

### さらに追加でやったこと
- Phase T2.8 として `WorldInformationCollectionWorker` と deterministic `DEFAULT_WORLD_TOPIC_SEEDS` を追加した。
- daily world observation prompt template に thinker2 topic seeds を追記し、既存 `world.observe` operator client で raw Markdown artifact を保存する形にした。
- normalizer / interpretation / candidate 化は既存 pipeline を再利用し、T2.8 では再実装しない境界にした。
- `WorldObservationSource` で low confidence / stale / sensitive / private / do_not_speak interpretation を candidate 化しない hardening を追加した。
- `thinkerv2.md` / PLAN.md の Phase T2.8 checklist を完了にした。

### さらに追加検証
- `uv run ruff check server/thinker/world_information.py server/thinker/sources/world_observation.py tests/unit/test_phase_t28_world_information_collection.py tests/unit/test_phase18_world_observation_source.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase_t28_world_information_collection.py tests/unit/test_phase18_world_observation_source.py -q`
  - 4 passed
- `uv run pytest -m unit -q`
  - 755 passed, 29 deselected, 3 warnings

### 次のセッションでやること（T2.8 後）
- Phase T2.9 runtime integration and inspection へ進み、`background-process/run_thinker2.py` と `make thinker2` / `make thinker2-once`、inspection artifact を追加する。

### 最後に追加でやったこと
- Phase T2.9 として `server/thinker2/main.py`、`background-process/run_thinker2.py` を追加した。
- `Thinker2Process` は `UserContextSnapshotBuilder` を実行し、screen / activity context sources から candidate seed を既存 `utterance_candidates` に積む。
- `make thinker2` / `make thinker2-once`、`THINKER2_LOG_FILE`、`THINKER2_INTERVAL_SEC`、`THINKER2_INSPECTION_HTML` を追加した。
- `thinker2_once` で queue depth / snapshot latency / skipped stale frame / skipped backlog frame / candidate count をログに出すようにした。
- `reports/thinker2/latest.html` の inspection HTML を生成するようにした。
- `run_thinker2.py --once` は perception / snapshot 系の新規 schema を ensure してから走るようにした。
- `thinkerv2.md` / PLAN.md の Phase T2.9 checklist を完了にした。

### 最後の検証
- `uv run ruff check server/thinker2/main.py background-process/run_thinker2.py tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py`
  - passed
- `uv run pytest -m unit tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py -q`
  - 14 passed
- `uv run python background-process/run_thinker2.py --config config/central_realtime.toml --once --inspection-output reports/thinker2/latest.html`
  - readiness=low_intrusion_ok
  - candidate_generated=0
  - candidate_inserted=0
  - inspection=reports/thinker2/latest.html
- `uv run pytest -m unit -q`
  - 758 passed, 29 deselected, 3 warnings

### 次のセッションでやること（thinkerv2.md 初期計画後）
- 実機 camera / screenshot capture provider を OS 別に追加するか、まず `make thinker2-once` の inspection HTML を見ながら snapshot / candidate tuning を行う。

## 2026-06-16 セッション54

### やること（開始時に書く）
- thinker2 の E2E 動作確認を行い、不具合がないか確認する。
- 実行入口は `make thinker2-once` とし、DB schema ensure、snapshot build、context candidate generation、inspection HTML 生成まで確認する。
- 追加で新規 integration tests と full unit を再実行する。

### やったこと
- `make thinker2-once` を実行し、schema ensure、snapshot build、inspection HTML 生成まで通ることを確認した。
- `reports/thinker2/latest.html` と DB 最新 `user_context_snapshots` を確認した。
- 一時的な screenshot observation fixture を実 DB に投入し、`Thinker2Process` が `needs_help_maybe` snapshot から `screen_context` candidate を 1 件作ることを確認した。
- 上記 E2E fixture で作った `screen_context` candidate と temporary `perception_frames` row は削除した。
- inspection HTML で同じ world interpretation が 3 回表示される問題を見つけ、`UserContextSnapshotBuilder` の world summary を title/text で dedupe するよう修正した。
...
- `make thinker2-once`
  ...
  - candidate_inserted=0
  - inspection=reports/thinker2/latest.html
- temporary screenshot observation E2E
  - readiness=needs_help_maybe
  - candidate_generated_count=1
  - candidate_inserted_count=1
  - cleanup: deleted_candidates=1 / deleted_frames=1
- `uv run pytest -m integration tests/integration/test_phase_t22_perception_frames_db.py tests/integration/test_phase_t23_camera_presence_db.py tests/integration/test_phase_t24_camera_activity_db.py tests/integration/test_phase_t25_screenshot_activity_db.py tests/integration/test_phase_t26_user_context_snapshot_db.py -q`
  - 5 passed
- `uv run pytest -m unit -q`
  - 759 passed, 29 deselected, 3 warnings
- focused `uv run ruff check ...`
  - passed
- `make -n thinker2-once thinker2`
  - printed expected commands
- `git diff --check -- .`
  - passed

### 見つけた不具合・対応
- `world_summary` に同一 interpretation が重複表示される不具合があった。
  - title / interpretation_text の組で dedupe する修正と unit test を追加して解決した。

### 次のセッションでやること
- 必要なら実 OS camera / screenshot capture provider を追加する。
- その前に `reports/thinker2/latest.html` の表示内容を見ながら readiness / candidate priority の tuning を行う。

## 2026-06-16 セッション55

### やること（開始時に書く）
- `thinker2` 起動時に、最新 snapshot に入ったスクリーンショット経由 / カメラ経由の情報を console で確認できるようにする。
- `--once` と `--watch` のどちらでも同じ内容を見られるようにし、inspection HTML も同じ情報を含める。

### やったこと
- `Thinker2RunResult` に `camera_context_summary` / `screen_context_summary` を追加した。
- `run_once()` の snapshot から camera 経由の presence/activity と screenshot 経由の screen activity を console 向け summary に整形するようにした。
- `--once` と `--watch` の両方で `thinker2_context camera="..." screen="..."` を出力するようにした。
- inspection HTML にも `camera_context_summary` / `screen_context_summary` を表示するようにした。

### 検証
- red test: `uv run pytest -m unit tests/unit/test_phase_t29_thinker2_runtime.py -q`
  - `format_thinker2_console_report` 未実装で失敗することを確認した。
- focused unit: `uv run pytest -m unit tests/unit/test_phase_t29_thinker2_runtime.py -q`
  - 3 passed
- ruff: `uv run ruff check server/thinker2/main.py tests/unit/test_phase_t29_thinker2_runtime.py`
  - passed
- E2E smoke: `make thinker2-once`
  - `thinker2_context camera="no_observation" screen="no_observation" readiness=low_intrusion_ok candidates=0/0`
  - 現在の DB には camera / screenshot observation が無いため `no_observation` として表示された。
- inspection HTML: `reports/thinker2/latest.html`
  - `camera_context_summary` / `screen_context_summary` が表示されることを確認した。

### 次のセッションでやること
- 実 camera / screenshot capture provider を追加する場合は、今回の console 表示に実観測値が流れるところまで E2E で確認する。

## 2026-06-16 セッション56

### やること（開始時に書く）
- 実OSから camera / screenshot を取り込む provider を追加する。
- `thinker2` 起動時に明示フラグで実captureを走らせ、raw frame が `perception_frames` に保存されることを console で確認できるようにする。
- 通常の `make thinker2` は急にカメラ・画面取得しないようにし、試験用ターゲットを分ける。

### やったこと
- `server/thinker/perception/os_capture.py` を追加し、macOS `screencapture` ベースの screenshot provider と OpenCV ベースの camera provider を実装した。
- `server/thinker2/main.py` に `--capture-camera` / `--capture-screenshot` / `--capture-perception` / `--camera-index` / `--perception-artifact-dir` を追加した。
- capture worker は `run_once()` の snapshot build 前に実行し、成功した raw frame を `perception_frames` に保存するようにした。
- capture 失敗時は thinker2 全体を落とさず、`capture_errors` として console / inspection HTML / log に出すようにした。
- `Makefile` に `thinker2-capture` / `thinker2-capture-once` を追加した。

### 検証
- red test: `uv run pytest -m unit tests/unit/test_phase_t30_os_capture_providers.py tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py -q`
  - provider module / result DTO 未実装で失敗することを確認した。
- focused unit: `uv run pytest -m unit tests/unit/test_phase_t30_os_capture_providers.py tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py -q`
  - 18 passed
- ruff: `uv run ruff check server/thinker/perception/os_capture.py server/thinker2/main.py tests/unit/test_phase_t30_os_capture_providers.py tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py`
  - passed
- dry-run: `make -n thinker2-capture-once thinker2-capture`
  - `--capture-perception` / `--camera-index` / `--perception-artifact-dir` が入ることを確認した。
- E2E smoke: `make thinker2-capture-once`
  - screenshot capture は成功し、`logs/perception/screenshot/20260616T050154829940Z-screenshot.png` を保存した。
  - camera capture は macOS 権限未許可で `CameraCaptureWorker: camera index 0 could not be opened` として `capture_errors` に出た。
  - thinker2 process は exit 0 で継続し、console に captured screenshot と camera error を表示した。
- artifact 確認:
  - screenshot PNG は 2048x1330 / RGBA / 995071 bytes。
  - `reports/thinker2/latest.html` に `captured_frames` / `capture_errors` が表示された。
  - DB `perception_frames` に screenshot row が `retained=True` で保存された。
- full unit: `uv run pytest -m unit -q`
  - 763 passed, 29 deselected, 3 warnings
- whitespace: `git diff --check -- ...`
  - passed

### 次のセッションでやること
- macOS の Camera 権限を許可した後、`make thinker2-capture-once` を再実行して camera frame 保存まで確認する。
- raw frame capture の次段として、必要なら VLM presence / activity / screen inference worker も `thinker2-capture-*` に明示フラグで配線する。

### 追加で見つけた問題・対応
- `make thinker2-capture-once` の camera frame がほぼ真っ黒になる問題を確認した。
  - 最新 camera jpg の平均輝度は RGB で約 0.25 で、実際の表示もほぼ黒だった。
  - OpenCV で 30 frame / 0.05 秒ずつウォームアップして最後の frame を保存すると、平均輝度が約 60〜85 まで上がることを確認した。
- `OpenCVCameraCaptureProvider` に `warmup_frames=30` / `warmup_delay_sec=0.05` を追加し、最初の暗い frame ではなくウォームアップ後の frame を保存するようにした。

### 追加検証
- manual warmup capture
  - `logs/perception/camera/manual-warmup-test.jpg`
  - 1920x1080、平均輝度 RGB 約 85.65 / 75.04 / 68.08
- `make thinker2-capture-once`
  - `logs/perception/camera/20260616T051035263246Z-camera.jpg`
  - 1920x1080、平均輝度 RGB 約 79.6 / 70.61 / 60.86
  - 表示確認でも黒フレーム問題が解消していることを確認した。
- focused unit: `uv run pytest -m unit tests/unit/test_phase_t30_os_capture_providers.py tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py -q`
  - 18 passed
- focused ruff: `uv run ruff check server/thinker/perception/os_capture.py tests/unit/test_phase_t30_os_capture_providers.py`
  - passed

### さらに追加でやったこと
- `thinker2-capture-once` で raw capture 後に VLM inference を走らせ、presence / camera activity / screen activity を console に出すようにした。
- `--infer-perception` / `--vlm-model` を追加し、`make thinker2-capture-once` / `make thinker2-capture` では inference まで走るようにした。
- 既定の VLM model を `mlx-community/gemma-4-e2b-it-4bit` から `mlx-community/Qwen2.5-VL-3B-Instruct-4bit` に変更した。
  - Gemma は画像入力で空文字・特殊トークン・壊れた JSON を返し、この用途では使えなかった。
  - Qwen2.5-VL は screen activity で JSON を返せた。
- VLM が複数 JSON を繰り返す出力でも、最初に decode できる JSON object を採用する parser にした。
- presence が `present=False` の場合は camera activity VLM を走らせず、同じ frame に `activity_label=away` / `model=presence_gate` を保存するようにした。

### さらに追加検証
- red test: `uv run pytest -m unit tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py -q`
  - `PerceptionInferenceSummary` 未実装で失敗することを確認した。
- focused unit: `uv run pytest -m unit tests/unit/test_phase_t23_camera_presence.py tests/unit/test_phase_t24_camera_activity.py tests/unit/test_phase_t25_screenshot_activity.py tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py -q`
  - 36 passed
- focused ruff: `uv run ruff check server/thinker/perception/presence.py server/thinker/perception/activity.py server/thinker/perception/screen.py server/thinker2/main.py tests/unit/test_phase_t23_camera_presence.py tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py`
  - passed
- E2E: `make thinker2-capture-once`
  - model download 後、presence / camera_activity / screen_activity が console に出ることを確認した。
  - 最新実行では `presence:present=False`, `camera_activity:away`, `screen_activity:Clicking on the 'Home' button` が表示された。
  - `perception_errors="none"` で完了した。
- full unit: `uv run pytest -m unit -q`
  - 764 passed, 29 deselected, 3 warnings
- whitespace: `git diff --check -- .`
  - passed

### OCR/OS metadata 補正の追加対応
- screenshot activity は VLM だけだと `Clicking on the 'Home' button` のような画面部品に引っ張られることを確認した。
- `MlxVlmScreenActivityBackend` で Tesseract OCR の結果を prompt に入れたうえで、macOS の front app / window title / Chrome title / URL を補助証拠として取得するようにした。
- OCR/OS metadata に `thinker2` が出ている場合は、VLM の generic label より証拠を優先し、`reviewing thinker2 inspection and capture output` に補正するようにした。
- 元の VLM label は `raw_reason_json.vlm_screen_activity_label` に残し、補正根拠は `refined_by=ocr_os_metadata` として保存する。

### OCR/OS metadata 補正の検証
- focused unit: `uv run pytest -m unit tests/unit/test_phase_t25_screenshot_activity.py -q`
  - 6 passed
- focused ruff: `uv run ruff check server/thinker/perception/screen.py tests/unit/test_phase_t25_screenshot_activity.py`
  - passed
- related unit: `uv run pytest -m unit tests/unit/test_phase_t23_camera_presence.py tests/unit/test_phase_t24_camera_activity.py tests/unit/test_phase_t25_screenshot_activity.py tests/unit/test_phase_t29_thinker2_runtime.py tests/unit/test_makefile_process_entries.py -q`
  - 38 passed
- E2E: `make thinker2-capture-once`
  - `screen_activity:reviewing thinker2 inspection and capture output conf=0.95`
  - `app=Codex document=thinker2 inspection url=file:///Users/seijiro/Sync/sync_work/by-llms/tomoko/reports/thinker2/latest.html`
  - `perception_errors="none"` で完了した。

### 長めの screen context 出力の追加対応
- `screen_activity_label` だけでは文脈が薄いため、VLM prompt に `summary` / `visible_text_evidence` / `task_context` を追加した。
- `MlxVlmScreenActivityBackend` の既定 `max_tokens` を 180 に増やし、短い label と長めの文脈を同時に返せるようにした。
- Qwen2.5-VL 3B は長め JSON で一度 JSON を崩したため、OCR/OS metadata に強い証拠がある場合は VLM JSON parse 失敗でも fallback result を作り、OCR/OS metadata 補正で observation を保存するようにした。
- thinker2 console detail は `summary=` / `task=` を `reason=` と一緒に表示するようにした。

### 長めの screen context 出力の検証
- focused unit: `uv run pytest -m unit tests/unit/test_phase_t25_screenshot_activity.py tests/unit/test_phase_t29_thinker2_runtime.py -q`
  - 11 passed
- focused ruff: `uv run ruff check server/thinker/perception/screen.py server/thinker2/main.py tests/unit/test_phase_t25_screenshot_activity.py tests/unit/test_phase_t29_thinker2_runtime.py`
  - passed
- E2E: `make thinker2-capture-once`
  - `screen_activity:reviewing thinker2 inspection and capture output conf=0.95`
  - console detail に `summary=The screen appears to be showing thinker2 inspection or capture output...` と `task=validating thinker2 perception capture and screen interpretation` が出ることを確認した。
  - `perception_errors="none"` で完了した。

### screen inference 2段階化の追加対応
- VLM に長文 summary まで出させると JSON が崩れるため、VLM の責務を coarse visual classifier に縮小した。
- 1段目 VLM は `watching_video` / `playing_game` / `coding_or_terminal` / `reading_document_or_web` / `reviewing_report_or_logs` / `communication_app` の boolean と短い label だけを返す schema にした。
- 2段目は OCR text / macOS front app / window title / Chrome title / URL / VLM coarse signal を text evidence として merge し、`summary` / `task_context` / `visible_text_evidence` を作るようにした。
- VLM が coarse JSON すら崩した場合も、OCR/OS metadata に強い証拠があれば observation は落とさず、`vlm_raw_output_prefix` を raw reason に残すようにした。

### screen inference 2段階化の検証
- focused unit: `uv run pytest -m unit tests/unit/test_phase_t25_screenshot_activity.py tests/unit/test_phase_t29_thinker2_runtime.py -q`
  - 11 passed
- focused ruff: `uv run ruff check server/thinker/perception/screen.py server/thinker2/main.py tests/unit/test_phase_t25_screenshot_activity.py tests/unit/test_phase_t29_thinker2_runtime.py`
  - passed
- E2E: `make thinker2-capture-once`
  - Qwen2.5-VL 3B はこの画面でも `vlm_json_parse_failed` になったが、OCR/OS metadata merge により `perception_errors="none"` で完了した。
  - console detail は `summary=The screen looks like validating thinker2 perception capture and screen interpretation...` と `task=validating thinker2 perception capture and screen interpretation` を出した。
- full unit: `uv run pytest -m unit -q`
  - 767 passed, 29 deselected, 3 warnings
- whitespace: `git diff --check -- .`
  - passed

### screen VLM structured output の追加対応
- `mlx_vlm 0.5.0` の `stream_generate` には OpenAI API の `response_format/json_schema` 相当の constrained structured output は見当たらなかった。
- JSON を prompt で依頼する方式では 3B/7B とも崩れる回があったため、VLM 1段目は固定行 structured output に変更した。
- 形式は `SCREEN_ACTIVITY_LABEL=...` / `CONFIDENCE=...` / `WATCHING_VIDEO=0` / `PLAYING_GAME=0` のような `KEY=VALUE` 行にし、deterministic parser で必要な行だけ読む。
- 後方互換として JSON parser fallback は残した。

### screen VLM structured output の検証
- focused unit: `uv run pytest -m unit tests/unit/test_phase_t25_screenshot_activity.py tests/unit/test_phase_t29_thinker2_runtime.py -q`
  - 13 passed
- focused ruff: `uv run ruff check server/thinker/perception/screen.py tests/unit/test_phase_t25_screenshot_activity.py`
  - passed
- E2E 3B: `make thinker2-capture-once`
  - `perception_errors="none"` で完了した。
  - screen detail に `The visual classifier flags document/web` が入り、固定行 structured output が parse されたことを確認した。
- E2E 7B: `THINKER2_VLM_MODEL=mlx-community/Qwen2.5-VL-7B-Instruct-4bit make thinker2-capture-once`
  - `perception_errors="none"` で完了した。
  - 7B は camera activity を `unknown` に寄せ、screen coarse flag は保守的だったが、fixed line parser で落ちずに処理できた。
- full unit: `uv run pytest -m unit -q`
  - 769 passed, 29 deselected, 3 warnings
- whitespace: `git diff --check -- .`
  - passed
...
### やること（開始時に書く）
- `make background-once` で world information の収集から ingest / interpret / thinker candidate 生成まで一括で進むようにする。
- dry-run と実行本体の target 構成がずれないように unit test で固定する。

### やったこと
- `background-once` の依存 target に `information-collect-world` を追加した。
- 実行順は `information-collect-world` → `information-ingest-once` → `information-interpret-once` → `thinker-once` として、world observation の収集から DB interpretation、candidate seed 生成まで一括で進むようにした。
- `test_makefile_has_grouped_background_maintenance_entries` で `background-once` に world collection が含まれ、ingest / interpret / thinker より前に実行されることを固定した。

### 検証
- red test: `uv run pytest -m unit tests/unit/test_makefile_process_entries.py::test_makefile_has_grouped_background_maintenance_entries -q`
  - `information-collect-world` が `background-once` に無いため失敗することを確認した。
- focused unit: `uv run pytest -m unit tests/unit/test_makefile_process_entries.py -q`
  - 12 passed
- dry-run: `make -n background-once`
  - `collect_world_observation.py` → `ingest_world_observations.py --once` → `interpret_world_observations.py --once` → `run_thinker.py --once` の順に出ることを確認した。
- full unit: `uv run pytest -m unit -q`
  - 769 passed, 29 deselected, 3 warnings
- whitespace: `git diff --check -- .`
  - passed
...
### 次のセッションでやること
- 実運用で定期実行する場合は、`make daily` または LaunchAgent / cron から `make background-once` を呼ぶ。
- world collection は timeout が長いため、重複起動しない運用にする。
