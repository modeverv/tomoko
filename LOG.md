...
- lora/ ディレクトリを作成し、以下の一式を開発完了：
  ...
  - `generate_data.py`: 指定されたシステムプロンプトに沿ったデータセット（チャット対話 JSONL）を自動生成するスクリプト（Ollama / MLX-LM 両対応）
  - `train.sh`: mlx-lm を使った LoRA ファインチューニング実行用のシェルスクリプト
  - `evaluate.py`: 学習済み LoRA モデルのバッチ評価およびインタラクティブチャットモード
  - `README.md`: 使用方法のドキュメント
- `tests/unit/test_lora_programs.py` に新規追加機能に対するユニットテストを追加
- `pytest -m unit` で全てのユニットテスト（651 passed）が通過することを確認
