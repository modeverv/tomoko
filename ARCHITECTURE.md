...
ユーザーが明示した task-like working context は、DB 永続化された
`task_ledger_entries` の structured row として扱う。

`ContextSnapshotBuilder` は `task_ledger` source を読み、active task の軽量 slice を
`TomokoContextSnapshot.task_ledger_entries` として返す。
fast / normal では active task の先頭10件を低コストで復帰する。
deep / reflective では、ユーザーがタスク詳細や残タスクを求めた時により多い active task を読めるよう、
policy の上限だけを広げる。

task ledger は会話原本ではなく、現在の作業状態を表す structured working state である。
source of truth は PostgreSQL に置き、ContextSnapshotBuilder は読み取り専用のままにする。
task の追加・完了・取消をどの transcript から deterministic に反映するかは別 Phase で扱う、
という未確定状態は 2026-06-02 の voice create/complete Phase で一部否定する。

会話 transcript からの task ledger 操作は create / complete のみに限定する。
update / cancel は row を変更せず unsupported として扱い、
ユーザーには「完了して新しく作る」運用を促す。

create は rule-first detector が明示 cue を拾い、normalized title 由来の deterministic id で
active task row を upsert する。
complete は active task title の exact / normalized partial match を先に試す。
一意に決まらない場合だけ background の structured extractor に active task id 候補を出させるが、
DB 更新の最終判断は deterministic validator が持つ。
single candidate / active id / confidence threshold を満たさない出力は
`needs_confirmation` として扱い、task row を変更しない。

TomoroSession は `task_ledger_requested` / `task_ledger_update_finished` を reducer で扱い、
...
## 2026-06-07 追記: calendar/task context は active session scoped cache にする

`calendar_events` と `task_ledger` を通常 TTL だけで扱う方針は否定する。
会話セッション中に一度読んだ予定/タスク context は、同じ session_id の後続 turn で再利用してよい。
一方で、session boundary を越えて無期限に使うことはしない。

`ContextSnapshotBuilder` は `active_session_id` が存在する時だけ session scoped cache を使う。
cache key には session_id と source key を含める。
session_id が変わると session scoped cache を prune するが、key 自体にも session_id があるため、
並行 build や await の前後で別 session の値を誤用しない。

`active_session_id=None` の場合は従来 TTL cache のままとする。
startup warm-up や ambient context が会話 session cache と混ざらないようにするためである。

`same_session_turns` は cache しない。
`task_ledger` は create / complete 成功時に source invalidate する。
この invalidation は `TaskLedgerUpdateResult.status` が `created` / `completed` の時だけ行う。

## 2026-06-07 追記: final user message は stable structured context を先頭へ寄せる

最後の user message で、すべての context を current user utterance の後ろに置く方針を更新する。
session 中に安定しやすい structured context は current user utterance の前に置ける。
...
現在発話への追従性を守るため、memory / research / current time は current utterance の後ろに残す。
calendar/task/persona は structured で用途が限定され、session scoped cache とも整合するため前へ寄せる。

実測では fast depth はほぼ同等、deep depth は first text が悪化する場合もあったが total は改善した。
calendar 41件 + task 3件の deep scenario では first / total とも改善した。
このため「常に first text が速い」変更ではなく、
structured context を含む会話で prompt prefix を安定させるための実装として扱う。
