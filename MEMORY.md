...
これにより、約2,700文字のフル人格プロンプト（Flag OFF）を使用せずとも、約500文字程度（Flag ON相当の基礎ルール＋100文字以下の minimal_persona.md）で、LoRAによるトモコの人格を十分にトリガー可能であることが実証された。システムプロンプト肥大化によるプレフィル遅延を削減する上で、極小人格設定の付与は極めて有効なアプローチであると決定する。
